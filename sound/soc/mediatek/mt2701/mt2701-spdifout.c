...
static int mt2701_spdifout_open(struct snd_pcm_substream *substream)
{
	int ret = 0;
	...
	#ifdef CONFIG_MTK_LEGACY_CLOCK
	enable_clock(MT_CG_AUDIO_SPDF2_CK, "AUDIO"); /*AUDIO_TOP_CON0[22]*/
	#else
	afe_msk_write(AUDIO_TOP_CON0, 0, PDN_SPDIF2_CK);
	#endif
	return ret;
}

static int mt2701_spdifout_close(struct snd_pcm_substream *substream)
{
	int i = 0;
	...
	#else
	afe_msk_write(AUDIO_TOP_CON0, PDN_SPDIF2_CK, PDN_SPDIF2_CK);
	#endif
	return i;
}


static int mt2701_spdifout_params(struct snd_pcm_substream *substream,
...
static int mt2701_spdifout_free(struct snd_pcm_substream *substream)
{
	pr_debug("%s()\n", __func__);
	return snd_pcm_lib_free_pages(substream);
}

static int mt2701_spdifout_prepare(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
...
static int mt2701_spdifout_start(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	...
	vAudioClockSetting(sample_rate_idx, 128, APLL_SPDIF2_CK, PCM_OUTPUT_32BIT, 0, 0);
...
static int mt2701_spdifout_stop(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct mt_stream *s = runtime->private_data;
	enum afe_sampling_rate sample_rate_idx = fs_enum(runtime->rate);
	...
	vAudioClockSetting(sample_rate_idx, 128, APLL_SPDIF2_CK, PCM_OUTPUT_32BIT, 0, 1);	/* power down clk */
	return 0;
}

static int mt2701_spdifout_trigger(struct snd_pcm_substream *substream, int cmd)
{
	pr_debug("%s() cmd = %d\n", __func__, cmd);
	switch (cmd) {
	case SNDRV_PCM_TRIGGER_START:
	case SNDRV_PCM_TRIGGER_RESUME:
		return mt2701_spdifout_start(substream);
	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
		return mt2701_spdifout_stop(substream);
	default:
...
static snd_pcm_uframes_t mt2701_spdifout_pointer(struct snd_pcm_substream *substream)
{
	int offset = 0;
...
static struct snd_pcm_ops spdif_iec2_ops = {
	.open = mt2701_spdifout_open,
	.close = mt2701_spdifout_close,
	.ioctl = snd_pcm_lib_ioctl,
	.hw_params = mt2701_spdifout_params,
	.hw_free = mt2701_spdifout_free,
	.prepare = mt2701_spdifout_prepare,
	.trigger = mt2701_spdifout_trigger,
	.pointer = mt2701_spdifout_pointer,
};
