...
static irqreturn_t asrc_isr(int irq, void *dev)
{
	...
	afe_mem_asrc_irq_clear(priv->id, status);
	return IRQ_HANDLED;
}
...
static int asrc_open(struct inode *node, struct file *filp)
{
	...
	if (snd_dma_alloc_pages(SNDRV_DMA_TYPE_DEV, 0, 128 * 1024, &priv->input_dmab) < 0) {
		pr_err("%s() error: snd_dma_alloc_pages failed for input buffer\n", __func__);
		goto input_dma_alloc_error;
	}
	if (snd_dma_alloc_pages(SNDRV_DMA_TYPE_DEV, 0, 128 * 1024, &priv->output_dmab) < 0) {
		pr_err("%s() error: snd_dma_alloc_pages failed for output buffer\n", __func__);
		goto output_dma_alloc_error;
	}
	{
		static const char *isr_names[MEM_ASRC_NUM] = {
			"asrc1-isr", "asrc2-isr", "asrc3-isr", "asrc4-isr", "asrc5-isr"
		};
		afe_mem_asrc_register_irq(id, asrc_isr, isr_names[id], priv);
	}
	priv->id = id;
	...
	return 0;
output_dma_alloc_error:
	snd_dma_free_pages(&priv->input_dmab);
input_dma_alloc_error:
	kzfree(priv);
open_error:
	return -EINVAL;
}
...
static ssize_t asrc_read(struct file *filp, char __user *buf, size_t count, loff_t *offset)
{
	...
	if (rp < wp) {
		...
		if (copy_to_user(buf, copy_start, copy) == 0) {
			...
			afe_mem_asrc_irq_enable(id, OBUF_OV_INT, 1);
			return copy;
		} else {
...
static ssize_t asrc_write(struct file *filp, const char __user *buf, size_t count, loff_t *offset)
{
	...
	if (wp < rp) {
		...
		if (copy_from_user(copy_start, buf, copy) == 0) {
			...
			afe_mem_asrc_irq_enable(id, IBUF_EMPTY_INT, 1);
			return copy;
		} else {
