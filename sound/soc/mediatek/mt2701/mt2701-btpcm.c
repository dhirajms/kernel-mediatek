...
static int mt2701_btpcm_set_sysclk(struct snd_soc_dai *dai, int clk_id, unsigned int freq, int dir)
{
...
static int mt2701_btpcm_set_clkdiv(struct snd_soc_dai *dai, int div_id, int div)
{
	struct mt_dai_private *priv;
...
static int mt2701_btpcm_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	struct mt_dai_private *priv;
...
static int mt2701_btpcm_hw_params(struct snd_pcm_substream *substream,
...
static int mt2701_btpcm_startup(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	struct mt_dai_private *priv;
	...
	if (priv->btpcm.occupied[substream->stream]) {
		pr_err(
		       "%s() error: can't open MT2701_DAI_BTPCM_ID(dir %d) because it has been occupied\n",
		       __func__, substream->stream);
		return -EINVAL;
	}
	if (priv->daibt.occupied[substream->stream]) {
		pr_err("%s() error: can't open MT2701_DAI_BTPCM_ID(dir %d) because MT2701_DAI_MRGIF_BT_ID(dir %d) has been occupied\n",
		       __func__, substream->stream, substream->stream);
		return -EINVAL;
	}
	#ifdef AUD_PINCTRL_SUPPORTING
	mt2701_GPIO_PCM_Select(PCM_MODE1_PCM);
	#else
...
static void mt2701_btpcm_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	struct mt_dai_private *priv;
...
static struct snd_soc_dai_ops mt2701_btpcm_ops = {
	.set_sysclk = mt2701_btpcm_set_sysclk,
	.set_clkdiv = mt2701_btpcm_set_clkdiv,
	.set_fmt = mt2701_btpcm_set_fmt,
	.startup = mt2701_btpcm_startup,
	.shutdown = mt2701_btpcm_shutdown,
	.hw_params = mt2701_btpcm_hw_params,
};
