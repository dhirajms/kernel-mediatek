...
static const struct snd_pcm_hardware mt2701_lp_pcm_hardware = {
...
static int mt2701_lp_pcm_open(struct snd_pcm_substream *substream)
{
	volatile struct lp_info *lp;

	pr_debug("%s()\n", __func__);
	lp = substream->runtime->private_data = bitstream_sram;
	snd_soc_set_runtime_hwparams(substream, &mt2701_lp_pcm_hardware);
	afe_power_mode(LOW_POWER_MODE);
	lp_substream = substream;
	return 0;
}

static int mt2701_lp_pcm_close(struct snd_pcm_substream *substream)
{
	pr_debug("%s()\n", __func__);
	lp_substream = NULL;
...
static int mt2701_lp_pcm_hw_params(struct snd_pcm_substream *substream,
...
static int mt2701_lp_pcm_hw_free(struct snd_pcm_substream *substream)
{
	pr_debug("%s()\n", __func__);
	return snd_pcm_lib_free_pages(substream);
}

static int mt2701_lp_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	);
	{
		struct audio_irq_config config = {
			...
			.init_val = 480 /* about 10ms */ /* runtime->buffer_size * 3 / 4 */
		};
...
static int mt2701_lp_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	volatile struct lp_info *lp = (volatile struct lp_info *)substream->runtime->private_data;
...
static snd_pcm_uframes_t mt2701_lp_pcm_pointer(struct snd_pcm_substream *substream)
{
...
static struct snd_pcm_ops mt2701_lp_pcm_ops = {
	.open = mt2701_lp_pcm_open,
	.close = mt2701_lp_pcm_close,
	.ioctl = snd_pcm_lib_ioctl,
	.hw_params = mt2701_lp_pcm_hw_params,
	.hw_free = mt2701_lp_pcm_hw_free,
	.prepare = mt2701_lp_pcm_prepare,
	.trigger = mt2701_lp_pcm_trigger,
	.pointer = mt2701_lp_pcm_pointer,
};

static int mt2701_lp_pcm_new(struct snd_soc_pcm_runtime *rtd)
{
	pr_debug("%s()\n", __func__);
...
static void mt2701_lp_pcm_free(struct snd_pcm *pcm)
{
	pr_debug("%s()\n", __func__);
	snd_pcm_lib_preallocate_free_for_all(pcm);
}

static int mt2701_lp_pcm_probe(struct snd_soc_platform *platform)
{
	pr_debug("%s()\n", __func__);
...
static int mt2701_lp_pcm_remove(struct snd_soc_platform *platform)
{
	pr_debug("%s()\n", __func__);
...
static struct snd_soc_platform_driver mt2701_lp_soc_platform_driver = {
	.probe = mt2701_lp_pcm_probe,
	.remove = mt2701_lp_pcm_remove,
	.pcm_new = mt2701_lp_pcm_new,
	.pcm_free = mt2701_lp_pcm_free,
	.ops = &mt2701_lp_pcm_ops,
};
...
static int mt2701_lp_audio_probe(struct platform_device *pdev)
{
	...
	#ifdef CONFIG_OF
	if (dev->of_node) {
		dev_set_name(dev, "%s", "mt2701-lp-audio");
	...
	#endif
	/*
	phy_write(0x11018000, 0x6);
	phy_write(0x11018008, 0x14);
	*/
	cmsys_write(0, 0x6);
	cmsys_write(8, 0x14);
	/* afe_msk_write(CLK_CFG_14, CLK_CMSYS_SEL_CLK26M, CLK_CMSYS_SEL_MASK); */
	/* afe_msk_write(CLK_CFG_14, CLK_CMSYS_SEL_SYSPLL_D5, CLK_CMSYS_SEL_MASK); */
	#ifdef CONFIG_MTK_LEGACY_CLOCK
	phy_msk_write(CLK_CFG_14, CLK_CMSYS_SEL_UNIVPLL1_D2, CLK_CMSYS_SEL_MASK);
	#else
	/* todo */
	#endif
	/*
	phy_write(0x11018008, 0x15);
	phy_write(0x11018000, 0x7);*/
	cmsys_write(8, 0x15);
	cmsys_write(0, 0x7);

	return snd_soc_register_platform(&pdev->dev, &mt2701_lp_soc_platform_driver);
}

static int mt2701_lp_audio_remove(struct platform_device *pdev)
{
	pr_debug("%s()\n", __func__);
...
#ifdef CONFIG_OF
static const struct of_device_id mt2701_lp_audio_dt_match[] = {
	{.compatible = "mediatek,mt2701-lp-audio",},
	{}
};
#endif
static struct platform_driver mt2701_lp_audio = {
	.driver = {
		.name = "mt2701-lp-audio",
		.owner = THIS_MODULE,
		#ifdef CONFIG_OF
		.of_match_table = mt2701_lp_audio_dt_match,
		#endif
		},
	.probe = mt2701_lp_audio_probe,
	.remove = mt2701_lp_audio_remove
};

module_platform_driver(mt2701_lp_audio);

MODULE_DESCRIPTION("mt2701 low-power audio driver");
MODULE_LICENSE("GPL");
