...
int spdif_chst_flag = 0;
int hdmi_chst_flag = 0;
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	memcpy((void *)ucontrol->value.bytes.data + sizeof(rate) + sizeof(state->u_bit),
	       (void *)state->c_bit, sizeof(state->c_bit));
	pr_debug("%s() rate=0x%X\n", __func__, rate);
	for (i = 0; i < 4; i++)
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	enum afe_spdifrx_port port = (enum afe_spdifrx_port)ucontrol->value.integer.value[0];
...
static int dummy_speaker_info_volume(struct snd_kcontrol *kctl, struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->count = 1;
...
static int dummy_speaker_get_volume(struct snd_kcontrol *kctl, struct snd_ctl_elem_value *ucontrol)
{
...
static int dummy_speaker_put_volume(struct snd_kcontrol *kctl, struct snd_ctl_elem_value *ucontrol)
{
	if (DUMMY_SPK_VOL_MAX < (unsigned int)(ucontrol->value.integer.value[0])) {
		pr_debug("dummy_speaker_put_volume() get invalid vol: %d\n",
			 (unsigned int)(ucontrol->value.integer.value[0]));
		g_dummy_spk_vol = DUMMY_SPK_VOL_MAX;
	} else
		g_dummy_spk_vol = (unsigned int)(ucontrol->value.integer.value[0]);
...
static int dummy_speaker_info_switch(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
	if (NULL == uinfo) {
...
static int spdif_out_info(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_IEC958;
	uinfo->count = 5;
	return 0;
}
static int spdif_out_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("%s() ch0=0x%x, ch1=0x%x\n", __func__, spdifinfo.ch_status0, spdifinfo.ch_status1);
	/*spin_unlock_irq(&spdifinfo->reg_lock);*/
	return 0;
}

static int spdif_out_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	/*spin_lock_irq(&spdifinfo->reg_lock);*/
	change = ((spdifinfo.ch_status0 != ch_status0_temp) || (spdifinfo.ch_status1 != ch_status1_temp));
	...
	if (change) {
		afe_msk_write(AFE_IEC2_CHL_STAT0, spdifinfo.ch_status0, 0xffffffff);
		afe_msk_write(AFE_IEC2_CHR_STAT0, spdifinfo.ch_status0, 0xffffffff);
		afe_msk_write(AFE_IEC2_CHL_STAT1, spdifinfo.ch_status1, 0x0000ffff);
		afe_msk_write(AFE_IEC2_CHR_STAT1, spdifinfo.ch_status1, 0x0000ffff);
	} else
...
static int hdmi_out_info(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_IEC958;
	uinfo->count = 5;
	return 0;
}
static int hdmi_out_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	/*spin_lock_irq(&spdifinfo->reg_lock);*/
	...
	pr_debug("%s() ch0=0x%x, ch1=0x%x\n", __func__, hdmiinfo.ch_status0, hdmiinfo.ch_status1);
	/*spin_unlock_irq(&spdifinfo->reg_lock);*/
	return 0;
}

static int hdmi_out_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	unsigned int ch_status0_temp = 0;
	unsigned int ch_status1_temp = 0;
	int change = 0;
	...
	ch_status1_temp = ((u32)ucontrol->value.iec958.status[4] << 0) |
			  ...
			  ((u32)ucontrol->value.iec958.status[7] << 24);
	/*spin_lock_irq(&spdifinfo->reg_lock);*/
	change = ((hdmiinfo.ch_status0 != ch_status0_temp) || (hdmiinfo.ch_status1 != ch_status1_temp));
	...
	if (change) {
		afe_msk_write(AFE_IEC_CHL_STAT0, hdmiinfo.ch_status0, 0xffffffff);
		afe_msk_write(AFE_IEC_CHR_STAT0, hdmiinfo.ch_status0, 0xffffffff);
		afe_msk_write(AFE_IEC_CHL_STAT1, hdmiinfo.ch_status1, 0x0000ffff);
		afe_msk_write(AFE_IEC_CHR_STAT1, hdmiinfo.ch_status1, 0x0000ffff);
	} else
...
static const struct snd_kcontrol_new mt2701_soc_controls[] = {
	...
	},
	{
		...
		.name = SNDRV_CTL_NAME_IEC958("spdif-set-chst ", PLAYBACK, DEFAULT),
		.info = spdif_out_info,
		.get = spdif_out_get,
		.put = spdif_out_put
	},
	{
		.iface = SNDRV_CTL_ELEM_IFACE_PCM,
		.name = SNDRV_CTL_NAME_IEC958("hdmi-set-chst ", PLAYBACK, DEFAULT),
		.info = hdmi_out_info,
		.get = hdmi_out_get,
		.put = hdmi_out_put
	},
