...
static void mrg_power(int on)
{
	if (on) {
		...
		mt2701_GPIO_PCM_Select(PCM_MODE2_MRG);
		#else
...
static int mt2701_btmrg_startup(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	if (priv->daibt.occupied[substream->stream]) {
		pr_err(
		       "%s() error: can't open MT2701_DAI_MRGIF_BT_ID(dir %d) because it has been occupied\n",
		       __func__, substream->stream);
		return -EINVAL;
	}
	if (priv->btpcm.occupied[substream->stream]) {
		pr_err("%s() error: can't open MT2701_DAI_MRGIF_BT_ID(dir %d) because MT2701_DAI_BTPCM_ID(dir %d) has been occupied\n",
...
static int mt2701_i2smrg_startup(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	struct mt_dai_private *priv;
	...
	if (i2s_all->i2s_out[AFE_I2S_OUT_6].occupied) {
		pr_err(
		       "%s() error: can't open MT2701_DAI_MRGIF_I2S_ID because MT2701_DAI_I2S6_ID has been occupied\n",
		       __func__);
		return -EINVAL;
	}
	if (priv->i2smrg.occupied) {
		pr_err(
		       "%s() error: can't open MT2701_DAI_MRGIF_I2S_ID because it has been occupied\n",
		       __func__);
		return -EINVAL;
	}
	mrg_power(1);
...
static int mt2701_i2smrg_hw_params(struct snd_pcm_substream *substream,
				   struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	struct mt_dai_private *priv;
	int stream_fs;
	...
	pr_debug("%s() cpu_dai id %d\n", __func__, dai->id);
	stream_fs = params_rate(params);
	if ((32000 != stream_fs) && (44100 != stream_fs) && (48000 != stream_fs)) {
		pr_err("%s() btmgr not supprt this stream_fs %d\n", __func__, stream_fs);
		return -EINVAL;
	}
	priv = dev_get_drvdata(dai->dev);
...
static void mt2701_i2smrg_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	struct mt_dai_private *priv;
...
static struct snd_soc_dai_ops mt2701_i2smrg_ops = {
	.startup = mt2701_i2smrg_startup,
	.hw_params = mt2701_i2smrg_hw_params,
	.shutdown = mt2701_i2smrg_shutdown,
};
