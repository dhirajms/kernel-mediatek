...
static struct cpuidle_driver mt8173_cpuidle_driver = {
	.name             = "mt8173_cpuidle",
	.owner            = THIS_MODULE,
	.states[0] = {
...
static const struct of_device_id mt8173_idle_state_match[] __initconst = {
...
int __init mt8173_cpuidle_init(void)
{
	 ...
	 */
	ret = dt_init_idle_driver(drv, mt8173_idle_state_match, 1);
	if (ret <= 0) {
		if (ret)
...
device_initcall(mt8173_cpuidle_init);
