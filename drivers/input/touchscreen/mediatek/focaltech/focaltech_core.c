...
#if FTS_GESTRUE_EN
extern int _is_open_gesture_mode;
#endif
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	//printk("[proxi_5206]command = 0x%02X\n", command);		
	switch (command)
	{
		case SENSOR_DELAY:
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				//printk("huang sensor_data->values[0] 1082 = %d\n", sensor_data->values[0]);
...
/* Vanzo:yangzhihong on: Sat, 16 Jan 2016 16:05:06 +0800
			if (tpd_dts_data.use_tpd_button) {
...
// End of Vanzo:yangzhihong
            if (tpd_dts_data.use_tpd_button && tpd_dts_data.tpd_switch_vkey) {
                if (cinfo.y[0] > TPD_RES_Y)
                    cinfo.x[0] = TPD_RES_X - cinfo.x[0];
            }
...
/*#ifdef CONFIG_FT_AUTO_UPGRADE_SUPPORT
    ...
    {
		TPD_DMESG("proxi_fts attach fail = %d\n", err);
    }
    else
    {
		TPD_DMESG("proxi_fts attach ok = %d\n", err);
    }		
    */
    //touchpanel_get_ps_value = &fts_get_ps_value;
...

	{
        ...
        fwver = ver;
        sprintf(fwvers, "%02X",fwver);
        strcpy(ft_name,"focaltech_V");
        strcat(ft_name,fwvers);
...
#ifdef VANZO_SHOW_TP_FW_VERSION
    v_set_dev_name(2, ft_name);
#else
