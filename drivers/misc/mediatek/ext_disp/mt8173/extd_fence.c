...
/************************* variable *********************/
static unsigned int isAEEEnabled;
/* static int fence_clean_up_task_wakeup; */
...
static void extd_ion_init(void)
{
...
#if 0
static void extd_ion_deinit(void)
{
	if (ion_client) {
...
static struct ion_handle *extd_ion_import_handle(struct ion_client *client, int fd)
{
	...
	mm_data.config_buffer_param.handle = (ion_user_handle_t) (unsigned long)handle;
...
static size_t extd_ion_phys_mmu_addr(struct ion_client *client, struct ion_handle *handle,
				     unsigned int *mva)
{
	size_t size;
...
static void extd_ion_cache_flush(struct ion_client *client, struct ion_handle *handle)
{
	...
	sys_data.cache_sync_param.handle = (ion_user_handle_t) (unsigned long)handle;
...
void extd_signal_fence(unsigned int layer, int fence)
{
	...
	if (timeline != NULL) {	/* make sure fence has been initialized */
		...
		if (num_fence > 0) {
			/* MMProfileLogEx(EXTD_MMP_Events.SignalSyncFence,*/
			/* MMProfileFlagStart, layer, info->timeline_idx); */
			timeline_inc(timeline, num_fence);
			/* MMProfileLogEx(EXTD_MMP_Events.SignalSyncFence, */
			/*MMProfileFlagEnd, layer, timeline->value); */
			mutex_lock(&info->mutex_lock);
		...
		EXTD_FENCE_LOG("layer%d  fence =%d timeidx %d timevalue %d\n", layer, fence,
			       info->timeline_idx, timeline->value);
	}
#endif
}
...
void extd_release_fence(unsigned int layer, int fence)
{
	...
	if (timeline != NULL) {
		...
		EXTD_FENCE_LOG("layer%d f %d tl %d inc %d max %d\n", layer, fence,
			       info->timeline_idx, num_fence, info->fence_idx);
	}
#endif
}
...
struct extd_fence_buf_info *extd_prepare_buf(struct fb_overlay_buffer_t *buf)
{
	...
	/*(buf->layer_en<<16)|(buf->ion_fd==MTK_FB_NO_ION_FD?0:buf->ion_fd)); */
	info = &fence_info[layer];
	timeline = sync_timelines[layer];
	...
	EXTD_FENCE_ERR("prepare 0x%08x, fence %d, idx %d\n",
		       (buf->layer_id << 24) | (buf->layer_en << 16) | (buf->ion_fd ==
									MTK_FB_NO_ION_FD ? 0 :
									buf->ion_fd),
		       buf_info->fence, buf_info->idx);
	return buf_info;
}
