...
int flag_resolution_4k(HDMI_VIDEO_RESOLUTION resolution)
{
	if ((resolution == HDMI_VIDEO_2160P_23_976HZ) ||
	    ...
	    (resolution == HDMI_VIDEO_2160P_30HZ) || (resolution == HDMI_VIDEO_2161P_24HZ))
		return true;
	else
		return false;
}
	...
	case MTK_HDMI_IS_FORCE_AWAKE:
		{
			if (!hdmi_drv_init_context()) {
				HDMI_LOG("[hdmi]%s, hdmi_drv_init_context fail\n", __func__);
	...
	case MTK_HDMI_FACTORY_MODE_ENABLE:
		{
			if (arg == 1) {
				if (hdmi_drv->power_on()) {
					r = -EAGAIN;
					HDMI_LOG("Enable HDMI factory mode test fail\n");
				}
			} else {
				HDMI_LOG("Disable HDMI factory mode test fail\n");
				hdmi_drv->power_off();
			}
			break;
		}
...
static int hdmi_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		HDMI_LOG("Fail to probe internal hdmi\n");
		return HDMI_STATUS_NOT_IMPLEMENTED;
	}
#endif
#if defined(CONFIG_MTK_INTERNAL_MHL_SUPPORT)
