...
static struct acc_hw *hw_info = &accel_cust;
...
/*----------------------------------------------------------------------------*/
static int mir3da_accel_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
    strlcpy(info->type, MIR3DA_DRV_NAME, sizeof(info->type));
...
//    .suspend     = mir3da_suspend,
 //   .resume      = mir3da_resume,
      .detect = mir3da_accel_i2c_detect,
    .id_table    = mir3da_id,
};
...
{    
  ...
  const char *name = "mediatek,mir3da";

  MI_FUN;

  node = of_find_compatible_node(NULL,NULL,name);
  ret = get_accel_dts_func(node, hw_info);
  if (ret < 0)
