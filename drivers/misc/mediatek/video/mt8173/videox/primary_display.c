...
#ifdef CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT
static KREE_SESSION_HANDLE extd_disp_secure_memory_session_handle(void)
{
...
#if defined(MTK_FB_ION_SUPPORT)
	{
		...
		for (i = 0; i < DISP_INTERNAL_BUFFER_COUNT; i++) {
			...
			if (buf) {
				...
				DISPMSG("buf:0x%lx, buf->list:0x%lx, buf->mva:0x%x",
					(unsigned long)buf, (unsigned long)&buf->list, buf->mva);
				DISPMSG("buf->size:0x%08x,buf->handle:0x%lx,dc_vaddr = 0x%lx\n",
					buf->size, (unsigned long)buf->handle, dc_vAddr[i]);
...
static void _cmdq_build_trigger_loop(void)
{
	...
	if (primary_display_is_video_mode()) {
		...
		ret = cmdqRecWaitNoClear(pgc->cmdq_handle_trigger, CMDQ_EVENT_DISP_RDMA0_SOF);
...
int _trigger_display_interface(int blocking, void *callback, unsigned int userdata)
{
	...
	/* enable SODI by CPU after flush CMDQ by CPU */
	if (primary_display_is_video_mode() == 1) {
	#ifdef DISP_ENABLE_SODI
		spm_enable_sodi(1);
	#endif
	...
	if (_should_insert_wait_frame_done_token() && (!_is_decouple_mode(pgc->session_mode))) {
		...
		/* 3. disable SODI by CMDQ before config */
		if (primary_display_is_video_mode() == 1)
			disp_set_sodi(0, pgc->cmdq_handle_config);
	}
...
/*wrokround for kernel 3.18*/
	...
	if (gEnableSODIControl == 1) {
		...
		if (cmdq_handle != NULL) {
			if (enable == 1 && dram_self_refresh == 0)
...
static int _ovl_fence_release_callback(uint32_t userdata)
{
	...
	if (primary_display_is_video_mode() == 1
		...
		if ((ovl_status[0] & 0x1) != 0) {
			/* ovl is not idle !! */
			DISPERR
			    ("disp ovl status(0x%x) error!config maybe not finish during blanking\n",
			     ovl_status[0]);
			ret = -1;
		}
	}
...
int primary_display_switch_mode(int sess_mode, unsigned int session, int force)
{
	...
	DISPDBG("primary_display_switch_mode switch mode from %d(%s) to %d(%s)\n",
