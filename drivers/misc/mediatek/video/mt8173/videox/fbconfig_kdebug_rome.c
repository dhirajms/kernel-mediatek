...
static void print_from_head_to_tail(void)
{
	...
	DISPMSG("DDIC=====>:print_from_head_to_tail  START\n");

	list_for_each_prev(p, &head_list.list) {
		print = list_entry(p, CONFIG_RECORD_LIST, list);
		DISPMSG("type:%d num %d value:\r\n", print->record.type, print->record.ins_num);
		for (i = 0; i < print->record.ins_num; i++)
			DISPMSG("0x%x\t", print->record.ins_array[i]);
		DDPMSG("\r\n");
	}
	DDPMSG("DDIC=====>:print_from_head_to_tail  END\n");
...
static void free_list_memory(void)
{
	...
	if (list_empty(&head_list.list))
		DISPMSG("*****list is empty!!\n");
	else
		DISPMSG("*****list is NOT empty!!\n");
	...
	case LCM_TEST_DSI_CLK:
		{
			...
			DISPMSG("fbconfig=>LCM_TEST_DSI_CLK:%d\n", ret);
	...
	case DRIVER_IC_CONFIG:
		{
			...
			if (copy_from_user(&record_tmp_list->record, (void __user *)arg, sizeof(CONFIG_RECORD))) {
				DISPMSG("list_add: copy_from_user failed! line:%d\n", __LINE__);
				return -EFAULT;
			}
	...
	case MIPI_SET_CC:
		{
			...
			if (get_user(enable, (UINT32 __user *) argp)) {
				DISPMSG("[MIPI_SET_CC]: copy_from_user failed! line:%d\n", __LINE__);
				return -EFAULT;
			}
	...
	case LCM_GET_DSI_CONTINU:
		{
			...
			DISPMSG("LCM_GET_DSI_CONTINU=>DSI: %d\n", ret);
	...
	case MIPI_SET_CLK:
		{
			...
			if (get_user(clk, (UINT32 __user *) argp)) {
				DISPMSG("[MIPI_SET_CLK]: copy_from_user failed! line:%d\n",
				       __LINE__);
				return -EFAULT;
			}
			DISPMSG("LCM_GET_DSI_CLK=>dsi:%d\n", clk);
	...
	case MIPI_SET_SSC:
		{
			...
			if (copy_from_user(&dsi_ssc, (void __user *)argp, sizeof(dsi_ssc))) {
				DISPMSG("[MIPI_SET_SSC]: copy_from_user failed! line:%d\n",
				       __LINE__);
				return -EFAULT;
			}
			DISPMSG("Pmaster:set mipi ssc line:%d\n", __LINE__);
	...
	case LCM_GET_DSI_LANE_NUM:
		{
			...
			DISPMSG("Panel Master=>LCM_GET_DSI_Lane_num=>dsi:%d\r\n", lane_num);
	...
	case LCM_GET_DSI_TE:
		{
			...
			DISPMSG("fbconfig=>LCM_GET_DSI_TE:%d\n", ret);
			return put_user(ret, (unsigned long *)argp);
		}
	case LCM_GET_DSI_TIMING:
		{
			...
			if (copy_from_user(&timing, (void __user *)argp, sizeof(timing))) {
				DISPMSG("[MIPI_GET_TIMING]: copy_from_user failed! line:%d\n",
				       __LINE__);
				return -EFAULT;
			}
			ret = PanelMaster_get_dsi_timing(dsi_id, timing.type);
			DISPMSG("fbconfig=>LCM_GET_DSI_TIMING:%d\n", ret);
			timing.value = ret;
	...
	case MIPI_SET_TIMING:
		{
			...
			if (is_early_suspended)
				return -EFAULT;
			if (copy_from_user(&timing, (void __user *)argp, sizeof(timing))) {
				DISPMSG("[MIPI_SET_TIMING]: copy_from_user failed! line:%d\n",
				       __LINE__);
				return -EFAULT;
			}
	...
	case FB_LAYER_GET_EN:
		{
			...
			DISPMSG("[LAYER_GET_EN]:L0:%d L1:%d L2:%d L3:%d\n", ovl_all[0].layer_en,
	...
	case FB_LAYER_GET_INFO:
		{
			...
			if (copy_from_user(&layer_info, (void __user *)argp, sizeof(layer_info))) {
				DISPMSG("[TE_SET_ENABLE]: copy_from_user failed! line:%d\n",
				       __LINE__);
				return -EFAULT;
			}
			...
			DISPMSG("===>: layer_size:0x%x height:%d\n", layer_info.layer_size,
			       layer_info.height);
	...
	case FB_LAYER_DUMP:
		{
			...
			if ((layer_size != 0) && (ovl_all[global_layer_id].layer_en != 0)) {
				/* DISPMSG("sxk==>FB_LAYER_DUMP==>layer_size is %d   mva is 0x%x\n",layer_size,mva); */
				...
				if (ret < 0) {
					DISPMSG("m4u_mva_map_kernel error: ret=%d 0x%x %d\r\n", ret,
					       real_mva, real_size);
					return ret;
				}
				if (layer_size > mapped_size) {
					DISPMSG("==>layer size(0x%x)>mapped size(0x%x)!!!",
					       layer_size, mapped_size);
					return -EFAULT;
				}
				DISPMSG("==> addr from user space is 0x%x\n", (unsigned int)argp);
				DISPMSG
				    ("==> kva is 0x%x real_mva %x mva %x mmaped size is %dlayer_size is %d\n",
				     kva, real_mva, mva, mapped_size, layer_size);
				ret =
				    copy_to_user(argp, (void *)kva + (real_mva - mva),
						 layer_size - (real_mva - mva)) ? -EFAULT : 0;
	...
	case LCM_GET_ESD:
		{
			...
			if (copy_from_user(&esd_para, (void __user *)arg, sizeof(esd_para))) {
				DISPMSG("[LCM_GET_ESD]: copy_from_user failed! line:%d\n", __LINE__);
				return -EFAULT;
			}
