...
static void process_dbg_opt(const char *opt)
{
	...
	if (0 == strncmp(opt, "dsipattern", 10)) {
		...
		STR_CONVERT(&p, &pattern, uint, return);
		...
		STR_CONVERT(&p, &option, uint, return);
		...
		STR_CONVERT(&p, &maxlayer, uint, return);
		if (maxlayer)
		...
		STR_CONVERT(&p, &enable, uint, return);
		...
		if (0 == strncmp(opt + 11, "on", 2)) {
			...
			STR_CONVERT(&p, &gCapturePriLayerDownX, uint, return);
			STR_CONVERT(&p, &gCapturePriLayerDownY, uint, return);
			STR_CONVERT(&p, &gCapturePriLayerNum, uint, return);
