...
int ovl2mem_init(unsigned int session)
{
	...
	if (ret) {
		...
		DISPCHECK("cmdqRecCreate SUCCESS, cmdq_handle=0x%p\n", pgc->cmdq_handle_config);
	}
...
int ovl2mem_input_config(ovl2mem_in_config *input)
{
	...
	for (i = 0; i < HW_OVERLAY_COUNT; i++) {
		...
		if (input[i].dirty) {
			ret =
			    _convert_disp_input_to_ovl(&(data_config->ovl_config[input[i].layer]),
	...
	DISPDBG("ovl2mem_input_config done\n");
	return ret;
}
...
int ovl2mem_trigger(int blocking, void *callback, unsigned int userdata)
{
	...
	if (pgc->need_trigger_path == 0) {
		...
		if ((atomic_read(&g_trigger_ticket) - atomic_read(&g_release_ticket)) == 1) {
			DISPMSG
			    ("ovl2mem_trigger(%x), configue input, but does not config output!!\n",
			     pgc->session);
			for (layid = 0; layid < (HW_OVERLAY_COUNT + 1); layid++) {
				fence_idx =
	...
	DISPDBG("ovl2mem_trigger done %d\n", get_ovl2mem_ticket());
