...
typedef struct {
	...
	uint32_t rsv_009C[25];	/* 0104..0100 */
...
#define DISP_WDMA_INDEX_OFFSET (0x1000)
#define DISP_RDMA_INDEX_OFFSET (0x1000)
#define DISP_OVL_INDEX_OFFSET  (0x1000)

#define DDP_REG_BASE_MMSYS_CONFIG  ddp_regbase_va[DISP_MODULE_CONFIG]
#define DDP_REG_BASE_DISP_OVL0     ddp_regbase_va[DISP_MODULE_OVL0]
#define DDP_REG_BASE_DISP_OVL1     ddp_regbase_va[DISP_MODULE_OVL1]
#define DDP_REG_BASE_DISP_RDMA0    ddp_regbase_va[DISP_MODULE_RDMA0]
#define DDP_REG_BASE_DISP_RDMA1    ddp_regbase_va[DISP_MODULE_RDMA1]
#define DDP_REG_BASE_DISP_RDMA2    ddp_regbase_va[DISP_MODULE_RDMA2]
#define DDP_REG_BASE_DISP_WDMA0    ddp_regbase_va[DISP_MODULE_WDMA0]
#define DDP_REG_BASE_DISP_WDMA1    ddp_regbase_va[DISP_MODULE_WDMA1]
#define DDP_REG_BASE_DISP_COLOR0   ddp_regbase_va[DISP_MODULE_COLOR0]
#define DDP_REG_BASE_DISP_COLOR1   ddp_regbase_va[DISP_MODULE_COLOR1]
#define DDP_REG_BASE_DISP_AAL      ddp_regbase_va[DISP_MODULE_AAL]
#define DDP_REG_BASE_DISP_GAMMA    ddp_regbase_va[DISP_MODULE_GAMMA]
#define DDP_REG_BASE_DISP_MERGE    ddp_regbase_va[DISP_MODULE_MERGE]
#define DDP_REG_BASE_DISP_SPLIT0   ddp_regbase_va[DISP_MODULE_SPLIT0]
#define DDP_REG_BASE_DISP_SPLIT1   ddp_regbase_va[DISP_MODULE_SPLIT1]
#define DDP_REG_BASE_DISP_UFOE     ddp_regbase_va[DISP_MODULE_UFOE]
#define DDP_REG_BASE_DSI0          ddp_regbase_va[DISP_MODULE_DSI0]
#define DDP_REG_BASE_DSI1          ddp_regbase_va[DISP_MODULE_DSI1]
#define DDP_REG_BASE_DPI0          ddp_regbase_va[DISP_MODULE_DPI0]
#define DDP_REG_BASE_DPI1          ddp_regbase_va[DISP_MODULE_DPI1]
#define DDP_REG_BASE_DISP_PWM0     ddp_regbase_va[DISP_MODULE_PWM0]
#define DDP_REG_BASE_DISP_PWM1     ddp_regbase_va[DISP_MODULE_PWM1]
#define DDP_REG_BASE_MM_MUTEX      ddp_regbase_va[DISP_MODULE_MUTEX]
#define DDP_REG_BASE_SMI_LARB0     ddp_regbase_va[DISP_MODULE_SMI_LARB0]
#define DDP_REG_BASE_SMI_COMMON    ddp_regbase_va[DISP_MODULE_SMI_COMMON]
...
#define DDP_REG_TVDPLL_CON1        ddp_regbase_va[DISP_MODULE_TVDPLL_CON1]
...
#define DISP_REG_GET(reg32) (*(volatile unsigned int *)((unsigned long)reg32))
#define DISP_REG_GET_FIELD(field, reg32) \
	((*(volatile unsigned int *)((unsigned long)reg32) & REG_FLD_MASK(field)) >> REG_FLD_SHIFT(field))
...
#define DISP_CPU_REG_SET_FIELD(field, reg32, val)  \
	do { \
		if (0) \
			dprec_reg_op(NULL, (unsigned int)(reg32), val<<REG_FLD_SHIFT(field), REG_FLD_MASK(field));\
			mt_reg_sync_writel((*(volatile unsigned int *)(reg32) \
			& ~REG_FLD_MASK(field))|REG_FLD_VAL((field), (val)), reg32);  \
	} while (0)

static inline unsigned int ddp_addr_convert_va2pa(unsigned long va)
{
	unsigned int i = 0;

	for (i = 0; i < DISP_MODULE_NUM; i++) {
		if (ddp_regbase_va[i] == (va & (~0xfffl)))
			return (ddp_regbase_pa[i] + (va & 0xfffl));
	}
	return 0;
}

static inline unsigned long ddp_addr_convert_pa2va(unsigned int pa)
{
	unsigned int i = 0;
...
{
	return ddp_addr_convert_va2pa(va);
}
...
#define DISP_REG_MASK(handle, reg32, val, mask) \
	do { \
		if (handle == NULL)\
			...
			cmdqRecWrite(handle, ddp_addr_convert_va2pa(reg32), val, mask); \
	} while (0)
...
#define DISP_REG_SET(handle, reg32, val) \
	do { \
		...
		if (handle == 0) { \
			mt_reg_sync_writel(val, (unsigned int *)(reg32));\
		} else { \
			dprec_reg_op(handle, reg32, val, 0x00000000);\
			cmdqRecWrite(handle, ddp_addr_convert_va2pa(reg32), val, ~0); \
		}  \
	} while (0)
...
#define DISP_REG_SET_FIELD(handle, field, reg32, val)  \
	do {  \
		...
		if (handle == 0) \
			...
			dprec_reg_op(handle, (unsigned long)(reg32),\
						val<<REG_FLD_SHIFT(field), REG_FLD_MASK(field));\
			cmdqRecWrite(handle, ddp_addr_convert_va2pa(reg32),\
						val<<REG_FLD_SHIFT(field), REG_FLD_MASK(field)); \
		} \
	} while (0)
...
#define DISP_REG_SET(handle, reg32, val) \
	do { \
		if (handle == 0) { \
			/*printk("%s %d DISP_REG_SET %p: 0x%x\n", __FUNCTION__, __LINE__, reg32, val);*/\
			mt_reg_sync_writel(val, (unsigned int *)(reg32));\
		} else { \
			/*printk("%s %d DISP_REG_SET(CMDQ) %p: 0x%x\n", __FUNCTION__, __LINE__, reg32, val);*/\
			cmdqRecWrite(handle, ddp_addr_convert_va2pa(reg32), val, ~0); \
		}  \
	} while (0)
...
#define DISP_REG_SET_FIELD(handle, field, reg32, val)  \
	do {  \
		if (handle == 0) { \
			/*printk("%s %d DISP_REG_SET_FIELD 0x%lx: 0x%x\n", __FUNCTION__, __LINE__, reg32, val);*/\
			mt_reg_sync_writel\
			((*(unsigned int *)(reg32) & ~REG_FLD_MASK(field))|REG_FLD_VAL((field), (val)), reg32);\
		} else { \
			/*printk("%s %d DISP_REG_SET_FIELD(CMDQ) 0x%lx: 0x%x\n", __FUNCTION__, __LINE__, reg32, val);*/\
			cmdqRecWrite(handle, ddp_addr_convert_va2pa(reg32),\
						val<<REG_FLD_SHIFT(field), REG_FLD_MASK(field)); \
		} \
	} while (0)
