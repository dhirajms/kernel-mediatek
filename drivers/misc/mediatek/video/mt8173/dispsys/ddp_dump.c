...
static void mutex_dump_reg(void)
{
	...
	DDPDUMP("MUTEX:0x000=0x%08x,0x004=0x%08x,0x020=0x%08x,0x028=0x%08x\n",
		...
		DISP_REG_GET(DISP_REG_CONFIG_MUTEX0_EN), DISP_REG_GET(DISP_REG_CONFIG_MUTEX0_RST));
	...
	DDPDUMP("MUTEX:0x02c=0x%08x,0x030=0x%08x,0x040=0x%08x,0x048=0x%08x\n",
		...
		DISP_REG_GET(DISP_REG_CONFIG_MUTEX1_EN), DISP_REG_GET(DISP_REG_CONFIG_MUTEX1_RST));
	...
	DDPDUMP("MUTEX:0x04c=0x%08x,0x050=0x%08x,0x060=0x%08x,0x068=0x%08x\n",
		...
		DISP_REG_GET(DISP_REG_CONFIG_MUTEX2_EN), DISP_REG_GET(DISP_REG_CONFIG_MUTEX2_RST));
	...
	DDPDUMP("MUTEX:0x06c=0x%08x,0x070=0x%08x,0x080=0x%08x,0x088=0x%08x\n",
		...
		DISP_REG_GET(DISP_REG_CONFIG_MUTEX3_EN), DISP_REG_GET(DISP_REG_CONFIG_MUTEX3_RST));
	...
	DDPDUMP("MUTEX:0x08c=0x%08x,0x090=0x%08x,0x0a0=0x%08x,0x0a8=0x%08x\n",
		...
		DISP_REG_GET(DISP_REG_CONFIG_MUTEX4_EN), DISP_REG_GET(DISP_REG_CONFIG_MUTEX4_RST));
	...
	DDPDUMP("MUTEX:0x0ac=0x%08x,0x0b0=0x%08x,0x0c0=0x%08x,0x0c8=0x%08x\n",
		...
		DISP_REG_GET(DISP_REG_CONFIG_MUTEX5_EN), DISP_REG_GET(DISP_REG_CONFIG_MUTEX5_RST));
	...
	for (i = 0; i < 5; i++) {
		...
		if (DISP_REG_GET(DISP_REG_CONFIG_MUTEX_MOD(i)) != 0 &&
			...
			len = sprintf(p, "MUTEX%d :mode=%s,module=(",
				      i,
				      ddp_mutex_sof_to_string(DISP_REG_GET
							      (DISP_REG_CONFIG_MUTEX_SOF(i))));
			p += len;
			for (j = 11; j <= 25; j++) {
...
static void mmsys_config_dump_analysis(void)
{
	...
	for (i = 0; i < 2; i++) {
		if ((reg & (1 << i)) == 0)
			len +=
			    scnprintf(clock_on + len, sizeof(clock_on) - len, "%s ",
				      ddp_clock_0(i));
	}
	for (i = 14; i < 32; i++) {
		if ((reg & (1 << i)) == 0)
...
static void gamma_dump_reg(void)
{
	...
	DDPDUMP("GAMMA:0x000=0x%08x,0x004=0x%08x,0x008=0x%08x,0x00c=0x%08x\n",
		...
		DISP_REG_GET(DISP_REG_GAMMA_INTEN), DISP_REG_GET(DISP_REG_GAMMA_INTSTA));
	...
	DDPDUMP("GAMMA:0x02c=0x%08x,0x030=0x%08x,0x0c0=0x%08x,0x800=0x%08x\n",
		...
		DISP_REG_GET(DISP_REG_GAMMA_DUMMY_REG), DISP_REG_GET(DISP_REG_GAMMA_LUT));
}
...
static void merge_dump_reg(void)
{
	...
	DDPDUMP("MERGE:0x000=0x%08x,0x004=0x%08x,0x008=0x%08x\n",
		...
		DISP_REG_GET(DISP_REG_MERGE_SW_RESET), DISP_REG_GET(DISP_REG_MERGE_DEBUG));
}
...
static void color_dump_reg(DISP_MODULE_ENUM module)
{
	...
	DDPDUMP("COLOR:0x400=0x%08x,0x404=0x%08x,0x408=0x%08x,0xc00=0x%08x\n",
		...
		DISP_REG_GET(DISP_COLOR_PXL_MAIN + DISP_INDEX_OFFSET * index),
		DISP_REG_GET(DISP_COLOR_LNE_MAIN + DISP_INDEX_OFFSET * index),
...
static void aal_dump_reg(void)
{
	...
	DDPDUMP("AAL:0x210=0x%08x,0x2a0=0x%08x,0x2a4=0x%08x,0x354=0x%08x\n",
		...
		DISP_REG_GET(DISP_AAL_STATUS_32), DISP_REG_GET(DISP_AAL_DRE_GAIN_FILTER_00));
...
static void aal_dump_analysis(void)
{
	...
	DDPDUMP("aal: en=%d, relay=%d, w=%d, h=%d\n",
		...
		(DISP_REG_GET(DISP_AAL_SIZE) >> 16) & 0x1fff, DISP_REG_GET(DISP_AAL_SIZE) & 0x1fff);
}
...
static void pwm_dump_reg(DISP_MODULE_ENUM module)
{
	...
	DDPDUMP("PWM:0x000=0x%08x,0x010=0x%08x,0x014=0x%08x,0x028=0x%08x\n",
		...
		DISP_REG_GET(reg_base + DISP_PWM_CON_1_OFF), DISP_REG_GET(reg_base + 0x28));
}
...
static void od_dump_reg(void)
{
	...
	DDPDUMP("OD:0x000=0x%08x,0x004=0x%08x,0x008=0x%08x,0x00c=0x%08x\n",
		...
		DISP_REG_GET(DISP_REG_OD_INTEN), DISP_REG_GET(DISP_REG_OD_INTSTA));
	...
	DDPDUMP("OD:0x010=0x%08x,0x020=0x%08x,0x024=0x%08x,0x028=0x%08x\n",
		...
		DISP_REG_GET(DISP_REG_OD_INPUT_COUNT), DISP_REG_GET(DISP_REG_OD_OUTPUT_COUNT));
	...
	DDPDUMP("OD:0x02c=0x%08x,0x030=0x%08x,0x040=0x%08x,0x044=0x%08x\n",
		...
		DISP_REG_GET(DISP_REG_OD_HSYNC_WIDTH), DISP_REG_GET(DISP_REG_OD_VSYNC_WIDTH));

	DDPDUMP("OD:0x048=0x%08x,0x0C0=0x%08x\n",
		DISP_REG_GET(DISP_REG_OD_MISC), DISP_REG_GET(DISP_REG_OD_DUMMY_REG));
}
...
static void ufoe_dump_analysis(void)
{
	...
	DDPDUMP("ufoe: bypass=%d,0x%08x\n",
		DISP_REG_GET(DISP_REG_UFO_START) == 0x4, DISP_REG_GET(DISP_REG_UFO_START));
}
	...
	if (DISP_MODULE_DSI0 == module) {
		...
		for (i = 0; i < 20 * 16; i += 16) {
			DDPDUMP("DSI0+%04x : 0x%08x  0x%08x  0x%08x  0x%08x\n", i,
				INREG32(ddp_regbase_va[DISP_MODULE_DSI0] + i),
				INREG32(ddp_regbase_va[DISP_MODULE_DSI0] + i + 0x4),
				INREG32(ddp_regbase_va[DISP_MODULE_DSI0] + i + 0x8),
				INREG32(ddp_regbase_va[DISP_MODULE_DSI0] + i + 0xc));
		}
		for (i = 0; i < 20 * 16; i += 16) {
			DDPDUMP("DSI0_PHY+%04x : 0x%08x  0x%08x  0x%08x  0x%08x\n", i,
				INREG32(ddp_regbase_va[DISP_MODULE_MIPITX0] + i),
				INREG32(ddp_regbase_va[DISP_MODULE_MIPITX0] + i + 0x4),
				INREG32(ddp_regbase_va[DISP_MODULE_MIPITX0] + i + 0x8),
				INREG32(ddp_regbase_va[DISP_MODULE_MIPITX0] + i + 0xc));
		}
	} else if (DISP_MODULE_DSI1 == module) {
		DDPDUMP("==DISP DSI1 REGS==\n");
		for (i = 0; i < 20 * 16; i += 16) {
			DDPDUMP("DSI1+%04x : 0x%08x  0x%08x  0x%08x  0x%08x\n", i,
				INREG32(ddp_regbase_va[DISP_MODULE_DSI1] + i),
				INREG32(ddp_regbase_va[DISP_MODULE_DSI1] + i + 0x4),
				INREG32(ddp_regbase_va[DISP_MODULE_DSI1] + i + 0x8),
				INREG32(ddp_regbase_va[DISP_MODULE_DSI1] + i + 0xc));
		}
		for (i = 0; i < 20 * 16; i += 16) {
			DDPDUMP("DSI1_PHY+%04x : 0x%08x  0x%08x  0x%08x  0x%08x\n", i,
				INREG32(ddp_regbase_va[DISP_MODULE_MIPITX1] + i),
				INREG32(ddp_regbase_va[DISP_MODULE_MIPITX1] + i + 0x4),
				INREG32(ddp_regbase_va[DISP_MODULE_MIPITX1] + i + 0x8),
				INREG32(ddp_regbase_va[DISP_MODULE_MIPITX1] + i + 0xc));
		}
	} else {
