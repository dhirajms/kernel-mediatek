...
static int split_clock_on(DISP_MODULE_ENUM module, void *handle)
{
	...
	ddp_module_clock_enable(MM_CLK_DISP_SPLIT0 + idx, true);
...
static int split_clock_off(DISP_MODULE_ENUM module, void *handle)
{
	...
	DDPMSG("Split%dClockoff CG 0x%x\n", idx, DISP_REG_GET(DISP_REG_CONFIG_MMSYS_CG_CON0));
	ddp_module_clock_enable(MM_CLK_DISP_SPLIT0 + idx, false);
	return 0;
}
...
static int split_busy(DISP_MODULE_ENUM module)
{
	int idx = split_index(module);
	unsigned int state =
	    DISP_REG_GET_FIELD(DEBUG_FLD_SPLIT_FSM, DISP_REG_SPLIT_DEBUG + DISP_INDEX_OFFSET * idx);
	return (state & 0x4);
}
...
int split_reset(DISP_MODULE_ENUM module, void *handle)
{
	...
	while ((DISP_REG_GET_FIELD
		(DEBUG_FLD_SPLIT_FSM, DISP_REG_SPLIT_DEBUG + DISP_INDEX_OFFSET * idx)
		& 0x3) == 0) {
		delay_cnt++;
		udelay(10);
		if (delay_cnt > 2000) {
...
static int split_dump_analysis(DISP_MODULE_ENUM module)
{
	...
	pixel =
	    DISP_REG_GET_FIELD(DEBUG_FLD_IN_PIXEL_CNT,
			       DISP_REG_SPLIT_DEBUG + DISP_INDEX_OFFSET * idx);
	state =
	    DISP_REG_GET_FIELD(DEBUG_FLD_SPLIT_FSM, DISP_REG_SPLIT_DEBUG + DISP_INDEX_OFFSET * idx);
