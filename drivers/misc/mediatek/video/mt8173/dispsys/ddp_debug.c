...
static void dbg_opt_ext(const char *opt, char *buf)
{
	...
	if (enable == 1) {
		...
		for (i = 0; i < DISP_MODULE_NUM; i++) {
			...
			sprintf(buf_temp,
				"i=%d, module=%s, va=0x%lx, pa=0x%x, irq(%d map to %d)\n",
				i, dpmgr_module_name_spy(i), ddp_regbase_va[i],
				ddp_regbase_pa[i], ddp_irq[i], ddp_irq_map[i]);
...
static void process_dbg_opt(const char *opt)
{
	...
	if (0 == strncmp(opt, "regr:", 5)) {
		...
		STR_CONVERT(&p, &addr, ul, goto Error);
		...
		STR_CONVERT(&p, &gRDMAUltraSetting, uint, goto Error);
		...
		STR_CONVERT(&p, &gRDMAFIFOLen, uint, goto Error);
		...
		char *p = (char *)opt + 7;
		unsigned int met_on;
		unsigned int rdma0_mode;
		unsigned int rdma1_mode;

		STR_CONVERT(&p, &met_on, uint, goto Error);
		STR_CONVERT(&p, &rdma0_mode, uint, goto Error);
		STR_CONVERT(&p, &rdma1_mode, uint, goto Error);
		...
		STR_CONVERT(&p, &aal_dbg_en, int, goto Error);
		...
		STR_CONVERT(&p, &mutex_idx, uint, goto Error);
		...
		STR_CONVERT(&p, &session, uint, goto Error);
