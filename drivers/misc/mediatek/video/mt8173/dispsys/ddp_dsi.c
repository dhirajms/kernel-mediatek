...
void DSI_PHY_TIMCONFIG(DISP_MODULE_ENUM module, cmdqRecHandle cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	if (0 != dsi_params->PLL_CLOCK) {
		...
		default:
			DDPMSG("pre_div should be less than 4!!\n");
			pre_div = 4;
			break;
		}
		...
		default:
			DDPMSG("post_div should be less than 4!!\n");
			post_div = 4;
			break;
		}
...
/* _WaitForEngineNotBusy(); */
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (0 != DSI_REG[i]->DSI_MODE_CTRL.MODE) {
			...
			DSI_WaitForNotBusy(i ==
					   0 ? DISP_MODULE_DSI0 : DISP_MODULE_DSI1 /*module */ ,
					   cmdq);
#ifdef ENABLE_DSI_ERROR_REPORT
			if ((pdata[0] & 1)) {
			...
			if (force_update) {
				DSI_Start(i == 0 ? DISP_MODULE_DSI0 : DISP_MODULE_DSI1 /*module */ ,
					  cmdq);
				udelay(20);
				DSI_WaitForNotBusy(i == 0 ? DISP_MODULE_DSI0 : DISP_MODULE_DSI1
						   /*module */ , cmdq);
			}
		}
	}
