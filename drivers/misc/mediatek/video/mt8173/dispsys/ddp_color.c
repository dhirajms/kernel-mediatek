			    ...
			    void *__cmdq)
{
	...
	DISP_REG_SET(cmdq, DISP_COLOR_G_PIC_ADJ_MAIN_1,
	...
	case DISP_IOCTL_READ_REG:
		{
			...
			addr_va = ddp_addr_convert_pa2va(rParams.reg);
	...
	case DISP_IOCTL_WRITE_REG:
		{
			...
			addr_va = ddp_addr_convert_pa2va(wParams.reg);

			if (addr_va == 0) {
				COLOR_ERR
				    ("reg write, addr invalid, addr min=0x%d, max=0x%lx, addr=0x%d\n",
				     DISPSYS_REG_ADDR_MIN, DISPSYS_REG_ADDR_MAX, wParams.reg);
				return -EFAULT;
			}
			/* unsigned int regVal; */
			/* regVal = DISP_REG_GET(wParams.reg); */
			/* regVal = (regVal & ~wParams.mask) | (wParams.val & wParams.mask); */
			/* DISP_CPU_REG_SET(wParams.reg, regVal); */
			DISP_REG_MASK(cmdq, addr_va, wParams.val, wParams.mask);

			COLOR_DBG("write  0x%x = 0x%x (0x%x)\n", wParams.reg, wParams.val,
				  wParams.mask);
