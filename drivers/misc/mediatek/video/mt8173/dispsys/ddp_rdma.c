...
#ifdef CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT
		DISP_BUFFER_TYPE sec, unsigned int split_buf_offset,
#endif
		void *handle)
{				/* ourput setting */
	...
	if (sec != DISP_SECURE_BUFFER) {
		...
		cmdqRecWriteSecure(handle,
				   ...
				   CMDQ_SAM_H_2_MVA, address, split_buf_offset, size, m4u_port);
#else
		/* test normal buffer */
...
static int do_rdma_config_l(DISP_MODULE_ENUM module, disp_ddp_path_config *pConfig, void *handle)
{
	...
	rdma_config(module, mode, (mode == RDMA_MODE_DIRECT_LINK) ? 0 : r_config->address,	/* address */
		    ...
		    width, height, r_config->security, r_config->split_buf_offset, handle);
