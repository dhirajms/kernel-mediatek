...
#include <mach/cust_leds.h>
#include <mach/cust_leds_def.h>
#include <mach/xlog.h>
#endif
...
#define BLS_DBG(string, args...) do { if (bls_dbg_log) pr_err("[BLS]"string, ##args); } while (0)
/* default on, important msg, not err */
#define BLS_MSG(string, args...) pr_err("[BLS]"string, ##args)
/* default on, err msg */
#define BLS_ERR(string, args...) pr_err("[BLS]error:"string, ##args)
	...
	{
	 {
	  ...
	  548, 552, 556, 560, 564, 568, 572, 576, 580, 584, 588, 592, 596,
	  600, 604, 608, 612, 616, 620, 624, 628, 632, 636, 640, 644,
	  648, 652, 656, 660, 664, 668, 672, 676, 680, 684, 688, 692, 696,
	  ...
	  948, 952, 956, 960, 964, 968, 972, 976, 980, 984, 988, 992, 996,
...
void disp_bls_update_gamma_lut(void)
{
	...
	for (i = 0; i < 256; i++) {
		CurVal = (((g_gamma_lut.entry[0][i] & 0x3FF) << 20) |
			  ((g_gamma_lut.entry[1][i] & 0x3FF) << 10) | (g_gamma_lut.
								       entry[2][i] & 0x3FF));
	 ...
	 */
	CurVal = (((g_gamma_lut.entry[0][256] & 0x3FF) << 20) |
		  ((g_gamma_lut.entry[1][256] & 0x3FF) << 10) | (g_gamma_lut.
								 entry[2][256] & 0x3FF));
...
void disp_bls_update_pwm_lut(void)
{
	...
	for (i = 0; i < PWM_LUT_ENTRY; i++) {
		...
		BLS_DBG("[%d] PWM LUT = 0x%x (%lu)\n", i, DISP_REG_GET(DISP_REG_BLS_LUMINANCE(i)),
			g_pwm_lut.entry[i]);
	}
...
{
#if !defined(CONFIG_MTK_AAL_SUPPORT) && !defined(MTK_BRINGUP_FOR_MT2701)
	struct cust_mt65xx_led *cust_led_list = get_cust_led_list();
	struct cust_mt65xx_led *cust = NULL;
	struct PWM_config *config_data = NULL;
	...
	DISP_REG_SET(DISP_REG_CONFIG_MUTEX_SOF(gBLSMutexID), 0);	/* single modei */
