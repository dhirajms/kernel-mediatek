			...
			_WaitForEngineNotBusy();
			{
				...
				for (i = 0; i < AS_UINT32(&DSI_REG->DSI_CMDQ_SIZE); i++)
					DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI",
						       ...
						       index, i * 4,
						       DISP_REG_GET(DDP_REG_BASE_DSI + 0x180 + i * 4));
...
DSI_STATUS DSI_DumpRegisters(void)
{
	...
	for (i = 0; i < sizeof(DSI_REGS); i += 16) {
		DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI",
			       ...
			       DISP_REG_GET(DDP_REG_BASE_DSI + i), DISP_REG_GET(DDP_REG_BASE_DSI + i + 0x4),
...
DSI_STATUS fbconfig_DSI_set_CLK(unsigned int clk)
{
	...
	pr_info("sxk==>fbconfig_DSI_set_CLK:will wait!!\n");

	_WaitForEngineNotBusy();	/* cmd mode */
	pr_info("sxk==>will fbconfig_DSI_set_CLK:%d\n", clk);
