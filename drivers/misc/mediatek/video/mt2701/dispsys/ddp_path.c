...
static unsigned int decouple_addr;
...
int disp_path_switch_ovl_mode(struct disp_path_config_ovl_mode_t *pConfig)
{
	...
	if (DISP_IsDecoupleMode()) {
		...
		WDMAConfig(0,
			   ...
			   pConfig->roi.width,
			   pConfig->roi.height,
			   pConfig->format, pConfig->address, pConfig->roi.width, 1, 0);
		WDMAStart(0);

		/* config rdma */
		RDMAConfig(0, RDMA_MODE_MEMORY,	/* memory mode */
			   pConfig->format,	/* inputFormat */
			   pConfig->address,	/* address, will be reset later */
			   RDMA_OUTPUT_FORMAT_ARGB,	/* output format */
			   pConfig->pitch,	/* pitch */
			   pConfig->roi.width, pConfig->roi.height, 0,	/* uint8_t swap */
			   0);	/* is RGB swap */
		RDMAStart(0);
		/* disp_dump_reg(DISP_MODULE_WDMA); */
	} else {
...
/* unsigned int reg_addr; */
	...
	DISP_DDP("[DDP] config_layer(), layer=%d, en=%d, source=%d, fmt=%d, addr=0x%x\n", pOvlConfig->layer,
		 pOvlConfig->layer_en, pOvlConfig->source,
		 pOvlConfig->fmt, pOvlConfig->addr);
	DISP_DDP("    src(%d, %d), pitch=%d, dst(%d, %d, %d, %d), keyEn=%d, key=%d\n", pOvlConfig->src_x,
		 pOvlConfig->src_y,
...
int disp_path_config_OVL_WDMA(struct disp_path_config_mem_out_struct *pConfig, int OvlSecure)
{
	...
	if (pConfig->enable == 1) {
...
#endif
		{
			...
			if (pConfig->outFormat == eYUV_420_3P) {
				unsigned int picSz, uAddr, vAddr, height_align;

				height_align = ((pConfig->srcROI.height + (16 - 1)) & (~(16 - 1)));
				picSz = pConfig->srcROI.width * height_align;
				{
					uAddr = pConfig->dstAddr + picSz;
					vAddr = uAddr + (picSz >> 2);
					pr_err
					    ("[normal] WFD dstAddr:0x%08x picSz:0x%08x uAddr:0x%08x vAddr:0x%08x",
...
int disp_path_config_(struct disp_path_config_struct *pConfig, int mutexId)
{
	...
	if (pConfig->srcModule == DISP_MODULE_OVL) {
		...
		if (pConfig->dstModule == DISP_MODULE_WDMA) {
			...
			if (DISP_IsDecoupleMode()) {
				pr_notice("mt8127 from de-couple\n");
				WDMAReset(0);
				if (decouple_addr == 0 ||
				    pConfig->srcROI.width == 0 || pConfig->srcROI.height == 0) {
					DISP_ERR
					    ("wdma parameter invalidate, addr=0x%x, w=%d, h=%d\n",
					     decouple_addr, pConfig->srcROI.width,
					     pConfig->srcROI.height);
					return -1;
				}
				WDMAConfig(0,
					   ...
					   pConfig->srcROI.width,
					   pConfig->srcROI.height,
					   eRGB888, decouple_addr, pConfig->srcROI.width, 1, 0);
				WDMAStart(0);
				/* Register WDMA intr */
				mem_out_done = 0;
				disp_register_irq(DISP_MODULE_WDMA, _disp_path_wdma_callback);
...
#ifdef CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT
				RDMAConfig_(0, RDMA_MODE_MEMORY,	/* mem mode */
					    eRGB888,	/* inputFormat */
...
#else
				RDMAConfig(0, RDMA_MODE_MEMORY,	/* mem mode */
					   eRGB888,	/* inputFormat */
					   decouple_addr,	/* display lk logo when entering kernel */
					   RDMA_OUTPUT_FORMAT_ARGB,	/* output format */
					   pConfig->srcROI.width * 3,	/* pitch, eRGB888 */
