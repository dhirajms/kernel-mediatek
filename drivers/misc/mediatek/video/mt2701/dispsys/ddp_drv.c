...
#define DISP_REGISTER_IRQ(irq_num) {\
	if (request_irq(irq_num , (irq_handler_t)disp_irq_handler,\
				IRQF_TRIGGER_LOW, DISP_DEVNAME , NULL)){ \
...
void disp_invoke_irq_callbacks(DISP_MODULE_ENUM module, unsigned int param)
{
	...
	for (i = 0; i < DISP_MAX_IRQ_CALLBACK; i++) {
		if (g_disp_irq_table[module][i]) {
			DISP_DBG("Invoke callback function. module=%d param=0x%X\n", module, param);
...
static const char *disp_reg_name_spy(DISP_DTS_REG_ENUM regs)
{
	...
	case DISP_REG_MDP_RDMA:
		return "mediatek,DISP_MDP_RDMA";
	case DISP_REG_MDP_RSZ0:
		return "mediatek,DISP_MDP_RSZ0";
	case DISP_REG_MDP_RSZ1:
		return "mediatek,DISP_MDP_RSZ1";
	case DISP_REG_MDP_WDMA:
		return "mediatek,DISP_MDP_WDMA";
	case DISP_REG_MDP_WROT:
		return "mediatek,DISP_MDP_WROT";
	case DISP_REG_MDP_TDSHP:
		return "mediatek,DISP_MDP_TDSHP";
	case DISP_REG_OVL:
		return "mediatek,DISP_OVL";
	case DISP_REG_RDMA0:
		return "mediatek,DISP_RDMA0";
	case DISP_REG_WDMA:
		return "mediatek,DISP_WDMA";
	case DISP_REG_BLS:
		return "mediatek,DISP_BLS";
	case DISP_REG_COLOR:
	...
	case DISP_REG_RDMA1:
		return "mediatek,DISP_RDMA1";
	case DISP_REG_UFOE:
		return "mediatek,DISP_UFOE";
	case DISP_REG_DPI1:
		return "mediatek,DISP_DPI1";
	case DISP_REG_HDMI:
		return "mediatek,DISP_HDMI";
	case DISP_REG_LVDS:
		return "mediatek,DISP_LVDS";
	case DISP_REG_TVE:
		return "mediatek,DISP_TVE";
	case DISP_REG_MIPI:
		return "mediatek,DISP_MIPI";
	default:
	...
	for (i = 0; i < DISP_REG_NUM; i++) {
		if (disp_is_intr_enable(i) && disp_dev.irq[i]) {
			ret = request_irq(disp_dev.irq[i], (irq_handler_t) disp_irq_handler,
					  IRQF_TRIGGER_LOW, disp_reg_name_spy(i), NULL);
			if (ret)
				DISP_ERR
...
static int disp_probe(struct platform_device *pdev)
{
	...
	if (i == DISP_REG_NUM) {
		DISP_ERR("dispsys compatible match failed\n");
		return -ENODEV;
	}
	id = i;
