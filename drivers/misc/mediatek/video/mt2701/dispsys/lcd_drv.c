...
static size_t dbi_log_on;
#define DBI_LOG(fmt, arg...) \
	do { \
		if (dbi_log_on)\
			DISP_LOG_PRINT(ANDROID_LOG_WARN, "LCD", fmt, ##arg);    \
	} while (0)

#define DBI_FUNC()	\
	do { \
		if (dbi_log_on)\
			DISP_LOG_PRINT(ANDROID_LOG_INFO, "LCD", "[Func]%s\n", __func__);  \
	} while (0)
...
static const uint32_t TO_BPP[LCD_FB_FORMAT_NUM] = { 2, 3, 4 };

unsigned int wait_time = 0;
struct LCD_CONTEXT {
	LCD_FB_FORMAT fbFormat;
	uint32_t fbPitchInBytes;
	LCD_REG_SIZE roiWndSize;
	LCD_OUTPUT_MODE outputMode;
	LCD_REGS regBackup;
	void (*pIntCallback)(DISP_INTERRUPT_EVENTS);
};
#define LCD_CONTEXT    struct LCD_CONTEXT


static LCD_CONTEXT _lcdContext = { 0 };

static int wst_step_LCD = -1;	/* for LCD&FM de-sense */
static bool is_get_default_write_cycle;
static unsigned int default_write_cycle;
static uint32_t default_wst;
static bool limit_w2m_speed;
static bool limit_w2tvr_speed;
...
LCD_STATUS LCD_Init(void)
{
	...
	MASKREG32(DDP_REG_BASE_MMSYS_CONFIG + 0x60, 0x1, 0x0);
#if ENABLE_LCD_INTERRUPT
	if (request_irq(disp_dev.irq[DISP_REG_DSI],
...
LCD_STATUS LCD_ConfigParallelIF(LCD_IF_ID id,
				...
				uint32_t writeWait,
				uint32_t readSetup,
				uint32_t readHold, uint32_t readLatency, uint32_t waitPeriod, uint32_t chw)
{
	...
	/* (1) Config Data Width */
	{
		...
		case LCD_IF_PARALLEL_0:
			pcnfdw.PCNF0_DW = (uint32_t) ifDataWidth;
			pcnfdw.PCNF0_CHW = chw;
			break;
		case LCD_IF_PARALLEL_1:
			pcnfdw.PCNF1_DW = (uint32_t) ifDataWidth;
			pcnfdw.PCNF1_CHW = chw;
			break;
		case LCD_IF_PARALLEL_2:
			pcnfdw.PCNF2_DW = (uint32_t) ifDataWidth;
	...
	/* (2) Config Timing */
	{
		...
		i = (uint32_t) id - LCD_IF_PARALLEL_0;
			      ...
			      LCD_IF_FORMAT format, LCD_IF_WIDTH busWidth)
{
	...
	ctrl.DATA_FMT = (uint32_t) format;
	ctrl.IF_FMT = (uint32_t) busWidth;
	ctrl.IF_24 = 0;
			      ...
			      uint32_t rd_1st, uint32_t rd_2nd, uint32_t wr_1st, uint32_t wr_2nd)
{
	...
	pr_info("LCD_ConfigSerialIF 1\n");
	ASSERT(id >= LCD_IF_SERIAL_0 && id <= LCD_IF_SERIAL_1);

	pr_info("LCD_ConfigSerialIF 2\n");

	_WaitForEngineNotBusy();

	pr_info("LCD_ConfigSerialIF 3\n");
...
/* LCD_REG->RESET = high ? 1 : 0; */
	LCD_OUTREG32(DDP_REG_BASE_MMSYS_CONFIG + 0x13C, high);
...
uint32_t LCD_LayerEnableTdshp(LCD_LAYER_ID id, uint32_t en)
{
#if !defined(MTK_OVERLAY_ENGINE_SUPPORT)
...
uint32_t LCD_LayerSetNextBuffIdx(LCD_LAYER_ID id, int32_t idx)
{
#if !defined(MTK_OVERLAY_ENGINE_SUPPORT)
...
uint32_t LCD_LayerGetInfo(LCD_LAYER_ID id, uint32_t *enabled, int32_t *curr_idx, int32_t *next_idx)
{
#if !defined(MTK_OVERLAY_ENGINE_SUPPORT)
...
uint32_t LCD_DisableAllLayer(uint32_t vram_start, uint32_t vram_end)
{
	int id;
	int layer_enable = 0;
#if defined(MTK_OVERLAY_ENGINE_SUPPORT)
	OVL_CONFIG_STRUCT cached_layer[DDP_OVL_LAYER_MUN] = {
		{.layer = 0, .isDirty = 1},
		{.layer = 1, .isDirty = 1},
		{.layer = 2, .isDirty = 1},
		{.layer = 3, .isDirty = 1}
	...
	return layer_enable;
#else
	Disp_Ovl_Engine_Dump_layer_info(mtkfb_instance, (OVL_CONFIG_STRUCT *) cached_layer, NULL,
					NULL);

	for (id = 0; id < DDP_OVL_LAYER_MUN; id++) {
		if (cached_layer[id].layer_en == 0)
			continue;

		if (cached_layer[id].addr >= vram_start && cached_layer[id].addr < vram_end) {
			DISP_LOG_PRINT(ANDROID_LOG_INFO, "LCD", "  not disable(%d)\n", id);
			layer_enable |= (1 << id);
			continue;
		}
		...
		DISP_LOG_PRINT(ANDROID_LOG_INFO, "LCD", "  disable(%d)\n", id);
		{
			...
			fb_layer.layer_id = id;
			Disp_Ovl_Engine_Get_layer_info(mtkfb_instance, &fb_layer);
			fb_layer.layer_enable = 0;
			Disp_Ovl_Engine_Set_layer_info(mtkfb_instance, &fb_layer);
		}
	}
	return layer_enable;
#endif
...
LCD_STATUS LCD_LayerSetDestColorKey(LCD_LAYER_ID id, bool enable, uint32_t colorKey)
{
	/* mt6588 OVL driver not yet export this function */
	return LCD_STATUS_OK;
}
...
LCD_STATUS LCD_TE_ConfigVHSyncMode(uint32_t hsDelayCnt,
				   uint32_t vsWidthCnt, LCD_TE_VS_WIDTH_CNT_DIV vsWidthCntDiv)
{
/*    LCD_REG_TECON tecon = LCD_REG->TEARING_CFG;
...
static inline void _LCD_WriteIF(unsigned int baseAddr, uint32_t value, LCD_IF_MCU_WRITE_BITS bits)
{
	switch (bits) {
...
LCD_STATUS LCD_WriteIF(LCD_IF_ID id, LCD_IF_A0_MODE a0, uint32_t value, LCD_IF_MCU_WRITE_BITS bits)
{
	unsigned int baseAddr = 0;

	switch (id) {
	case LCD_IF_PARALLEL_0:
		baseAddr = (unsigned int) &LCD_REG->PCMD0;
		break;
	case LCD_IF_PARALLEL_1:
		baseAddr = (unsigned int) &LCD_REG->PCMD1;
		break;
	case LCD_IF_PARALLEL_2:
		baseAddr = (unsigned int) &LCD_REG->PCMD2;
		break;
	case LCD_IF_SERIAL_0:
		baseAddr = (unsigned int) &LCD_REG->SCMD0;
		break;
	case LCD_IF_SERIAL_1:
		baseAddr = (unsigned int) &LCD_REG->SCMD1;
		break;
	default:
		ASSERT(0);
	}
...
static inline uint32_t _LCD_ReadIF(unsigned int baseAddr, LCD_IF_MCU_WRITE_BITS bits)
{
	switch (bits) {
	case LCD_IF_MCU_WRITE_8BIT:
		return (uint32_t) INREG8(baseAddr);

	case LCD_IF_MCU_WRITE_16BIT:
		return (uint32_t) INREG16(baseAddr);

	case LCD_IF_MCU_WRITE_32BIT:
		return (uint32_t) DISP_REG_GET(baseAddr);
...
LCD_STATUS LCD_ReadIF(LCD_IF_ID id, LCD_IF_A0_MODE a0, uint32_t *value, LCD_IF_MCU_WRITE_BITS bits)
{
	unsigned int baseAddr = 0;
...
LCD_STATUS LCD_DumpRegisters(void)
{
	...
	for (i = 0; i < offsetof(LCD_REGS, GMC_ULTRA_TH); i += 4)
		pr_info("LCD+%04x : 0x%08x\n", i, DISP_REG_GET(LCD_BASE + i));
...
#if 0				/* !defined(CONFIG_MTK_M4U_SUPPORT) */
#ifdef BUILD_UBOOT
				   ...
				   unsigned int video_rotation)
{
	...
	case 1:
		...
		break;
	case 2:
		LCD_CHECK_RET(LCD_LayerSetRotation(LCD_LAYER_2, LCD_LAYER_ROTATE_180));
		break;
	case 3:
...
LCD_STATUS LCD_FM_Desense(LCD_IF_ID id, unsigned long freq)
{
	uint32_t a, b;
	uint32_t c, d;
	uint32_t wst, c2wh, chw, write_cycles;
	LCD_REG_PCNF config;
/* LCD_REG_WROI_CON ctrl; */
	LCD_REG_PCNFDW pcnfdw;

	OUTREG32(&config, AS_UINT32(&LCD_REG->PARALLEL_CFG[(uint32_t) id]));
	DBI_LOG("[enter LCD_FM_Desense]:parallel IF = 0x%x",
		DISP_REG_GET(&LCD_REG->PARALLEL_CFG[(uint32_t) id]));
	wst = config.WST;
	c2wh = config.C2WH;
...
/* OUTREG32(&ctrl, AS_UINT32(&LCD_REG->WROI_CONTROL)); */
	...
	if ((b > a) || (c == 0) || (d == 0)) {	/* need modify setting to avoid interference */
		...
		config.WST = wst;
		LCD_WaitForNotBusy();
		OUTREG32(&LCD_REG->PARALLEL_CFG[(uint32_t) id], AS_UINT32(&config));
	} else {
...
LCD_STATUS LCD_Reset_WriteCycle(LCD_IF_ID id)
{
	...
	if (wst_step_LCD > 0) {
		...
		DBI_LOG
		    ("[LCD_Reset_WriteCycle] parallel is default setting, not need to reset it\n");
	}
...
/* LCD_REG_WROI_CON ctrl; */
	...
	OUTREG32(&config, AS_UINT32(&LCD_REG->PARALLEL_CFG[(uint32_t) id]));
	DBI_LOG("[enter LCD_Get_Default_WriteCycle]:parallel IF = 0x%x\n",
		DISP_REG_GET(&LCD_REG->PARALLEL_CFG[(uint32_t) id]));
	wst = config.WST;
	c2wh = config.C2WH;
	/* Config Delay Between Commands */
...
/* LCD_REG_WROI_CON ctrl; */
	...
	OUTREG32(&config, AS_UINT32(&LCD_REG->PARALLEL_CFG[(uint32_t) id]));
	DBI_LOG("[enter LCD_Get_Current_WriteCycle]:parallel IF = 0x%x\n",
		DISP_REG_GET(&LCD_REG->PARALLEL_CFG[(uint32_t) id]));
	wst = config.WST;
	c2wh = config.C2WH;
	/* Config Delay Between Commands */
...
LCD_STATUS LCD_Change_WriteCycle(LCD_IF_ID id, unsigned int write_cycle)
{
	...
	DBI_LOG("[enter LCD_Change_WriteCycle]:parallel IF = 0x%x, ctrl = 0x%x\n",
		DISP_REG_GET(&LCD_REG->PARALLEL_CFG[(uint32_t) id]), DISP_REG_GET(&LCD_REG->WROI_CONTROL));
...
#endif
#if 0
	m4u_insert_seq_range(M4U_CLNTMOD_LCDC_UI, *mva, *mva + size - 1, RT_RANGE_HIGH_PRIORITY, 0);
#endif
	return LCD_STATUS_OK;
}
...
LCD_STATUS LCD_AllocOverlayMva(unsigned int va, unsigned int *mva, unsigned int size)
{
	...
	m4u_insert_seq_range(M4U_CLNTMOD_LCDC, *mva, *mva + size - 1, RT_RANGE_HIGH_PRIORITY, 0);
#endif
	return LCD_STATUS_OK;
}
...
int m4u_alloc_mva_stub(M4U_MODULE_ID_ENUM eModuleID, const unsigned int BufAddr,
		       const unsigned int BufSize, unsigned int *pRetMVABuf)
{
#if 0
	if (!_m4u_lcdc_func.isInit) {
...
int m4u_dealloc_mva_stub(M4U_MODULE_ID_ENUM eModuleID, const unsigned int BufAddr,
...
int m4u_insert_tlb_range_stub(M4U_MODULE_ID_ENUM eModuleID,
			      ...
			      M4U_RANGE_PRIORITY_ENUM ePriority, unsigned int entryCount)
{
