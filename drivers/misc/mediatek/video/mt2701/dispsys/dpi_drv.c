...
#if !(defined(CONFIG_MT8127_FPGA) || defined(BUILD_UBOOT))
/* #define DPI_MIPI_API */
#endif
...
DPI_STATUS DPI_Init(bool isDpiPoweredOn)
{
	...
	enable_mux(MT_MUX_DPILVDS, "DPILVDS");
	disable_mux(MT_MUX_DPILVDS, "DPILVDS");
#endif
...
DPI_STATUS DPI_PowerOn(void)
{
	if (!s_isDpiPowerOn) {
		...
		ret += enable_clock(MT_CG_DISP1_LVDS_PIXEL_CLOCK, "DPI0");
		ret += enable_clock(MT_CG_DISP1_LVDS_CTS_CLOCK, "DPI0");

		if (lcm_params->dpi.lvds_tx_en == 0) {

			enable_mux(MT_MUX_DPI0, "DPI0");
			clkmux_sel(MT_MUX_DPI0, 1, "DPI0");
