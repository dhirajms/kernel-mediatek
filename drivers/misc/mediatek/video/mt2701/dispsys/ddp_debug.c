...
static void process_dbg_opt(const char *opt)
{
	...
	if (0 == strncmp(opt, "regr:", 5)) {
		...
		STR_CONVERT(&p, &module, uint, goto Error);
...
("ovl%d: en=%d, layer(%d,%d,%d,%d), bg_w=%d, bg_h=%d, cur_x=%d, cur_y=%d, layer_pix_hit(%d, %d, %d, %d)\n",
		...
		DISP_MSG
		    ("ovl%d: greq(ovl_req_smi)=%d, valid(ovl_send_data)=%d, ready(follow_engine_req_data)=%d\n",
		     index, DISP_REG_GET(DISP_REG_CONFIG_GREQ) & 0x1,
