...
enum OVL_INPUT_FORMAT ovl_fmt_convert(DpColorFormat fmt)
{
	...
	default:
		pr_notice("error: DDP, unknown ovl input format = %d\n", fmt);
	}
...
int OVLReset(void)
{
	...
	pr_notice("[DDP] OVLReset called %d\n", cnt++);
	...
	while (((DISP_REG_GET(DISP_REG_OVL_FLOW_CTRL_DBG) & 0x3ff) != 0x1) &&
		...
		delay_cnt++;
		if (delay_cnt > 10000) {
			pr_notice("[DDP] error, OVLReset() timeout!\n");
...
int OVLLayerSwitch(unsigned layer, bool en)
{
	...
	case 3:
		...
		break;
	default:
		pr_notice("error: invalid layer=%d\n", layer);	/* invalid layer */
		ASSERT(0);
	}
		...
		unsigned int en_3d, unsigned int landscape, unsigned int r_first)
{
	...
	case 3:
		...
		break;
	default:
		pr_notice("error: OVL3DConfig(), invalid layer=%d\n", layer_id);	/* invalid layer */
		ASSERT(0);
	}
		...
		unsigned char alpha)
{
	...
	DISP_DDP
	    ("layer%d/source%d/format0x%x/addr0x%x/src(%d,%d,%d)/dst(%d,%d,%d,%d)/key(%d,%d)/alpha(%d,%d)\n",
