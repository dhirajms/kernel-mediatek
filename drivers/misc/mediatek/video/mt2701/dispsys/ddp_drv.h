...
/*
 * CMDQ sec address metadata:
 * replase (_d)th instruciton to sec_addr,  sec_addr = hadnle_sec_base_addr(baseHandle, isMva) + offset(_b)
 */
typedef struct {
	uint32_t instrIndex;	/* _d, index of instruction, which will be replace instruciton value in secure world */
	uint32_t baseHandle;	/* _h, secure handle */
	uint32_t offset;	/* _b, buffser offset to secure handle */
	bool isMVA;		/* true, if address has to config as mva */
	uint32_t size;		/* buffer size */
	uint32_t port;		/* hw port */
} DISP_SEC_ADDR_METADATA;

/*
 * CMDQ sec port metadata:
 * config HW port to sec/non-sec, useMva or not
 */
typedef struct {
	uint32_t port;
	bool useMVA;		/* true, if HW engine access mva, not PA */
} DISP_SEC_PORT_METADATA;
...
typedef struct DISP_EXEC_SEC_METADATA {
	...
	/* discarded! */
	/* the following will be phased out latter */
...
#define DISP_IOCTL_CMDQ_SEC_TEST        _IOR(DISP_IOCTL_MAGIC, 36, int)
...
#define DISP_SECURE_SET_MODE_BITBLT       _IOW(DISP_IOCTL_MAGIC, 82 , enum DpSecure)
#define DISP_SECURE_SET_MODE_OVL_MEM_OUT  _IOW(DISP_IOCTL_MAGIC, 83 , enum DpSecure)
...
extern struct disp_device disp_dev;
