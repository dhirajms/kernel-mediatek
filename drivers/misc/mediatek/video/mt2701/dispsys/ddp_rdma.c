		...
		bool isRGBSwap, unsigned int RDMASecurity)
{
	...
	if ((width > RDMA_MAX_WIDTH) || (height > RDMA_MAX_HEIGHT))
		pr_notice("DDP error, RDMA input overflow, w=%d, h=%d, max_w=%d, max_h=%d\n", width,
			  height, RDMA_MAX_WIDTH, RDMA_MAX_HEIGHT);

	if (width == 0 || height == 0) {
		pr_notice("DDP error, RDMA input error, w=%d, h=%d, pitch=%d\n", width, height,
			  pitch);
		ASSERT(width > 0);
		ASSERT(height > 0);
	}
	...
	default:
		pr_notice("DDP error, unknown RDMA input format = %d\n", inputFormat);
		ASSERT(0);
	}
	...
	pr_notice("RDMA%d: fifo_pseudo_length=%d, fifo_threashold=%d\n",
		  idx, fifo_pseudo_length, fifo_threashold);
	pr_notice("ultra_low_level=%x, pre_ultra_low_level=%d, pre_ultra_high_level=%d",
		  ultra_low_level, pre_ultra_low_level, pre_ultra_high_level);
	pr_notice("ultra_high_ofs=%d, pre_ultra_low_ofs=%d, pre_ultra_high_ofs=%d\n",
	...
	default:
		pr_notice("DDP error, unknown input format is %d\n", inputFormat);
		ASSERT(0);
	}
...
void RDMAWait(unsigned idx)
{
	...
	while ((DISP_REG_GET(idx * DISP_INDEX_OFFSET + DISP_REG_RDMA_INT_STATUS) & 0x1) != 0x1) {
		...
		mdelay(1);
		if (delay_cnt > 100) {
			pr_notice("[DDP] error:RDMA%dWait timeout\n", idx);
			break;
		}
	}
...
{
	unsigned int line = DISP_REG_GET(idx * DISP_INDEX_OFFSET + DISP_REG_RDMA_TARGET_LINE);
	return line;
}
...
enum RDMA_INPUT_FORMAT rdma_fmt_convert(DpColorFormat fmt)
{
	...
	case eVYUY:
		...
		break;
	default:
		pr_notice("error: rdma_fmt_convert fmt=%d, rdma_fmt=%d\n", fmt, rdma_fmt);
	}
	return rdma_fmt;
}
