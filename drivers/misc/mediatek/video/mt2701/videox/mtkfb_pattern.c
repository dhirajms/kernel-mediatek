...
void *fb_pattern_allocate_secure_memory(int size)
{
	...
	/* Allocate */
	ret =
	    KREE_AllocSecurechunkmem(fb_pattern_secure_memory_session_handle(), &mem_handle, 4096,
				     size);
	if (ret != TZ_RESULT_SUCCESS) {
		pr_notice("KREE_AllocSecurechunkmem fail, ret=%d\n", ret);
		return NULL;
	}

	pr_notice("KREE_AllocSecurchunkemem handle=0x%x\n", mem_handle);
