...
static bool disp_drv_dbi_init_context(void)
{
	...
	pr_info("%s, lcm_drv=0x%08x, lcm_params=0x%08x\n", __func__,
		(unsigned int)lcm_drv, (unsigned int)lcm_params);

	pr_info("%s, lcm_drv=0x%08x\n", __func__, (unsigned int)lcm_drv);
	if (NULL == lcm_drv) {
...
static void init_lcd_te_control(void)
{
	 ...
	 */
	/* but for uboot, the boot mode selection is done after lcd init,
	   so we have to disable te always in uboot. */
...
static DISP_STATUS dbi_init(uint32_t fbVA, uint32_t fbPA, bool isLcmInited)
{
...
#ifdef MT65XX_NEW_DISP
	{
		...
		/* Config FB_Layer port to be physical. */
		{
			...
			portStruct.ePortID = M4U_PORT_OVL_CH3;
		...
		/* Config LK UI layer port to be physical. */
		{
			M4U_PORT_STRUCT portStruct;
			/* hardware port ID, defined in M4U_PORT_ID_ENUM */
			portStruct.ePortID = M4U_PORT_OVL_CH2;
			portStruct.Virtuality = 1;
			portStruct.Security = 0;
			portStruct.domain = 3;	/* domain : 0 1 2 3 */
			portStruct.Distance = 1;
...
static uint32_t dbi_get_working_buffer_size(void)
{
	return 0;
}

static uint32_t dbi_get_working_buffer_bpp(void)
{
	return 0;
}
...
static uint32_t dbi_get_dithering_bpp(void)
{
	return PANEL_COLOR_FORMAT_TO_BPP(dbi_get_panel_color_format());
}

DISP_STATUS dbi_capture_framebuffer(uint32_t pvbuf, uint32_t bpp)
{
