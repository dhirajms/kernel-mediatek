...
void disp_ovl_engine_dump_ovl(int idx)
{
	...
	if (gDumpOVLEnable) {
		if (idx == 0) {
			...
			Bitmap.pData =
			    (void *)disp_ovl_engine.OvlBufAddr_va[disp_ovl_engine.RdmaRdIdx];

			pr_notice("[OVL] start to dump OVL\n");
			...
			/* pr_notice("[OVL] map 0x%x(mva)->0x%x(va) size:0x%x\n", */
			/* disp_ovl_engine.OvlBufAddr[disp_ovl_engine.RdmaRdIdx], */
			/* Bitmap.pData, Bitmap.data_size); */

			/* if (Bitmap.data_size != 0) */
			MMProfileLogMetaBitmap(MTKFB_MMP_Events.OvlDump, MMProfileFlagPulse,
					       &Bitmap);

			/* disphal_dma_unmap_kernel(disp_ovl_engine.OvlBufAddr[disp_ovl_engine.RdmaRdIdx], */
			/* Bitmap.data_size, (unsigned int)Bitmap.pData); */
			pr_notice("[OVL] dump OVL end\n");
		}
	}
}

static void disp_ovl_engine_release_fence(int idx, DISP_OVL_ENGINE_INSTANCE *instance)
{
#ifdef DISP_OVL_ENGINE_FENCE
	if (instance->outFence != -1) {
		sw_sync_timeline_inc(instance->timeline, 1 + instance->timeline_skip);
		DISP_OVL_ENGINE_DBG("disp_ovl_engine_kthread %d timeline inc %d\n",
				    idx, 1 + instance->timeline_skip);
		instance->timeline_skip = 0;
		MMProfileLogEx(MTKFB_MMP_Events.OVLEngine_timeline[idx],
			       MMProfileFlagPulse, idx, instance->timeline->value);
	}

	/* free ion handle */
	if (DECOUPLE_MODE == instance->mode) {
		int j;

		for (j = 0; j < HW_OVERLAY_COUNT; j++) {
			if (instance->cached_layer_config[j].fgIonHandleImport) {
				ion_free(disp_ovl_engine.ion_client,
					 instance->cached_layer_config[j].ion_handles);
				instance->cached_layer_config[j].fgIonHandleImport = false;
				instance->cached_layer_config[j].ion_handles = NULL;
			}
		}
	}
#endif
...
static int disp_ovl_engine_kthread(void *data)
{
	...
	while (1) {
		...
		wait_ret =
		    wait_event_interruptible(disp_ovl_engine_wq,
		...
		/* Overlay complete notification */
		{
			...
			for (i = 0; i < OVL_ENGINE_INSTANCE_MAX_INDEX; i++) {
				if (disp_ovl_engine.Instance[i].status == OVERLAY_STATUS_COMPLETE) {
					...
					disp_ovl_engine_release_fence(i,
		...
		if (temp->status == OVERLAY_STATUS_BUSY) {
			...
			if ((currentTime - gStartTriggerTime) > 1000) {
				...
				disp_ovl_engine_release_fence(curr_instance_id,
							      &disp_ovl_engine.
							      Instance[curr_instance_id]);
