...
#define DISP_OVL_ENGINE_ERR(string, args...) \
	do { \
		if (disp_ovl_engine_log_level >= 0) \
			pr_err("[OVL] mt8127 "string, ##args); \
	} while (0)
#define DISP_OVL_ENGINE_WARN(string, args...) \
	do { \
		if (disp_ovl_engine_log_level >= 1) \
			pr_err("[OVL] mt8127 "string, ##args); \
	} while (0)
#define DISP_OVL_ENGINE_INFO(string, args...) \
	do { \
		if (disp_ovl_engine_log_level >= 2) \
			pr_err("[OVL] mt8127 "string, ##args); \
	} while (0)
#define DISP_OVL_ENGINE_DBG(string, args...) \
	do { \
		if (disp_ovl_engine_log_level >= 3) \
			pr_err("[OVL] mt8127 "string, ##args); \
	} while (0)
