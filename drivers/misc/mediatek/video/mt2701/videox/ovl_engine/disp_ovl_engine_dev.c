...
int disp_ovl_engine_open(struct inode *inode, struct file *file)
{
	int i;
	...
	DISP_OVL_ENGINE_INFO("disp_ovl_engine_open\n");
	...
	for (i = 0; i < DISP_OVL_ENGINE_INODE_NUM; i++)
		if (disp_ovl_engine_file_handle_map[i].mFile == NULL) {
			...
			file->private_data = (void *)&disp_ovl_engine_file_handle_map[i];
			break;
		}
#ifdef DISP_OVL_ENGINE_REQUEST
	/* For OvlFd */
...
int disp_ovl_engine_release(struct inode *inode, struct file *file)
{
	DISP_OVL_ENGINE_FILE_HANDLE_MAP *map;

	DISP_OVL_ENGINE_INFO("disp_ovl_engine_release\n");

	map = (DISP_OVL_ENGINE_FILE_HANDLE_MAP *) file->private_data;
	Disp_Ovl_Engine_ReleaseInstance(map->mHandle);
	map->mFile = NULL;
...
long disp_ovl_engine_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (is_early_suspended) {
		DISP_OVL_ENGINE_ERR
		    ("[disp_ovl_engine_unlocked_ioctl] is_early_suspended,cmd=0x%x\n", cmd);
		return -1;
	}
	...
	case DISP_OVL_ENGINE_IOCTL_GET_LAYER_INFO:
		{
			...
			DISP_OVL_ENGINE_INFO
			    ("disp_ovl_engine_unlocked_ioctl cmd: %d DISP_OVL_ENGINE_IOCTL_GET_LAYER_INFO\n",
			     cmd & 0xff);
	...
	case DISP_OVL_ENGINE_IOCTL_SET_LAYER_INFO:
		{
			...
			DISP_OVL_ENGINE_INFO
			    ("disp_ovl_engine_unlocked_ioctl cmd: %d DISP_OVL_ENGINE_IOCTL_SET_LAYER_INFO\n",
			     cmd & 0xff);
	...
	case DISP_OVL_ENGINE_IOCTL_SET_OVERLAYED_BUF:
		{
			...
			DISP_OVL_ENGINE_INFO
			    ("disp_ovl_engine_unlocked_ioctl cmd: %d DISP_OVL_ENGINE_IOCTL_SET_OVERLAYED_BUF\n",
			     cmd & 0xff);
			...
			ret = Disp_Ovl_Engine_Set_Overlayed_Buffer(handle, &overlayedBufInfo);
	...
	case DISP_OVL_ENGINE_IOCTL_TRIGGER_OVERLAY:
		{
			...
			DISP_OVL_ENGINE_INFO
			    ("disp_ovl_engine_unlocked_ioctl cmd: %d DISP_OVL_ENGINE_IOCTL_TRIGGER_OVERLAY\n",
			     cmd & 0xff);
	...
	case DISP_OVL_ENGINE_IOCTL_TRIGGER_OVERLAY_FENCE:
		{
			...
			DISP_OVL_ENGINE_INFO
			    ("disp_ovl_engine_unlocked_ioctl cmd: %d DISP_OVL_ENGINE_IOCTL_TRIGGER_OVERLAY_FENCE\n",
			     cmd & 0xff);

			if (copy_from_user
			    (&setting, (void __user *)arg, sizeof(struct disp_ovl_engine_setting)))
				return -EFAULT;

			ret = Disp_Ovl_Engine_Trigger_Overlay_Fence(handle, &setting);

			if (copy_to_user
			    ((void __user *)arg, &setting, sizeof(struct disp_ovl_engine_setting)))
				return -EFAULT;
	...
	case DISP_OVL_ENGINE_IOCTL_GET_FENCE_FD:
		{
			...
			DISP_OVL_ENGINE_INFO
			    ("disp_ovl_engine_unlocked_ioctl cmd: %d DISP_OVL_ENGINE_IOCTL_GET_FENCE_FD\n",
			     cmd & 0xff);
	...
	case DISP_OVL_ENGINE_IOCTL_WAIT_OVERLAY_COMPLETE:
		{
			...
			DISP_OVL_ENGINE_INFO("DISP_OVL_ENGINE_IOCTL_WAIT_OVERLAY_COMPLETE start\n");
			ret = Disp_Ovl_Engine_Wait_Overlay_Complete(handle, timeout);
			DISP_OVL_ENGINE_INFO("DISP_OVL_ENGINE_IOCTL_WAIT_OVERLAY_COMPLETE done\n");
			return ret;
		}
		break;
	case DISP_OVL_ENGINE_IOCTL_SECURE_MVA_MAP:
		{
			...
			DISP_OVL_ENGINE_INFO
			    ("disp_ovl_engine_unlocked_ioctl cmd: %d DISP_OVL_ENGINE_IOCTL_SECURE_MVA_MAP\n",
			     cmd & 0xff);
	...
	case DISP_OVL_ENGINE_IOCTL_SECURE_MVA_UNMAP:
		{
			...
			DISP_OVL_ENGINE_INFO
			    ("disp_ovl_engine_unlocked_ioctl cmd: %d DISP_OVL_ENGINE_IOCTL_SECURE_MVA_UNMAP\n",
			     cmd & 0xff);
	...
	case DISP_OVL_ENGINE_IOCTL_GET_REQUEST:
		{
			...
			DISP_OVL_ENGINE_INFO
			    ("disp_ovl_engine_unlocked_ioctl cmd: %d DISP_OVL_ENGINE_IOCTL_GET_REQUEST\n",
			     cmd & 0xff);
	...
	case DISP_OVL_ENGINE_IOCTL_ACK_REQUEST:
		{
			...
			DISP_OVL_ENGINE_INFO
			    ("disp_ovl_engine_unlocked_ioctl cmd: %d DISP_OVL_ENGINE_IOCTL_ACK_REQUEST\n",
			     cmd & 0xff);
...
/* Kernel interface */
static const struct file_operations disp_ovl_engine_fops = {
	.owner = THIS_MODULE,
...
late_initcall(_disp_ovl_engine_init);
