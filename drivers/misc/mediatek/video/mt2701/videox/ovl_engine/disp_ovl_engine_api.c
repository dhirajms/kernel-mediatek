				   ...
				   struct fb_overlay_layer *layerInfo)
{
	...
	if (layerInfo->layer_enable) {
		...
		ovl_config_tmp->fence_fd = layerInfo->fence_fd;
		MMProfileLogEx(MTKFB_MMP_Events.OVLEngine_fence[handle], MMProfileFlagPulse,
			       layerInfo->layer_id, layerInfo->fence_fd);
#endif
		...
		struct disp_ovl_engine_config_mem_out_struct *overlayedBufInfo)
{
	...
	do {
		...
		/* pr_err("[layeredbuf]overlayedBufInfo->dstAddr=0x%08x overlayedBufInfo->ion_fd=%d",
...
int Disp_Ovl_Engine_Wait_Overlay_Complete(DISP_OVL_ENGINE_INSTANCE_HANDLE handle, int timeout)
{
	...
	while (loopNum) {
		...
		wait_ret = wait_event_interruptible_timeout(disp_ovl_complete_wq,
							    atomic_read(&disp_ovl_engine.Instance
									[handle].fgCompleted),
							    subTimeout);
...
static void Disp_Ovl_Engine_Trigger_Overlay_Handler(struct work_struct *work_)
{
	...
	MMProfileLogEx(MTKFB_MMP_Events.Trigger_Overlay_Handler, MMProfileFlagPulse,
		       instance->index, 0);
	...
	atomic_set(&disp_ovl_engine.Instance[instance->index].fgCompleted, 0);
