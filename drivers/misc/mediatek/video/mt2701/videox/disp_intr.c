...
static DISP_INTERRUPT_CALLBACK_STRUCT DISP_CallbackArray[DISP_LCD_INTERRUPT_EVENTS_NUMBER +
							 DISP_DSI_INTERRUPT_EVENTS_NUMBER +
							 DISP_DPI_INTERRUPT_EVENTS_NUMBER];
...
static void _DISP_InterruptCallbackProxy(DISP_INTERRUPT_EVENTS eventID)
{
	...
	} else if (eventID >= DISP_DSI_INTERRUPT_EVENTS_START
		   && eventID <= DISP_DSI_INTERRUPT_EVENTS_END) {
		offset =
		    eventID - DISP_DSI_INTERRUPT_EVENTS_START + DISP_LCD_INTERRUPT_EVENTS_NUMBER;
		if (DISP_CallbackArray[offset].pFunc)
			DISP_CallbackArray[offset].pFunc(DISP_CallbackArray[offset].pParam);

	} else if (eventID >= DISP_DPI_INTERRUPT_EVENTS_START
		   && eventID <= DISP_DPI_INTERRUPT_EVENTS_END) {
		offset =
		    eventID - DISP_DPI_INTERRUPT_EVENTS_START + DISP_LCD_INTERRUPT_EVENTS_NUMBER +
		    DISP_DSI_INTERRUPT_EVENTS_NUMBER;
		if (DISP_CallbackArray[offset].pFunc)
			DISP_CallbackArray[offset].pFunc(DISP_CallbackArray[offset].pParam);

	} else {
		pr_info("Invalid event id: %d\n", eventID);
		ASSERT(0);
	}

}

DISP_STATUS DISP_SetInterruptCallback(DISP_INTERRUPT_EVENTS eventID,
				      DISP_INTERRUPT_CALLBACK_STRUCT *pCBStruct)
{
	uint32_t offset;
