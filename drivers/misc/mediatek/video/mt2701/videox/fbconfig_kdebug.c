...
#define dsi_set_cmdq(pdata, queue_size, force_update) \
			fbconfig_lcm_utils.dsi_set_cmdq(pdata, queue_size, force_update)
#define read_reg_v2(cmd, buffer, buffer_size) \
			fbconfig_lcm_utils.dsi_dcs_read_lcm_reg_v2(cmd, buffer, buffer_size)
...
static void record_list_init(void)
{
	...
	record_head = kmalloc(sizeof(CONFIG_RECORD), GFP_KERNEL);
	...
	record_tmp = kmalloc(sizeof(CONFIG_RECORD), GFP_KERNEL);
...
{
	CONFIG_RECORD *record_add = kmalloc(sizeof(CONFIG_RECORD), GFP_KERNEL);
...
static void fbconfig_backup_lcm_setting(void)
{
	...
	if (tmp != NULL) {
		backup_head = kmalloc(sizeof(CONFIG_RECORD), GFP_KERNEL);
	...
	while (tmp != NULL) {
		new_node = kmalloc(sizeof(CONFIG_RECORD), GFP_KERNEL);
