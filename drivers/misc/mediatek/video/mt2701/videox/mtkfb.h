...
typedef enum {			/* map sessions to scenairos in kernel driver */
	...
	DISP_SESSION_WFD = 1 << 2,	/* Extension mode */
...
struct fb_overlay_config {
	int fence;
	int time;
	struct fb_overlay_layer layers[4];	/* HW_OVERLAY_COUNT */
};
...
typedef struct {
	...
	struct sync_fence *fences[4];	/* HW_OVERLAY_COUNT */
	struct ion_handle *ion_handles[4];	/* HW_OVERLAY_COUNT */
	void *dev;
	unsigned int is_v4l2;	/* v4l2 flag */
} update_ovls_work_t;
...
extern void MTK_HDMI_Set_Security_Output_SVP_P1(int enable);
extern void MMProfileEnable(int enable);
extern void MMProfileStart(int start);
extern void init_mtkfb_mmp_events(void);
extern void init_ddp_mmp_events(void);
