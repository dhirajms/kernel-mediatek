...
static SYNC_STATUS disp_sync_ion_import_handle(struct ion_client *client, int fd,
					       struct ion_handle **hnd);
...
static size_t disp_sync_ion_phys_mmu_addr(struct ion_client *client, struct ion_handle *handle,
					  UINT *mva);
...
UINT disp_sync_query_buffer_mva(UINT layer, UINT idx)
{
	...
	if (mva != 0x0)
		...
		else		/* FIXME: non-ion buffer need cache sync here? */
 ...
 * 1. free ion handle
 * 2. recycle @buf_info node, put it into pool list
 */
...
/**
 * Markable the current buffers(Fence)
 * called this when HWC really provide the OVL buffer to be displayed
 * IF DISABLE LAYER, DON NOT call this! and its value is the last enabled buffer
 */
...
void disp_sync_signal_fence(UINT layer, UINT cur_idx)
{
	...
	if (timeline != NULL) {
		...
		if (num_fence > 0) {
			MMProfileLogEx(MTKFB_MMP_Events.SignalSyncFence, MMProfileFlagStart, layer,
				       info->timeline_idx);
			timeline_inc(timeline, num_fence);
			info->timeline_idx = cur_idx;
			XLOG_DBG("L%d signal idx=%d,cnt=%d\n", layer, info->timeline_idx,
				 num_fence);
			MMProfileLogEx(MTKFB_MMP_Events.SignalSyncFence, MMProfileFlagEnd, layer,
				       timeline->value);
		}
		mutex_unlock(&info->lock);
...
/**
 * 1. query a @buf_info list node
 * 2. create fence object
 * 3. create ion mva
 * 4. save fence fd, mva to @buf_info node
 * 5. add @buf_info node to @sync_info.buf_list
...
SYNC_STATUS disp_sync_prepare_buffer(struct fb_overlay_buffer *buf)
{
	...
	else if (disp_sync_ion_alloc_buffer(ion_client, buf->ion_fd, &mva, &handle) !=
		 SYNC_STATUS_OK)
		return SYNC_STATUS_ERROR;
	/* Fence Object */
