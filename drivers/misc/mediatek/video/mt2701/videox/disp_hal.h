...
#define DISP_STATUS		enum DISP_STATUS
...
#define DISP_STATE		enum DISP_STATE
...
#define PANEL_COLOR_FORMAT		enum PANEL_COLOR_FORMAT



struct DISP_IF_DRIVER {
	DISP_STATUS (*init)(uint32_t fbVA, uint32_t fbPA, bool isLcmInited);
	DISP_STATUS (*enable_power)(bool enable);
	DISP_STATUS (*update_screen)(bool isMuextLocked);

	uint32_t (*get_working_buffer_size)(void);
	uint32_t (*get_working_buffer_bpp)(void);
	PANEL_COLOR_FORMAT (*get_panel_color_format)(void);
	void (*init_te_control)(void);
	uint32_t (*get_dithering_bpp)(void);
...
#define DISP_IF_DRIVER		struct DISP_IF_DRIVER
...
int disphal_update_screen(const LCM_DRIVER *lcm_drv, struct mutex *pLcmCmdMutex, uint32_t x,
			  uint32_t y, uint32_t width, uint32_t height);
int disphal_set_backlight(const LCM_DRIVER *lcm_drv, struct mutex *pLcmCmdMutex, uint32_t level);
int disphal_set_backlight_mode(const LCM_DRIVER *lcm_drv, struct mutex *pLcmCmdMutex, uint32_t mode);
int disphal_set_pwm(const LCM_DRIVER *lcm_drv, struct mutex *pLcmCmdMutex, uint32_t divider);
int disphal_get_pwm(const LCM_DRIVER *lcm_drv, struct mutex *pLcmCmdMutex, uint32_t divider,
		    unsigned int *freq);
