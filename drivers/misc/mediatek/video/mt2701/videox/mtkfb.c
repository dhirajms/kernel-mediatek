...
#else
#define mtkfb_aee_print(string, args...) pr_err(string, ##args)
#endif
...
#endif
#if DISPLAY_V4L2
HOOK_PAN_DISPLAY_IMPL mtkfb_hook_pan_display = NULL;	/* Used for V4L2 */
bool g_is_v4l2_active = false;	/* Used for V4L2, Just for test. */
BUF_NOTIFY_CALLBACK mtkfb_buf_receive_cb = NULL;
BUF_NOTIFY_CALLBACK mtkfb_buf_remove_cb = NULL;
#endif
...
#define disp_dfo_printf(string, args...) pr_notice("[DISP/DFO]"string, ##args)
#else
...
static int mtkfb_pan_display_proxy(struct fb_var_screeninfo *var, struct fb_info *info)
{
	 ...
	 */
	if (mtkfb_hook_pan_display)
		if (0 == (*mtkfb_hook_pan_display) (var, info))
			return 0;
...
static void mtkfb_update_ovls_handler(struct work_struct *_work)
{
	...
	u32 inc = 0;
	unsigned int is_v4l2 = work->is_v4l2;
	...
	for (i = 0; i < HW_OVERLAY_COUNT; i++) {
		...
		pr_debug("[mtkfb_fence]Waitdone: layer%d/fence_fd%d/%s/ion_fd%d\n",
			 i, config->layers[i].fence_fd, fences[i]->name, config->layers[i].ion_fd);
		if (down_interruptible(&sem_early_suspend)) {
			dev_err(fbdev->dev, "semaphore down failed in %s\n", __func__);
			goto out;
		}
		...
		if (!is_early_suspended) {	/* Wait for the fence to signal */
			...
			if (sync_fence_wait(fences[i], 1000) < 0) {
				...
				/* ion_free(fbdev->ion_client, ion_handles[i]); */
				/* ion_handles[i] = NULL; */
				pr_err(" error waiting for fence to signal fences[%d]= 0x%p\n", i,
				       fences[i]);
				goto out;
			}
			pr_debug("[mtkfb_fence]Waitdone: layer%d/fence_fd%d/%s/ion_fd%d\n",
	...
	/* By locking OverlaySettingMutex here we prevent the update thread from committing the
...
#if DISPLAY_V4L2
	if (is_v4l2 && mtkfb_buf_receive_cb)
		mtkfb_buf_receive_cb(config->layers[0].src_phy_addr);
#endif
	...
	ret =
	    wait_event_interruptible_timeout(disp_reg_update_wq, atomic_read(&OverlaySettingApplied)
					     && !atomic_read(&OverlaySettingDirtyFlag), wait_time);
...
#if DISPLAY_V4L2
	if (is_v4l2 && mtkfb_buf_remove_cb)
		mtkfb_buf_remove_cb(config->layers[0].src_phy_addr);
#endif
...
out:
	...
	list_for_each_entry_safe(curr, tmp, &tmp_list, list) {
		/* pr_err("free ion handle = %x signal fence = %x\n",
		   curr->ion_handles[0], curr->fences[0], curr->ion_handles[1],
		   curr->fences[1], curr->ion_handles[2], curr->fences[2],
		   curr->ion_handles[3], curr->fences[3]); */
		for (i = 0; i < HW_OVERLAY_COUNT; i++) {
			/* ion handles */
			...
			/* fences */
			if (curr->fences[i]) {
				MMProfileLogEx(MTKFB_MMP_Events.HWCFence[i], MMProfileFlagPulse, 1,
					       (unsigned int)curr->fences[i]);
	...
	/* pr_err("step into out label in update_ovls_handler inc=%d fbdev->timeline->value = %d",
	   inc,   fbdev->timeline->value); */
	...
	if (0 == is_v4l2) {
		mutex_lock(&fbdev->timeline_lock);
		inc = fbdev->timeline_max - fbdev->timeline->value;
		sw_sync_timeline_inc(fbdev->timeline, 1);
		MMProfileLogEx(MTKFB_MMP_Events.FBTimeline, MMProfileFlagEnd,
			       fbdev->timeline->value, inc);
		pr_debug("[mtkfb_fence]Release: timeline%d/inc%d\n", fbdev->timeline->value, inc);
		mutex_unlock(&fbdev->timeline_lock);
	}
#endif
				  ...
				  unsigned int is_v4l2)
{
	...
	MMProfileLogStructure(MTKFB_MMP_Events.QueueWork, MMProfileFlagStart, config,
...
/*	if (!work) {
		pr_err("could not allocate update_ovls_work_t\n");
		ret = -ENOMEM;
		goto err;
	}*/
	...
	for (i = 0; i < HW_OVERLAY_COUNT; i++) {
		...
		MMProfileLogEx(MTKFB_MMP_Events.HWCFence[i], MMProfileFlagStart, layer->fence_fd,
			       (unsigned int)work->fences[i]);
		pr_debug("[mtkfb_fence]Prepare: layer%d/fence_fd%d/%s/ion_fd%d\n", i,
			 layer->fence_fd, work->fences[i]->name, layer->ion_fd);
		if (!work->fences[i]) {
	...
	if (0 == is_v4l2) {
		...
		pr_debug("[mtkfb_fence]create: value%d/timeline%d\n", fbdev->timeline_max,
			 fbdev->timeline->value);
		mutex_unlock(&fbdev->timeline_lock);
	}
...
static int mtkfb_set_overlay_layer(struct mtkfb_device *fbdev, struct fb_overlay_layer *layerInfo)
{
	...
	MTKFB_LOG("mtkfb_set_overlay_layer ===============================\n");
	MTKFB_LOG("  id=%u, en=%u, next_idx=%u, vaddr=0x%x, paddr=0x%x\n",
		  layerInfo->layer_id, layerInfo->layer_enable, layerInfo->next_buff_idx,
		  (unsigned int)(layerInfo->src_base_addr),
		  (unsigned int)(layerInfo->src_phy_addr));
	MTKFB_LOG("  fmt=%u, d-link=%u, pitch=%u, xoff=%u, yoff=%u, w=%u, h=%u\n",
	...
	MTKFB_LOG("  target xoff=%u, target yoff=%u, target w=%u, target h=%u\n\n",
...
static int mtkfb_get_overlay_layer_info(struct fb_overlay_layer_info *layerInfo)
{
	...
	MTKFB_LOG("[FB Driver] mtkfb_get_overlay_layer_info():\n");
	MTKFB_LOG
...
void mtkfb_dump_layer_info(void)
{
	...
	for (i = 0; i < DDP_OVL_LAYER_MUN; i++)
		pr_info("[mtkfb] layer=%d, layer_en=%d, idx=%d, fmt=%d, addr=0x%x, %d, %d, %d\n ",
			...
			captured_layer[i].layer_en, captured_layer[i].buff_idx, captured_layer[i].fmt,
	...
	for (i = 0; i < DDP_OVL_LAYER_MUN; i++)
		pr_info("[mtkfb] layer=%d, layer_en=%d, idx=%d, fmt=%d, addr=0x%x, %d, %d, %d\n ",
			...
			realtime_layer[i].layer_en, realtime_layer[i].buff_idx, realtime_layer[i].fmt,
...
#if DISPLAY_V4L2
int mtkfb_check_v4l2_cmd(unsigned int *cmd)
{
	int is_v4l2_cmd = 0;

	if (MTKFB_QUEUE_OVERLAY_CONFIG_V4L2 == *cmd) {
		*cmd = MTKFB_QUEUE_OVERLAY_CONFIG;
		is_v4l2_cmd = 1;
	}
...
void mtkfb_clear_lcm(void)
{
	...
	for (i = 0; i < DDP_OVL_LAYER_MUN; i++) {
		...
		/* TODO save status */
		{
			...
			layer_status[i] =
			    disp_ovl_engine.Instance[mtkfb_instance].
	...
	case MTKFB_QUEUE_OVERLAY_CONFIG:
		{
			...
			for (i = 0; i < HW_OVERLAY_COUNT; i++) {
				MTKFB_LOG("MTKFB_QUEUE_OVERLAY_CONFIG:\n");
				MTKFB_LOG("    fence:%d, time: 0x%x, layer%d en%d, next_idx=0x%x\n",
				...
				MTKFB_LOG("    fence_fd=%d, ion_fd=%d, security=%d\n",
					  config.layers[i].fence_fd, config.layers[i].ion_fd,
					  config.layers[i].security);
			}
#if DISPLAY_V4L2
			if (g_is_v4l2_active && 0 == from_v4l2) {
				config.fence = -1;
				r = 0;
			} else
#endif
...
#ifdef MTK_ANDROID_SUPPORT	/*ion is not ready */
#ifndef NONFLUSH
			/* Wait until queued compositions are applied */
			flush_workqueue(((struct mtkfb_device *)info->par)->update_ovls_wq);
#else
			/* Do not flush work queue, because this will cause fence disorder */
			/* disable all layer */
			{
				struct fb_overlay_config config;
				struct sync_fence *fence;
				...
				mtkfb_queue_overlay_config((struct mtkfb_device *)info->par,
							   &config);
				...
				if (config.fence != -1) {
					...
					if (sync_fence_wait(fence, 1000) < 0)
						pr_err
						    ("MTKFB_POWEROFF error waiting for fence to signal\n");
	...
	case MTKFB_META_RESTORE_SCREEN:
		{
			...
			MTKFB_LOG("[MTKFB] ioctl MTKFB_META_RESTORE_SCREEN\n");
...
/* ////////////////////////////////////////////// */
	default:
		pr_info("mtkfb_ioctl 0x%08x(%s) Not support", (unsigned int)cmd,
...
static int mtkfb_parse_tag_videolfb(void)
{
	...
	if (node) {
		tag_ptr = of_get_flat_dt_prop(node, "atag,videolfb", &size);
		if (tag_ptr) {
...
static int mtkfb_probe(struct device *dev)
{
	...
	if (!fbdev->layer_format) {
		pr_err("[mtkfb.c FB driver] vmalloc failed, %d\n", __LINE__);
		r = -ENOMEM;
		goto cleanup;
	}
	...
	if (0xFF == mtkfb_instance) {
		pr_err("allocate overlay engine fail\n");
		goto cleanup;
	}
	...
	if (r) {
		pr_err("mtkfb_register_sysfs fail\n");
		goto cleanup;
	}
	init_state++;		/* 5 */
