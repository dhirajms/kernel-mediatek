...
int m4u_query_mva(M4U_MODULE_ID_ENUM eModuleID,
		...
		unsigned int *pRetMVABuf, struct file *a_pstFile)
{
	return m4u_do_query_mva(eModuleID, BufAddr,
				BufSize, pRetMVABuf, a_pstFile);
}
...
int m4u_dealloc_mva(M4U_MODULE_ID_ENUM eModuleID,
		    const unsigned int BufAddr, const unsigned int BufSize,
...
{
	return m4u_do_insert_seq_range(eModuleID, MVAStart, MVAEnd, entryCount);
}

int m4u_invalid_seq_range(M4U_MODULE_ID_ENUM eModuleID,
	unsigned int MVAStart, unsigned int MVAEnd)
{
	m4u_invalid_seq_range_by_mva(m4u_module_2_m4u_id(eModuleID),
			MVAStart, MVAEnd);
	return 0;
...
int m4u_sync_nonsec_sec_pgtable(unsigned int mva, unsigned int size, int port)
{
	return m4u_do_sync_nonsec_sec_pgtable(mva, size, port);
}
...
int m4u_alloc_mva_fix(M4U_MODULE_ID_ENUM eModuleID,
		const unsigned int BufAddr,
		const unsigned int BufSize,
		int security, int cache_coherent,
		unsigned int *pRetMVABuf)
{
	/* m4u_fill_linear_pagetable(BufAddr, BufSize); */
	pr_err("m4u alloc mva fix null\n");
	return 0;
}
