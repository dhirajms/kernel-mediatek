...
/*@entryCount 0:disable multi-entry, 1,2,4,8,16: enable multi-entry*/
 ...
 * will check link list for mva rather than re-build pagetable
 * by get_user_pages(), if can not find the VA in link list,
 * will call m4u_alloc_mva() internally*/
 ...
 *TEE
 *only for ovl 4 layger (sync nonsec pt 2 sec pt)
 *if ovl config port NonSec to Sec, and also want to use a nonsec memory,
 *     please call this function before you use it.
 * return : 0 : successful
 *          -1: failed
 */
int m4u_sync_nonsec_sec_pgtable(unsigned int mva,
			unsigned int size,
			int port);
