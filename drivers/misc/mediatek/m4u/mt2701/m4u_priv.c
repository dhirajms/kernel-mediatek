...
/* #define M4U_PAGETABLE_ENHANCEMENT */

/* MTK_IN_HOUSE_TEE_SUPPORT -kk */
#ifdef CONFIG_MTK_IN_HOUSE_TEE_SUPPORT	/* aosp */
#define M4U_TEE_SERVICE_ENABLE
#endif

#ifdef M4U_TEE_SERVICE_ENABLE
static int m4u_alloc_mva_sec(mva_info_t *pMvaInfo);
static int m4u_dealloc_mva_sec(M4U_MODULE_ID_ENUM eModuleID,
			       const unsigned int BufAddr,
			       const unsigned int BufSize, const unsigned int MVAHandle);
static int m4u_tf_sec_dump(unsigned int m4u_index, unsigned int port, int fault_mva);
...
static mva_info_t gMvaNode_unknown = {
	.bufAddr = 0,
	.mvaStart = 0,
	.size = 0,
...
#define M4U_MVA_DBUG(string, args...) M4UMSG("[M4U_K][MVA]"string, ##args)
...
#define __M4U_MAP_MVA_TO_KERNEL_FOR_DEBUG__ 0
...
static int m4u_dealloc_mva_dynamic(M4U_MODULE_ID_ENUM eModuleID,
				   ...
				   unsigned int mvaRegionAddr, struct sg_table *sg_table);
static unsigned int m4u_do_mva_alloc(M4U_MODULE_ID_ENUM eModuleID,
				     const unsigned int BufAddr,
				     const unsigned int BufSize, mva_info_t *pMvaInfo);
static int m4u_do_mva_free(M4U_MODULE_ID_ENUM eModuleID,
			   const unsigned int BufAddr,
			   const unsigned int BufSize, unsigned int mvaRegionStart);
...
static int m4u_get_pages(M4U_MODULE_ID_ENUM eModuleID,
			 unsigned int BufAddr, unsigned int BufSize, unsigned int *pPageTableAddr);
static int m4u_get_pages_sg(M4U_MODULE_ID_ENUM eModuleID, unsigned int BufAddr,
			    unsigned int BufSize, struct sg_table *sg_table, unsigned int *pPhys);
...
static int m4u_manual_insert_entry(M4U_PORT_ID_ENUM eModuleID,
				   unsigned int EntryMVA, int secure_pagetable, int Lock);
static int m4u_add_to_garbage_list(struct file *a_pstFile, mva_info_t *pList);
static mva_info_t *m4u_delete_from_garbage_list(M4U_MOUDLE_STRUCT *p_m4u_module,
...
#ifdef M4U_PAGETABLE_ENHANCEMENT
static struct page *m4u_alloc_pages(void);
static void m4u_dma_cache_by_range(unsigned int BufAddr, unsigned int BufSize,
...
static unsigned int gM4UBaseAddr[TOTAL_M4U_NUM];
...
static unsigned int smi_port0_in_larbx[SMI_LARB_NR + 1] = { 0, 11, 21, 44 };
static unsigned int m4u_port0_in_larbx[SMI_LARB_NR + 1] = { 0, 11, 21, 44 };
...
static unsigned int ProtectPA;
static unsigned int *pProtectVA_nonCache;

/* unsigned int gM4U_align_page_va = 0; */
static unsigned int gM4U_align_page_pa;

/* #define BACKUP_REG_SIZE (M4U_REG_SIZE*TOTAL_M4U_NUM) */
#define BACKUP_REG_SIZE 640
static unsigned int *pM4URegBackUp;

static M4U_RANGE_DES_T *pRangeDes;
static M4U_WRAP_DES_T *pWrapDes;
#define RANGE_DES_ADDR 0x11

static int g_debug_make_translation_fault;
static int g_debug_print_detail_in_isr = 1;
static int g_debug_enable_error_hang;
static int g_debug_recover_pagetable_TF;
static int g_debug_dump_rs_in_isr;
...
/*static struct cdev *g_pMTKM4U_CharDrv;
...
typedef enum {
	...
	/* stricker verification ,may use M4UERR instead M4UMSG sometimes, used for our own internal test */
	M4U_TEST_LEVEL_STRESS = 2
} M4U_TEST_LEVEL_ENUM;
...
static int m4u_invalid_tlb(M4U_ID_ENUM m4u_id, int L2_en, int isInvAll, unsigned int mva_start,
			   unsigned int mva_end)
{
	...
	if (L2_en) {
		if (isInvAll) {
			...
			while (!(event =
				m4uHw_get_field_by_mask(0, REG_L2_GDC_STATE,
							F_L2_GDC_ST_EVENT_MSK)))
				;

			m4uHw_set_field_by_mask(0, REG_L2_GDC_STATE, F_L2_GDC_ST_EVENT_MSK, 0);
		} else {
			while (!(m4uHw_get_field_by_mask
				(0, REG_L2_GPE_STATUS, F_L2_GPE_ST_RANGE_INV_DONE)))
				;

			m4uHw_set_field_by_mask(0, REG_L2_GPE_STATUS, F_L2_GPE_ST_RANGE_INV_DONE,
						0);
		}
	}
...
static void m4u_invalid_tlb_by_range(M4U_ID_ENUM m4u_id,
				     int L2_en, unsigned int mva_start, unsigned int mva_end)
{
					 ...
					 int L2_en, unsigned int mva_start, unsigned int mva_end)
{
	...
	if (L2_en) {
		while (!(m4uHw_get_field_by_mask
			(0, REG_L2_GPE_STATUS_SEC, F_L2_GPE_ST_RANGE_INV_DONE_SEC)))
			;
...
static int MTK_M4U_release(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	if (pNode->isM4uDrvConstruct == 0 || pNode->isM4uDrvDeconstruct == 0) {
		M4UMSG
		    ("warning on close : construct = %d, deconstruct = %d, open_pid = %d, cur_pid = %d\n",
		     pNode->isM4uDrvConstruct, pNode->isM4uDrvDeconstruct, pNode->open_pid,
		     current->pid);
		M4UMSG("open->tgid = %d, cur->tgid = %d, cur->mm = 0x%x\n", pNode->open_tgid,
		       current->tgid, (unsigned int)current->mm);
	}
	...
	while (pListHead != &(pNode->mvaList)) {
		...
		M4UMSG
		    ("warnning : clean garbage at m4u close : module = %s, va = 0x%x, mva = 0x%x, size = %d\n",
		...
		if (pList->mvaStart != 0) {
			...
			m4u_reclaim_mva_callback_ovl(pList->eModuleId, pList->bufAddr, pList->size,
						     pList->mvaStart);
#endif
#endif

			ret =
			    m4u_dealloc_mva(pList->eModuleId, pList->bufAddr, pList->size,
					    pList->mvaStart);
			if (ret)
				m4u_free_garbage_list(pList);
		} else {
			if (!(pList->flags & MVA_REGION_REGISTER))
				M4UERR("warning : in garbage reclaim : mva == 0,");
				M4UERR("but MVA_REGION_REGISTER is not set !!flag = 0x%x\n", pList->flags);
		}
	}
...
static long MTK_M4U_ioctl(struct file *a_pstFile, unsigned int a_Command, unsigned long a_Param)
{
	int ret = 0;
	...
	case MTK_M4U_T_ALLOC_MVA:
		...
		if (m4u_module.MVAStart == -1) {	/* work around for wrap layer */
			...
			pList = m4u_alloc_garbage_list(0,
						       m4u_module.BufSize, m4u_module.eModuleID,
						       m4u_module.BufAddr, MVA_REGION_FLAG_NONE,
						       m4u_module.security,
		...
		ret =
		    copy_to_user(&(((M4U_MOUDLE_STRUCT *) a_Param)->MVAStart),
				 &(m4u_module.MVAStart), sizeof(unsigned int));
		if (ret) {
			m4u_module.MVAStart = 0;
			M4UMSG("MTK_M4U_T_ALLOC_MVA error, copy_to_user failed :  %d\n", ret);
			return -EFAULT;
		}
		break;
	...
	case MTK_M4U_T_QUERY_MVA:
		...
		ret = copy_from_user(&m4u_module, (void *)a_Param, sizeof(M4U_MOUDLE_STRUCT));
		if (ret) {
			M4UMSG("MTK_M4U_T_QUERY_MVA, copy_from_user failed :  %d\n", ret);
			return -EFAULT;
		}
		...
		m4u_query_mva(m4u_module.eModuleID,
			      m4u_module.BufAddr,
			      m4u_module.BufSize, &(m4u_module.MVAStart), a_pstFile);

		ret =
		    copy_to_user(&(((M4U_MOUDLE_STRUCT *) a_Param)->MVAStart),
				 &(m4u_module.MVAStart), sizeof(unsigned int));
		if (ret) {
			M4UMSG("MTK_M4U_T_QUERY_MVA, copy_to_user failed :  %d\n", ret);
			return -EFAULT;
		}
	...
	case MTK_M4U_T_DEALLOC_MVA:
		{
			...
			M4U_ASSERT(a_Param);
			ret =
			    copy_from_user(&m4u_module, (void *)a_Param, sizeof(M4U_MOUDLE_STRUCT));
			if (ret) {
			...
			M4UDBG
			    ("MTK_M4U_T_DEALLOC_MVA, eModuleID : %d, VABuf:0x%x, Length : %d, MVAStart = 0x%x \r\n",
			     m4u_module.eModuleID, m4u_module.BufAddr, m4u_module.BufSize,
			     m4u_module.MVAStart);
			...
			if (pMvaInfo == NULL) {
				M4UMSG
				    ("error to dealloc mva : id = %s, va = 0x%x, size = %d, mva = 0x%x\n",
				...
				if (m4u_module.MVAStart != 0) {
					...
					M4UMSG
					    ("error to dealloc mva : id = %s, va = 0x%x, size = %d, mva = 0x%x\n",
					     m4u_get_module_name(m4u_module.eModuleID),
	...
	case MTK_M4U_T_DEALLOC_MVA_SEC:
		{
			...
			M4UDBG("MTK_M4U_T_DEALLOC_MVA_SEC,");
			M4UDBG("eModuleID : %d, VABuf:0x%x, Length : %d, MVAStart = 0x%x \r\n",
				m4u_module.eModuleID, m4u_module.BufAddr, m4u_module.BufSize,
				m4u_module.MVAStart);
			...
			if (pMvaInfo == NULL) {
				...
				/* when deallocate, m4u_module.MVAStart==0, we think this is right. */
				if (m4u_module.MVAStart != 0) {
					m4u_dealloc_mva_sec(m4u_module.eModuleID,
							    m4u_module.BufAddr,
							    m4u_module.BufSize,
							    m4u_module.MVAStart);
				} else {
	...
	case MTK_M4U_T_MANUAL_INSERT_ENTRY:
		...
		ret = copy_from_user(&m4u_module, (void *)a_Param, sizeof(M4U_MOUDLE_STRUCT));
		if (ret) {
			M4UMSG("MTK_M4U_Manual_Insert_Entry, copy_from_user failed :  %d\n", ret);
			return -EFAULT;
		}
		...
		ret = m4u_manual_insert_entry(m4u_module.eModuleID,
					      m4u_module.EntryMVA,
					      !!(m4u_module.security), m4u_module.Lock);
		break;
	...
	case MTK_M4U_T_INSERT_TLB_RANGE:
		...
		ret = copy_from_user(&m4u_module, (void *)a_Param, sizeof(M4U_MOUDLE_STRUCT));
		if (ret) {
			M4UMSG("m4u_insert_seq_range , copy_from_user failed :  %d\n", ret);
			return -EFAULT;
		}
		M4UDBG
		    ("m4u_insert_seq_range , eModuleID : %d, MVAStart:0x%x, MVAEnd:0x%x, ePriority = %d \r\n",
		...
		ret = m4u_insert_seq_range(m4u_module.eModuleID,
					   m4u_module.MVAStart,
					   m4u_module.MVAEnd,
					   m4u_module.ePriority, m4u_module.entryCount);
		break;
	...
	case MTK_M4U_T_INVALID_TLB_RANGE:
		...
		M4UDBG
		    ("MTK_M4U_Invalid_TLB_Range(), eModuleID : %d, MVAStart = 0x%x, MVAEnd = 0x%x\n",
		     m4u_module.eModuleID, m4u_module.MVAStart, m4u_module.MVAEnd);
	...
	case MTK_M4U_T_CACHE_SYNC:
		...
		if (ret) {
			M4UMSG
			    ("MTK_M4U_T_CACHE_INVALID_AFTER_HW_WRITE_MEM, copy_from_user failed :  %d\n",
			     ret);
			return -EFAULT;
		}
		M4UDBG("MTK_M4U_T_CACHE_INVALID_AFTER_HW_WRITE_MEM(),");
		M4UDBG("moduleID = %d, eCacheSync = %d, buf_addr = 0x%x, buf_length = 0x%x\n",
			m4u_cache_data.eModuleID, m4u_cache_data.eCacheSync, m4u_cache_data.BufAddr,
			m4u_cache_data.BufSize);

		switch (m4u_cache_data.eCacheSync) {
		case M4U_CACHE_FLUSH_BEFORE_HW_WRITE_MEM:
		case M4U_CACHE_FLUSH_BEFORE_HW_READ_MEM:
			ret =
			    m4u_dma_cache_maint(m4u_cache_data.eModuleID,
						(unsigned int *)(m4u_cache_data.BufAddr),
						m4u_cache_data.BufSize, M4U_DMA_READ_WRITE);
			break;
		...
		case M4U_CACHE_CLEAN_BEFORE_HW_READ_MEM:
			ret =
			    m4u_dma_cache_maint(m4u_cache_data.eModuleID,
						(unsigned int *)(m4u_cache_data.BufAddr),
						m4u_cache_data.BufSize, M4U_DMA_READ);
			break;
		...
		case M4U_CACHE_INVALID_AFTER_HW_WRITE_MEM:
			ret =
			    m4u_dma_cache_maint(m4u_cache_data.eModuleID,
						(unsigned int *)(m4u_cache_data.BufAddr),
						m4u_cache_data.BufSize, M4U_DMA_WRITE);
			break;
		default:
			M4UMSG
	...
	case MTK_M4U_T_CONFIG_PORT:
		...
		M4UDBG
		    ("ePortID = %d, Virtuality = %d, Security = %d, Distance = %d, Direction = %d\n",
		     m4u_port.ePortID, m4u_port.Virtuality, m4u_port.Security, m4u_port.Distance,
		     m4u_port.Direction);
	...
	case MTK_M4U_T_CONFIG_PORT_ROTATOR:
		M4U_ASSERT(a_Param);
		ret =
		    copy_from_user(&m4u_port_rotator, (void *)a_Param,
				   sizeof(M4U_PORT_STRUCT_ROTATOR));
		if (ret) {
			M4UMSG("MTK_M4U_T_CONFIG_PORT_ROTATOR, copy_from_user failed :  %d\n",
			       ret);
			return -EFAULT;
		}
	...
	case MTK_M4U_T_INSERT_WRAP_RANGE:
		...
		if (ret) {
			M4UMSG("MTK_M4U_T_INSERT_WRAP_RANGE, copy_from_user failed :  %d\n", ret);
			return -EFAULT;
		}
		M4UDBG("PortID = %d, eModuleID = %d, MVAStart = 0x%x, MVAEnd = 0x%x\n",
		       m4u_wrap_range.ePortID,
		       m4u_wrap_range.eModuleID, m4u_wrap_range.MVAStart, m4u_wrap_range.MVAEnd);

		ret = m4u_insert_wrapped_range(m4u_wrap_range.eModuleID,
					       m4u_wrap_range.ePortID,
					       m4u_wrap_range.MVAStart, m4u_wrap_range.MVAEnd);
		break;
	...
	case MTK_M4U_T_RESET_MVA_RELEASE_TLB:
		...
		if (ret) {
			M4UMSG("MTK_M4U_T_RESET_MVA_RELEASE_TLB, copy_from_user failed :  %d\n",
			       ret);
			return -EFAULT;
		}
		break;
	...
	case MTK_M4U_T_DUMP_PAGETABLE:
		do {
			...
			M4UMSG("==  ==  ==  ==  = compare these automaticlly ==  ==  ==  =>\n");
			for (i = 0; i < page_num; i++) {
				pa = m4u_user_v2p(va + i * M4U_PAGE_SIZE);
				entry =
				    *(unsigned int *)mva_pteAddr_nonsec((mva + i * M4U_PAGE_SIZE));

				if ((pa & (~0xfff)) != (pa & (~0xfff))) {
					M4UMSG
					    ("warning warning !!va = 0x%x, mva = 0x%x, pa = 0x%x, entry = 0x%x\n",
					     va + i * M4U_PAGE_SIZE, mva + i * M4U_PAGE_SIZE, pa,
					     entry);
				}
			}
	...
	case MTK_M4U_T_REGISTER_BUFFER:
		{
			...
			pMvaInfo = kmalloc(sizeof(mva_info_t), GFP_KERNEL);
			if (pMvaInfo == NULL) {
				M4UERR("MTK_M4U_T_REGISTER_BUFFER()\n");
				return -EFAULT;
			}
			ret =
			    copy_from_user(&m4u_module, (void *)a_Param, sizeof(M4U_MOUDLE_STRUCT));
			...
			M4ULOG
				("-MTK_M4U_T_REGISTER_BUF, module_id = %d, BufAddr = 0x%x, BufSize = %d \r\n",
				m4u_module.eModuleID, m4u_module.BufAddr, m4u_module.BufSize);
...
static volatile int gM4u_L2_invalid_range_done;
static volatile int gM4u_L2_invalid_all_done;
...
static irqreturn_t MTK_M4U_isr(int irq, void *dev_id)
{
	...
	if (IntrSrc & F_INT_TRANSLATION_FAULT) {
		...
		{
			MMProfileLogEx(M4U_MMP_Events[PROFILE_M4U_ERROR], MMProfileFlagPulse,
				       m4u_port, faultMva);

			m4u_aee_print("\nCRDISPATCH_KEY : %s\ntranslation fault :", m4u_get_port_name(m4u_port));
			m4u_aee_print("larb = %d, module = %s, port = %s, mva = 0x%x\n",
				      larbID,
				      m4u_get_module_name(m4u_port_2_module(m4u_port)),
				      m4u_get_port_name(m4u_port), faultMva);
		}
		...
		if (faultMva < M4U_MVA_MAX - 0x1000 && faultMva > 0x40000) {
			...
			M4UMSG("pagetable @ 0x%x : 0x%x, 0x%x, 0x%x\n", faultMva, pteStart[-1],
		...
		/* search invalid main tlb */
		{
			...
			for (i = 0; i < M4U_MAIN_TLB_NR; i++) {
				...
				if ((mva & (F_MAIN_TLB_VALID_BIT | F_MAIN_TLB_INV_DES_BIT))
					...
					M4UMSG("%d:0x%x, 0x%x", i,
					       mva & F_MAIN_TLB_VA_MSK, des);
				}
			}
			/* printk(KERN_INFO"\n"); */
		}
		...
		/* search invalid pfh tlb */
		{
			...
			for (i = 0; i < M4U_MAIN_TLB_NR; i++) {
				...
				if ((tag_valid & des_invalid) != 0) {
					M4UMSG("%d:0x%x, 0x%x 0x%x 0x%x 0x%x", i, tag,
...
#ifdef M4U_TEE_SERVICE_ENABLE
		m4u_tf_sec_dump(m4u_index, m4u_port, faultMva);
#endif
	}
	...
	if (IntrSrc & F_INT_INVALID_PHYSICAL_ADDRESS_FAULT) {
		if (!(IntrSrc & F_INT_TRANSLATION_FAULT)) {
			...
			m4u_aee_print("invalid PA:0x%x->0x%x\n", faultMva,
				      M4U_ReadReg32(m4u_base, REG_MMU_INVLD_PA));
			m4u_dump_main_tlb_des(m4u_index);
			m4u_dump_pfh_tlb_des(m4u_index);
		} else
	...
	if (IntrSrc & F_INT_ENTRY_REPLACEMENT_FAULT) {
		...
		for (i = 0; i < g4M4UTagCount[m4u_index]; i++) {
			lock_cnt[mva2module
				 (M4U_ReadReg32(m4u_base, REG_MMU_MAIN_TAG(i)) & (~0xfff))]++;
		}
...
int m4u_parse_dt(struct platform_device *pdev, struct m4u_device *m4udev)
{
	struct resource *res;
	...
	m4udev->m4u_base[0] = devm_ioremap_resource(&pdev->dev, res);
	if (IS_ERR(m4udev->m4u_base[0]))
		return PTR_ERR(m4udev->m4u_base[0]);

	gM4UBaseAddr[0] = (unsigned long)m4udev->m4u_base[0];
	gM4UBaseAddr[1] = (unsigned long)m4udev->m4u_base[0];
	m4udev->m4u_base[1] = m4udev->m4u_base[0];

	m4udev->L2_base = m4udev->m4u_base[0] + 0X600;
	res = platform_get_resource(pdev, IORESOURCE_MEM, 1);
	m4udev->smi_common_ao = devm_ioremap_resource(&pdev->dev, res);
	if (IS_ERR(m4udev->smi_common_ao))
		return PTR_ERR(m4udev->smi_common_ao);
	...
	m4udev->irq_num[0] = irq_of_parse_and_map(node, 0);
...
static int m4u_probe(struct platform_device *pdev)
{
	...
	gM4uDev->m4u_dev_proc_entry = proc_create("M4U_device", 0, NULL, &g_stMTK_M4U_fops);
	if (!gM4uDev->m4u_dev_proc_entry) {
		M4UERR("proc m4u create error\n");
		return -ENODEV;
	}

	ret = m4u_parse_dt(pdev,  gM4uDev);
	if (ret) {
		M4UERR("parse  m4u dtsi error\n");
		return -ENODEV;
	}

	pmodule_current_size = kmalloc(M4U_CLIENT_MODULE_NUM * 4, GFP_KERNEL | __GFP_ZERO);
	pmodule_max_size = kmalloc(M4U_CLIENT_MODULE_NUM * 4, GFP_KERNEL | __GFP_ZERO);
	pmodule_locked_pages = kmalloc(M4U_CLIENT_MODULE_NUM * 4, GFP_KERNEL | __GFP_ZERO);
	...
	/* Set IRQ */
	if (request_irq(gM4uDev->irq_num[0], MTK_M4U_isr, IRQF_TRIGGER_LOW, M4U_DEVNAME, NULL)) {
	...
	M4uPort.Distance = 1;
	M4uPort.Direction = 0;
	ret = m4u_config_port(&M4uPort);
	if (ret)
		M4UMSG("m4u config port second failed\n");
	else
		M4UMSG("m4u config port success\n");
#endif
	return 0;
...
static int m4u_remove(struct platform_device *pdev)
{
	...
	/* Release IRQ */
	free_irq(gM4uDev->irq_num[0], NULL);
...
static int m4u_confirm_range_invalidated(int m4u_index, unsigned int MVAStart, unsigned int MVAEnd)
{
		...
		if (regval & (F_MAIN_TLB_VALID_BIT)) {
			...
			if (mva >= sa_align && mva <= ea_align) {
				if (gTestLevel == M4U_TEST_LEVEL_STRESS) {
					M4UERR("main : i = %d, idx = 0x%x, MVAStart = 0x%x",
						i, m4u_index, MVAStart);
					M4UERR("MVAEnd = 0x%x, RegValue = 0x%x\n",
						MVAEnd, regval);
					m4u_dump_reg(m4u_index);
				} else if (gTestLevel == M4U_TEST_LEVEL_ENG) {
					M4UMSG("main : i = %d, idx = 0x%x, MVAStart = 0x%x", i, m4u_index, MVAStart);
					M4UMSG("MVAEnd = 0x%x, RegValue = 0x%x\n",
					       MVAEnd, regval);
				}
				result = -1;
			}
	...
	for (i = 0; i < g4M4UTagCount[m4u_index]; i++) {
		...
		if (regval & F_PFH_TAG_VALID_MSK) {	/* /> a valid Prefetch TLB entry */
			...
			if (mva >= sa_align && mva <= ea_align) {
				if (gTestLevel == M4U_TEST_LEVEL_STRESS) {
					M4UERR("prefetch : i = %d, idx = 0x%x", i, m4u_index);
					M4UERR("MVAStart = 0x%x, MVAEnd = 0x%x, RegValue = 0x%x\n",
						MVAStart, MVAEnd, regval);
					m4u_dump_reg(m4u_index);
				} else if (gTestLevel == M4U_TEST_LEVEL_ENG) {
					M4UMSG("prefetch : i = %d, idx = 0x%x",  i, m4u_index);
					M4UMSG("MVAStart = 0x%x, MVAEnd = 0x%x, RegValue = 0x%x\n",
						MVAStart, MVAEnd, regval);
				}
				result = -1;
			}
...
static unsigned int m4u_get_main_descriptor(unsigned int m4u_base, unsigned int idx)
{
	...
	regValue = F_READ_ENTRY_TLB_SEL_MAIN | F_READ_ENTRY_INDEX_VAL(idx)
...
static unsigned int m4u_get_pfh_descriptor(unsigned int m4u_base, int tlbIndex, int tlbSelect)
{
	...
	regValue = F_READ_ENTRY_TLB_SEL_PFH | F_READ_ENTRY_INDEX_VAL(tlbIndex)
...
static int m4u_dump_pfh_tlb_tags(int m4u_id)
{
	...
	for (i = 0; i < M4U_PRE_TLB_NR; i++) {
		M4UINFO("0x%x ", M4U_ReadReg32(m4u_base, REG_MMU_PFH_TAG(i)));
		if ((i + 1) % 8 == 0)
			M4UINFO("\n");
	}
	M4UINFO("\n");
...
static int m4u_dump_pfh_tlb_des(int m4u_id)
{
	...
	M4UINFO("dump main tlb ==  ==  ==  =>\n");
	for (i = 0; i < M4U_PRE_TLB_NR; i++) {
		M4UINFO("%d:0x%x:0x%x, 0x%x, 0x%x, 0x%x ", i,
...
static int m4u_dump_pagetable(M4U_MODULE_ID_ENUM eModuleID)
{
	...
	M4UINFO("[M4U_K] dump pagetable by module :  %s, page_num = %d ==  ==  ==  ==  >\n",
...
/* this function may be called in ISR */
	...
	M4UINFO
	    ("[M4U_K]  dump pagetable by module done ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  ==  =  <\n");
...
static int m4u_dump_mva_info(void)
{
	...
	M4UINFO
	    ("mva_start   mva_end          va       size     block_end  module block_num sec snoop\n");
	for (index = 1; index < MVA_MAX_BLOCK_NR + 1; index += nr) {
		addr = index << MVA_BLOCK_SIZE_ORDER;
		nr = MVA_GET_NR(index);
		if (MVA_IS_BUSY(index)) {
			...
			M4UINFO
				("0x%-8x, 0x%-8x, 0x%-8x, 0x%-8x, 0x%-8x, %s, %d, %d, %d\n",
				pMvaInfo->mvaStart, pMvaInfo->mvaStart + pMvaInfo->size - 1,
				pMvaInfo->bufAddr, pMvaInfo->size, addr + nr * MVA_BLOCK_SIZE,
				m4u_get_module_name(pMvaInfo->eModuleId), nr, pMvaInfo->security,
				pMvaInfo->cache_coherent);
		} else {
...
{/*
	...
	clk_prepare_enable(gM4uDev->infrasmi); /*never close it*/
...
{
	clk_disable_unprepare(gM4uDev->infram4u);
...
static void m4u_mvaGraph_init(void)
{
	...
	mvaInfoGraph[0] = &gMvaNode_unknown;
	mvaGraph[1] = MVA_MAX_BLOCK_NR;
	mvaInfoGraph[1] = &gMvaNode_unknown;
	mvaGraph[MVA_MAX_BLOCK_NR] = MVA_MAX_BLOCK_NR;
	mvaInfoGraph[MVA_MAX_BLOCK_NR] = &gMvaNode_unknown;
				     ...
				     const unsigned int BufSize, mva_info_t *pMvaInfo)
{
	...
	if (BufSize == 0)
		return 0;
	M4UMSG("m4u_do_mva_alloc, module id is %d, bufaddr is 0x%x, size is 0x%x\n",
		eModuleID, BufAddr, BufSize);
	MMProfileLogEx(M4U_MMP_Events[PROFILE_ALLOC_MVA_REGION], MMProfileFlagStart, eModuleID,
		       BufAddr);
	/* /----------------------------------------------------- */
	...
	MMProfileLogEx(M4U_MMP_Events[PROFILE_ALLOC_MVA_REGION], MMProfileFlagEnd, eModuleID,
		       BufSize);
			   ...
			   const unsigned int BufSize, unsigned int mvaRegionStart)
{
	...
	if (!(startIdx != 0	/* startIdx is not NULL */
	      ...
	      && (mvaInfoGraph[startIdx]->eModuleId == eModuleID)
	      && (nr == nrRequire)
	    )
	    ) {
		...
		M4UMSG("ModuleID = %s (expect %s) [%s]\n",
		       ...
		       m4u_get_module_name(mvaInfoGraph[startIdx]->eModuleId),
		       RightWrong(eModuleID == (mvaInfoGraph[startIdx]->eModuleId)));
		M4UMSG("BufSize = %d(unit:0x%xBytes) (expect %d) [%s]\n", nrRequire,
		       MVA_BLOCK_SIZE, nr, RightWrong(nrRequire == nr));
		M4UMSG("mva = 0x%x, (IsBusy ?) = %d (expect %d) [%s]\n", mvaRegionStart,
		       MVA_IS_BUSY(startIdx), 1, RightWrong(MVA_IS_BUSY(startIdx)));
		m4u_mvaGraph_dump();
...
#if  __M4U_MAP_MVA_TO_KERNEL_FOR_DEBUG__
	/* unmap kernel va for debug */
	{
		if (pMvaInfo->mapped_kernel_va_for_debug) {
			M4UMSG
			    ("[kernel_va_debug] unmap va : mva = 0x%x, kernel_va = 0x%x, size = 0x%x\n",
			     pMvaInfo->mvaStart, pMvaInfo->mapped_kernel_va_for_debug,
			     pMvaInfo->size);
			m4u_do_mva_unmap_kernel(pMvaInfo->mvaStart, pMvaInfo->size,
...
{/*
	...
	M4U_MMP_Events[PROFILE_ALLOC_MVA_REGION] =
	    MMProfileRegisterEvent(M4U_MMP_Events[PROFILE_ALLOC_MVA], "Alloc MVA Region");
	M4U_MMP_Events[PROFILE_GET_PAGES] =
	    MMProfileRegisterEvent(M4U_MMP_Events[PROFILE_ALLOC_MVA], "Get Pages");
	M4U_MMP_Events[PROFILE_FOLLOW_PAGE] =
	    MMProfileRegisterEvent(M4U_MMP_Events[PROFILE_GET_PAGES], "Follow Page");
	M4U_MMP_Events[PROFILE_FORCE_PAGING] =
	    MMProfileRegisterEvent(M4U_MMP_Events[PROFILE_GET_PAGES], "Force Paging");
	M4U_MMP_Events[PROFILE_MLOCK] =
	    MMProfileRegisterEvent(M4U_MMP_Events[PROFILE_GET_PAGES], "MLock");
	M4U_MMP_Events[PROFILE_ALLOC_FLUSH_TLB] =
	    MMProfileRegisterEvent(M4U_MMP_Events[PROFILE_ALLOC_MVA], "Alloc Flush TLB");
	M4U_MMP_Events[PROFILE_DEALLOC_MVA] = MMProfileRegisterEvent(M4U_Event, "DeAlloc MVA");
	M4U_MMP_Events[PROFILE_RELEASE_PAGES] =
	    MMProfileRegisterEvent(M4U_MMP_Events[PROFILE_DEALLOC_MVA], "Release Pages");
	M4U_MMP_Events[PROFILE_MUNLOCK] =
	    MMProfileRegisterEvent(M4U_MMP_Events[PROFILE_RELEASE_PAGES], "MUnLock");
	M4U_MMP_Events[PROFILE_PUT_PAGE] =
	    MMProfileRegisterEvent(M4U_MMP_Events[PROFILE_RELEASE_PAGES], "Put Page");
	M4U_MMP_Events[PROFILE_RELEASE_MVA_REGION] =
	    MMProfileRegisterEvent(M4U_MMP_Events[PROFILE_DEALLOC_MVA], "Release MVA Region");
	...
	M4U_MMP_Events[PROFILE_DMA_CLEAN_RANGE] =
	    MMProfileRegisterEvent(M4U_MMP_Events[PROFILE_DMA_MAINT_ALL], "Clean Range");
	M4U_MMP_Events[PROFILE_DMA_CLEAN_ALL] =
	    MMProfileRegisterEvent(M4U_MMP_Events[PROFILE_DMA_MAINT_ALL], "Clean All");
	M4U_MMP_Events[PROFILE_DMA_INVALID_RANGE] =
	    MMProfileRegisterEvent(M4U_MMP_Events[PROFILE_DMA_MAINT_ALL], "Invalid Range");
	M4U_MMP_Events[PROFILE_DMA_INVALID_ALL] =
	    MMProfileRegisterEvent(M4U_MMP_Events[PROFILE_DMA_MAINT_ALL], "Invalid All");
	M4U_MMP_Events[PROFILE_DMA_FLUSH_RANGE] =
	    MMProfileRegisterEvent(M4U_MMP_Events[PROFILE_DMA_MAINT_ALL], "Flush Range");
	M4U_MMP_Events[PROFILE_DMA_FLUSH_ALL] =
	    MMProfileRegisterEvent(M4U_MMP_Events[PROFILE_DMA_MAINT_ALL], "Flush All");
	M4U_MMP_Events[PROFILE_CACHE_FLUSH_ALL] =
	    MMProfileRegisterEvent(M4U_Event, "Cache Flush All");
	M4U_MMP_Events[PROFILE_CONFIG_PORT] = MMProfileRegisterEvent(M4U_Event, "Config Port");
	M4U_MMP_Events[PROFILE_MAIN_TLB_MON] =
	    MMProfileRegisterEvent(M4U_Event, "Main TLB Monitor");
	M4U_MMP_Events[PROFILE_PREF_TLB_MON] =
	    MMProfileRegisterEvent(M4U_Event, "PreFetch TLB Monitor");
...
int m4u_do_query_mva(M4U_MODULE_ID_ENUM eModuleID,
		     const unsigned int BufAddr,
		     const unsigned int BufSize, unsigned int *pRetMVABuf, struct file *a_pstFile)
{
	...
	*pRetMVABuf = 0;
	...
	list_for_each(pListHead, &(pNode->mvaList)) {
		...
		if ((pList->eModuleId == eModuleID) && (query_start >= s && query_end <= e)) {
			if (pList->mvaStart > 0) {	/* here we have allocated mva for this buffer */
				*pRetMVABuf = pList->mvaStart + (query_start - s);
				/* here we havn't allocated mva (this buffer is registered and query for first time) */
			} else {
				mva_info_t *pMvaInfo;
				...
				pMvaInfo = m4u_alloc_garbage_list(pList->mvaStart, pList->size,
								  pList->eModuleId, pList->bufAddr,
								  pList->flags, pList->security,
								  pList->cache_coherent);
				...
				M4ULOG
				    ("allocate for first query : id = %s, addr = 0x%08x, size = %d, mva = 0x%x\n",
...
static int m4u_invalidate_and_check(unsigned int m4u_index, unsigned int start, unsigned int end)
{
	...
	/* first time fail, invalidate range again */
	if (0 != m4u_confirm_range_invalidated(m4u_index, start, end)) {
		m4u_invalid_tlb_by_range(m4u_index, gM4U_L2_enable, start, end);
		/* again failed, invalidate all */
		if (0 != m4u_confirm_range_invalidated(m4u_index, start, end)) {
			M4UMSG("invalidate range twice, also fail !\n");
			m4u_invalid_tlb_all(m4u_index, gM4U_L2_enable);
			/* invalidate all failed, die */
			if (0 != m4u_confirm_range_invalidated(m4u_index, start, end))
				   ...
				   unsigned int mvaRegionAddr, struct sg_table *sg_table)
{
	...
	MMProfileLogEx(M4U_MMP_Events[PROFILE_RELEASE_PAGES], MMProfileFlagStart, eModuleID,
		       BufAddr);
	...
	/* fill pagetable with 0 */
	{
		...
		pteNr =
		    ((BufSize + (BufAddr & 0xfff)) / DEFAULT_PAGE_SIZE) +
		    (((BufAddr + BufSize) & 0xfff) != 0);
		pteNr += align_page_num;
		memset((void *)pteStart, 0, pteNr << 2);


		spin_lock(&gM4u_reg_lock);
		m4u_invalidate_and_check(m4u_module_2_m4u_id(eModuleID), mvaRegionAddr,
					 mvaRegionAddr + BufSize + align_page_num * 0x1000 - 1);
...
#ifdef M4U_PAGETABLE_ENHANCEMENT
	/* m4u_dma_cache_flush_all(); */
	m4u_dma_cache_by_range(BufAddr, BufSize, mvaRegionAddr);
#endif

	MMProfileLogEx(M4U_MMP_Events[PROFILE_RELEASE_MVA_REGION], MMProfileFlagStart, eModuleID,
		       BufAddr);

	{
		ret =
		    m4u_do_mva_free(eModuleID, BufAddr, BufSize + align_page_num * 0x1000,
				    mvaRegionAddr);
	}

	MMProfileLogEx(M4U_MMP_Events[PROFILE_RELEASE_MVA_REGION], MMProfileFlagEnd, eModuleID,
		       BufSize);
...
int m4u_fill_linear_pagetable(unsigned int pa, unsigned int size)
{
	...
	M4UDBG("m4u_fill_linear_pagetable, pa = 0x%08x, size = 0x%08x\n", pa, size);
			      ...
			      struct sg_table *sg_table)
{
	...
	page_num = M4U_GET_PAGE_NUM(BufAddr, BufSize);
	pPhys = vmalloc(page_num * sizeof(unsigned int *));
	if (pPhys == NULL) {
		MMProfileLogEx(M4U_MMP_Events[PROFILE_GET_PAGES], MMProfileFlagEnd, eModuleID,
			       BufSize);
	...
	MMProfileLogEx(M4U_MMP_Events[PROFILE_ALLOC_FLUSH_TLB], MMProfileFlagStart, eModuleID,
		       BufAddr);
	spin_lock(&gM4u_reg_lock);
	m4u_invalidate_and_check(m4u_module_2_m4u_id(eModuleID), mvaStart, mvaStart + BufSize - 1);
	spin_unlock(&gM4u_reg_lock);
	MMProfileLogEx(M4U_MMP_Events[PROFILE_ALLOC_FLUSH_TLB], MMProfileFlagEnd, eModuleID,
		       BufSize);
...
int __m4u_alloc_mva(mva_info_t *pMvaInfo, struct sg_table *sg_table)
{
	...
	unsigned int entry_flag =
	    F_DESC_VALID | F_DESC_NONSEC(!security) | F_DESC_SHARE(!!cache_coherent);
	...
	mvaStart =
	    m4u_do_mva_alloc(eModuleID, BufAddr, BufSize + align_page_num * 0x1000, pMvaInfo);
	if (mvaStart == 0) {
		M4UMSG("alloc mva fail : larb = %d, module = %s, size = %d\n",
			      m4u_module_2_larb(eModuleID), m4u_get_module_name(eModuleID),
			      BufSize);
...
#ifdef M4U_PAGETABLE_ENHANCEMENT
	/* m4u_dma_cache_flush_all(); */
	m4u_dma_cache_by_range(BufAddr, BufSize, mvaStart);
#endif
	...
	M4UINFO
	    ("alloc_mva_dynamic : id = %s, addr = 0x%08x, size = %d, sec = %d, mva = 0x%x, mva_end = 0x%x\n",
	     m4u_get_module_name(eModuleID), BufAddr, BufSize, security, pMvaInfo->mvaStart,
	     pMvaInfo->mvaStart + BufSize - 1);
...
#if __M4U_MAP_MVA_TO_KERNEL_FOR_DEBUG__
	{
		...
		ret = m4u_do_mva_map_kernel(mvaStart, BufSize, security, &kernel_va, &kernel_size);
		if (ret)
			...
			M4UMSG
			    ("[kernel_va_debug] map va : mva = 0x%x, kernel_va = 0x%x, size = 0x%x\n",
			     mvaStart, kernel_va, BufSize);
		}
	}
#endif
		      ...
		      const unsigned int BufSize, const unsigned int MVA, struct sg_table *sg_table)
{
	...
	M4UINFO
	    ("m4u_dealloc_mva, module = %s, addr = 0x%x, size = 0x%x, MVA = 0x%x, mva_end = 0x%x\n",
	...
	{
		if (m4u_invalid_seq_range_by_mva(m4u_module_2_m4u_id(eModuleID),
						 MVA, MVA + BufSize - 1) == 0) {
			M4UMSG("warning : dealloc mva without invalid tlb range !!id = %s",
				m4u_get_module_name(eModuleID));
			M4UMSG("add = 0x%x, size = 0x%x, mva = 0x%x\n",
				BufAddr, BufSize, MVA);
		}
	}
				   ...
				   unsigned int EntryMVA, int secure_pagetable, int Lock)
{
	...
	M4UDBG
	    ("m4u_manual_insert_entry, module : %s, EntryMVA:0x%x, secure : %d, Lock : %d \r\n",
...
int m4u_do_insert_seq_range(M4U_PORT_ID_ENUM eModuleID,
			    unsigned int MVAStart, unsigned int MVAEnd, unsigned int entryCount)
{
	...
	if (entryCount != 1 && entryCount != 2 && entryCount != 4 && entryCount != 8
	    && entryCount != 16)
		entryCount = 1;
	...
	if (mva2module(MVAStart) >= M4U_PORT_UNKNOWN) {
		m4u_aee_print("insert range fail : larb = %d, module = %s\n",
			      m4u_module_2_larb(eModuleID), m4u_get_module_name(eModuleID));
		M4UMSG
		    ("m4u_insert_seq_range module = %s, MVAStart = 0x%x is %s, MVAEnd = 0x%x is %s\n",
		     m4u_get_module_name(eModuleID), MVAStart,
		     m4u_get_module_name(mva2module(MVAStart)), MVAEnd,
		     m4u_get_module_name(mva2module(MVAEnd)));
		m4u_mvaGraph_dump();
	}

	M4ULOG
	    ("m4u_insert_seq_range , module : %s, MVAStart:0x%x, MVAEnd:0x%x, entryCount = %d \r\n",
	...
	if (FreeSEQRegs[m4u_index] == 0) {
		...
		for (i = 0; i < TOTAL_RANGE_NUM; i++) {
			if (1 == pRangeDes[i].Enabled) {
				M4UMSG
				    ("pRangeDes[%d] : Enabled = %d, module = %s, MVAStart = 0x%x, MVAEnd = 0x%x\n",
				     i, pRangeDes[i].Enabled,
				     m4u_get_module_name(pRangeDes[i].eModuleID),
		...
		if (1 == pRangeDes[i].Enabled) {
			if (MVAEnd < pRangeDes[i].MVAStart || MVAStart > pRangeDes[i].MVAEnd) {	/* no overlap */
				...
				M4UMSG("insert range overlap !: larb = %d, module = %s\n",
				       m4u_module_2_larb(eModuleID),
				       m4u_get_module_name(eModuleID));

				M4UMSG("error : insert tlb range is overlapped with previous ranges,");
				M4UMSG("current process = %s, !\n", current->comm);
				...
				M4UMSG
				    ("overlapped range id = %d, module = %s, mva_start = 0x%x, mva_end = 0x%x\n",
				     i, m4u_get_module_name(pRangeDes[i].eModuleID),
				     pRangeDes[i].MVAStart, pRangeDes[i].MVAEnd);
				mutex_unlock(&gM4uMutex);
				return 0;
			}
...
/* ======================================== */
	...
	if (RangeReg_ID == M4U_INVALID_ID) {
		...
		mutex_unlock(&gM4uMutex);
		/* do not have to return erro, nothing will happen if insert tlb range fails */
		return 0;
	}
/* ====================================================== */
	...
	{
		M4U_POW_ON_TRY(eModuleID);
		M4U_WriteReg32(m4u_base, REG_MMU_SQ_START(RangeReg_ID - m4u_index_offset),
			       MVAStart);
	...
	if (FreeSEQRegs[m4u_index] < SEQ_RANGE_NUM) {
		for (i = m4u_index_offset; i < m4u_index_offset + SEQ_RANGE_NUM; i++) {
			if (pRangeDes[i].Enabled == 1 &&
			    pRangeDes[i].MVAStart >= MVAStart && pRangeDes[i].MVAEnd <= MVAEnd) {
				pRangeDes[i].Enabled = 0;
...
KREE_SESSION_HANDLE m4u_session = NULL;
...
static int smi_reg_backup_sec(void)
{
	MTEEC_PARAM param[4];
	uint32_t paramTypes;
	TZ_RESULT ret;

	/* enable_mux(MT_MUX_MM, "m4u");for REG_SMI_SECUR_CON(i) */
...
static int smi_reg_restore_sec(void)
{
	MTEEC_PARAM param[4];
	uint32_t paramTypes;
	TZ_RESULT ret;
...
{				/* native */
	...
	M4ULOG("config_port-sec port = %s Vir = %d Sec = %d Dis = %d Dir = %d\n",
	       m4u_get_port_name(pM4uPort->ePortID), pM4uPort->Virtuality,
...
static int m4u_sec_init(unsigned int u4NonSecPa,	/* VA */
...
int m4u_do_sync_nonsec_sec_pgtable(unsigned int mva, unsigned int size, int port)
{
	MTEEC_PARAM param[4];
	uint32_t paramTypes;
	TZ_RESULT ret;
	...
	param[0].value.a = mva;
	param[0].value.b = size;
	param[1].value.a = port;
	param[1].value.b = FALSE;	/* fgDumpSecPGT only for debug */

	M4ULOG("map NonSec pgt 0x%x Sz 0x%x\n", mva, size);

	paramTypes = TZ_ParamTypes2(TZPT_VALUE_INPUT, TZPT_VALUE_INPUT);

	ret = KREE_TeeServiceCall(m4u_session, M4U_TZCMD_MAP_NONSEC_BUF, paramTypes, param);

	if (ret != TZ_RESULT_SUCCESS) {
		M4UMSG("m4u sec map error 0x%x\n", ret);
		return -1;
	}
...
static int m4u_tf_sec_dump(unsigned int m4u_index, unsigned int port, int fault_mva)
{
	...
	param[0].value.a = m4u_index;
	param[1].value.a = port;
	param[1].value.b = fault_mva;
	...
	if (ret != TZ_RESULT_SUCCESS) {
		M4UMSG("m4u sec info error 0x%x\n", ret);
		return -1;
	}
...
static int m4u_alloc_mva_sec(mva_info_t *pMvaInfo)
{
	...
	param[0].value.a = pMvaInfo->bufAddr;
	param[1].value.a = pMvaInfo->eModuleId;
	param[1].value.b = pMvaInfo->cache_coherent;

	paramTypes = TZ_ParamTypes3(TZPT_VALUE_INPUT, TZPT_VALUE_INPUT, TZPT_VALUE_OUTPUT);

	ret = KREE_TeeServiceCall(m4u_session, M4U_TZCMD_ALLOC_MVA_SEC, paramTypes, param);

	pMvaInfo->mvaStart = param[2].value.a;

	if (ret != TZ_RESULT_SUCCESS) {
		M4UMSG("m4u alloc mva sec tz error 0x%x\n", ret);
		return -1;
	}
			       ...
			       const unsigned int BufSize, const unsigned int MVAHandle)
{
	...
	param[0].value.a = eModuleID;
	param[0].value.b = MVAHandle;
	param[1].value.a = BufAddr;
	param[1].value.b = BufSize;

	paramTypes = TZ_ParamTypes2(TZPT_VALUE_INPUT, TZPT_VALUE_INPUT);

	ret = KREE_TeeServiceCall(m4u_session, M4U_TZCMD_DEALLOC_MVA_SEC, paramTypes, param);

	if (ret != TZ_RESULT_SUCCESS) {
		M4UMSG("m4u dealloc mva sec tz error 0x%x\n", ret);
		return -1;
	}
	return 0;
}
...
{				/* native */
	...
	M4UMSG("enter config port\n");
	if (PortID >= M4U_PORT_UNKNOWN) {
		M4UMSG("m4u portid error %d(max : %d)\n", PortID, M4U_PORT_UNKNOWN);
		return 0;
	}
	...
	M4UMSG
	    ("m4u_config_port(), port = %s, Virtuality = %d, Security = %d, Distance = %d, Direction = %d\n",
	     m4u_get_port_name(pM4uPort->ePortID), pM4uPort->Virtuality, pM4uPort->Security,
	     pM4uPort->Distance, pM4uPort->Direction);

	MMProfileLogEx(M4U_MMP_Events[PROFILE_CONFIG_PORT], MMProfileFlagStart, eModuleID,
		       pM4uPort->ePortID);
	...
	m4uHw_set_field_by_mask(m4u_base, REG_MMU_PFH_DIR(PortID),
				F_MMU_PFH_DIR(PortID, 1), F_MMU_PFH_DIR(PortID,
									pM4uPort->Direction));

	/* Distance
	if (pM4uPort->Distance > 16) {
		M4UMSG("m4u_config_port() error, port = %s,"
		       " Virtuality = %d, Security = %d, Distance = %d, Direction = %d\n",
		       m4u_get_port_name(pM4uPort->ePortID), pM4uPort->Virtuality,
		       pM4uPort->Security, pM4uPort->Distance, pM4uPort->Direction);
	}*/

	m4uHw_set_field_by_mask(m4u_base, REG_MMU_PFH_DIST(PortID),
				F_MMU_PFH_DIST_MASK(PortID), F_MMU_PFH_DIST_VAL(PortID,
										pM4uPort->Distance));
	...
	MMProfileLogEx(M4U_MMP_Events[PROFILE_CONFIG_PORT], MMProfileFlagEnd, pM4uPort->Virtuality,
		       pM4uPort->ePortID);
	M4UMSG("out config port\n");
	return 0;
...
static void m4u_get_perf_counter(int m4u_index, M4U_PERF_COUNT *pM4U_perf_count)
{
	...
	pM4U_perf_count->transaction_cnt = M4U_ReadReg32(m4u_base, REG_MMU_ACC_CNT);
	/* /> Main TLB miss count */
	pM4U_perf_count->main_tlb_miss_cnt = M4U_ReadReg32(m4u_base, REG_MMU_MAIN_MSCNT);
...
int m4u_do_monitor_stop(int m4u_id)
{
	...
	MMProfileLogEx(M4U_MMP_Events[PROFILE_MAIN_TLB_MON], MMProfileFlagStart,
		       (unsigned int)m4u_id, cnt.transaction_cnt);
	MMProfileLogEx(M4U_MMP_Events[PROFILE_PREF_TLB_MON], MMProfileFlagStart,
		       (unsigned int)m4u_id, cnt.pfh_cnt);
	MMProfileLogEx(M4U_MMP_Events[PROFILE_MAIN_TLB_MON], MMProfileFlagEnd,
		       (unsigned int)m4u_id, cnt.main_tlb_miss_cnt);
	MMProfileLogEx(M4U_MMP_Events[PROFILE_PREF_TLB_MON], MMProfileFlagEnd,
		       (unsigned int)m4u_id, cnt.pfh_tlb_miss_cnt);
...
static int m4u_put_unlock_page(struct page *page)
{
	...
	MMProfileLogEx(M4U_MMP_Events[PROFILE_MUNLOCK], MMProfileFlagStart, 0,
		       (unsigned int)(pfn << 12));
	...
	if (pMlock_cnt[pfn]) {
		...
		if (pMlock_cnt[pfn] == 0) {
			...
			if (PageMlocked(page) == 1) {
				...
				dump_page(page, "Can't munloc page");
			}
		}
		put_page(page);
	} else {
...
static int m4u_get_pages(M4U_MODULE_ID_ENUM eModuleID, unsigned int BufAddr,
			 unsigned int BufSize, unsigned int *pPhys)
{
	int ret, i;
	int page_num;
	...
	if (M4U_CLNTMOD_LCDC_UI == eModuleID) {
		...
		/*if (is_pmem_range((unsigned long *)start_pa, BufSize)) {
			M4UMSG
			    ("warning: m4u_get_pages virtual addr from pmem! start_pa=0x%x\n",
			     start_pa);
			for (i = 0; i < page_num; i++) {
				*(pPhys + i) =
				    m4u_user_v2p((BufAddr & 0xfffff000) + i * DEFAULT_PAGE_SIZE);
			}
		} else */{
			if (BufSize > MAX_BUF_SIZE_TO_GET_USER_PAGE) {
				m4u_aee_print("alloc mva fail: larb=%d,module=%s,size=%d\n",
					      m4u_module_2_larb(eModuleID),
					      m4u_get_module_name(eModuleID), BufSize);
				M4UMSG
				    ("m4u_get_pages(), single time alloc size=0x%x, bigger than limit=0x%x\n",
			...
			if ((vma->vm_flags) & VM_PFNMAP) {
				...
				if (bufEnd > vma->vm_end) {
					M4UMSG
					    ("error: page_num=%d,module=%s, va=0x%x, size=0x%x, vm_flag=0x%x\n",
				...
				M4UINFO
				    ("alloc_mva VM_PFNMAP module=%s, va=0x%x, size=0x%x, vm_flag=0x%x\n",
				     m4u_get_module_name(eModuleID), BufAddr, BufSize,
				...
				ret =
				    m4u_get_user_pages(eModuleID, current, current->mm,
							BufAddr, page_num, write_mode,
							/* m4u_get_write_mode_by_module(eModuleID),*/
							0, (struct page **)pPhys, NULL);
				...
				for (i = 0; i < page_num; i++) {
					*(pPhys + i) =
					    page_to_phys((struct page *)(*(pPhys + i))) | 0x20;
				}
			}
		}
		...
		if (BufAddr >= VMALLOC_START && BufAddr <= VMALLOC_END) {	/* vmalloc */
			...
			for (i = 0; i < page_num; i++)
				*(pPhys + i) =
				    virt_to_phys((void *)((BufAddr & 0xfffff000) +
							  i * DEFAULT_PAGE_SIZE));
		}

		M4UDBG
		    ("\n [kernel verify] BufAddr_sv=0x%x, BufAddr_sp=0x%x, BufAddr_ev=0x%x, BufAddr_ep=0x%x\n",
		     BufAddr, virt_to_phys((void *)BufAddr), BufAddr + BufSize - 1,
	...
	if (BufAddr < PAGE_OFFSET && sg_table == NULL) {	/* from user space */
		...
		for (i = 0; i < page_num; i++) {
			...
			if ((start_pa & 0x02) == 0)
				...
				/*
				 * we should check page count before call put_page,
				 * because m4u_release_pages() may fail in the middle of buffer
				 * that is to say, first several pages may be put successfully
				 * in m4u_release_pages().
				 */
				if (page_count(page) > 0) {
					/* to avoid too much log, we only save tha last err here. */
					if (m4u_put_unlock_page(page)) {
						M4UMSG("warning: put_unlock_page fail module=%s, va=0x%x",
							m4u_get_module_name(eModuleID), BufAddr);
						M4UMSG("size=0x%x,mva=0x%x (page is unlocked before put page)\n",
							BufSize, MVA);
						M4UMSG("i=%d (%d)\n", i, page_num);
					}
				} else {
					M4UMSG("page_count is 0: pfn=%d\n", (int)page_to_pfn(page));
					dump_page(page, "page release error");
				}
		...
		if (put_page_err == M4U_ERR_PAGE_UNLOCKED) {
			M4UMSG("warning: in m4u_release_page: module=%s",
				m4u_get_module_name(eModuleID));
			M4UMSG("va=0x%x, size=0x%x,mva=0x%x (page is unlocked before put page)\n",
				BufAddr, BufSize, MVA);
		}
	}
...
RELEASE_FINISH:
	...
	if (pmodule_current_size[eModuleID] < BufSize) {
		...
		M4UMSG
		    ("error pmodule_current_size is less than BufSize, module=%s, current_size=%d, BufSize=%d\n",
...
int m4u_do_dma_cache_maint(M4U_MODULE_ID_ENUM eModuleID, const void *start, size_t size,
			   int direction)
{
...
/* void (*outer_op)(unsigned long, unsigned long); */
	...
	if (0 == start) {
		m4u_aee_print("cache sync fail!: larb=%d,module=%s\n",
			      m4u_module_2_larb(eModuleID), m4u_get_module_name(eModuleID));
		M4UMSG
		    ("m4u_dma_cache_maint():  module=%s, start=0x%x, size=%d, direction=%d\n",
		     m4u_get_module_name(eModuleID), (unsigned int)start, size, direction);
		return -1;
	}
	...
	if (m4u_get_dir_by_module(eModuleID) == M4U_DMA_WRITE &&
		...
		if (1) {	/* screen red in debug mode */
			m4u_aee_print
			    ("Buffer align error: larb=%d,module=%s,addr=0x%x,size=%d,align=%d\n",
			     m4u_module_2_larb(eModuleID), m4u_get_module_name(eModuleID),
			     (unsigned int)start, size, L1_CACHE_BYTES);
			M4UMSG
			    ("error: addr un-align, module=%s, addr=0x%x, size=0x%x, process=%s, align=0x%x\n",
			     m4u_get_module_name(eModuleID), (unsigned int)start, size,
			     current->comm, L1_CACHE_BYTES);
		} else {
			M4UMSG
			    ("error: addr un-align, module=%s, addr=0x%x, size=0x%x, process=%s, align=0x%x\n",
			     m4u_get_module_name(eModuleID), (unsigned int)start, size,
	...
	case DMA_FROM_DEVICE:	/* invalidate only, HW write to memory */
		/*
		 * M4UMSG("error: someone call cache maint with DMA_FROM_DEVICE,
		 * module=%s\n",m4u_get_module_name(eModuleID));
		 */
...
/* < check wether input buffer is valid (has physical pages allocated) */
	...
	if (size < BUFFER_SIZE_FOR_FLUSH_ALL) {
		pPhy = kmalloc_array(page_num, sizeof(int), GFP_KERNEL);
		if (pPhy == NULL) {
			M4UMSG
			    ("error to kmalloc in m4u_cache_maint: module=%s, start=0x%x, size=%d, direction=%d\n",
			     m4u_get_module_name(eModuleID), (unsigned int)start, size, direction);
			goto out;
		}
		...
		if ((unsigned int)start < PAGE_OFFSET) {	/* from user space */
			for (i = 0; i < page_num; i++, page_start += DEFAULT_PAGE_SIZE) {
				...
				if ((pPhy[i] == 0)) {	/* || (!PageMlocked(page))) */
					ret = -1;
					M4UMSG("error: cache_maint() fail, ");
					M4UMSG("module=%s, start=0x%x, page_start=0x%x, size=%d, pPhy[i]=0x%x\n",
						m4u_get_module_name(eModuleID),
						(unsigned int)start, (unsigned int)page_start,
						size, pPhy[i]);
					/* dump_page(page); */
			...
			/* vmalloc */
		} else if ((unsigned int)start >= VMALLOC_START
			   && (unsigned int)start <= VMALLOC_END) {
			...
			for (i = 0; i < page_num; i++, page_start += DEFAULT_PAGE_SIZE) {
				...
				if (ppage == NULL) {
					ret = -1;
					M4UMSG("error: ppage is 0 in cache_maint of vmalloc!, ");
					M4UMSG("module=%s, start=0x%x, pagestart=0x%x\n",
						m4u_get_module_name(eModuleID),
						(unsigned int)start, page_start);
					goto out;
				}
...
int m4u_do_dma_cache_maint(M4U_MODULE_ID_ENUM eModuleID, const void *va, size_t size, int direction)
{
	...
	MMProfileLogEx(M4U_MMP_Events[PROFILE_DMA_MAINT_ALL], MMProfileFlagStart,
	...
	if ((((unsigned int)va % L1_CACHE_BYTES != 0) || (size % L1_CACHE_BYTES) != 0)) {
		...
		M4UMSG
		    ("error: addr un-align, module=%s, addr=0x%x, size=0x%x, process=%s, align=0x%x\n",
		     m4u_get_module_name(eModuleID), (unsigned int)va, size, current->comm,
		     L1_CACHE_BYTES);
	}
	...
	MMProfileLogEx(M4U_MMP_Events[PROFILE_DMA_MAINT_ALL], MMProfileFlagEnd,
		       ((unsigned int)eModuleID << 16) | direction, ret);
...
#ifdef M4U_PAGETABLE_ENHANCEMENT
struct page *m4u_alloc_pages()
{
	int i, start, end;
	struct page *tmp_page;
	unsigned long order = 10;	/* allocate 4M page table */

	tmp_page = alloc_pages(GFP_KERNEL | __GFP_ZERO, 10);
	if (!tmp_page) {
		M4UMSG("m4u_alloc_pages() alloc_pages failed.\n");
		return false;
	}
...
void m4u_dma_cache_by_range(unsigned int BufAddr, unsigned int BufSize, unsigned int mvaRegionAddr)
{
	unsigned int *pteStart;
	unsigned int pteNr;
	unsigned int align_page_num, page_num;
	...
	dmac_flush_range(pteStart, pteStart + pteNr - 1);
}
#endif

/* page table addr should (2^16)x align */
#define M4U_PAGE_TABLE_ALIGN (PT_TOTAL_ENTRY_NUM*sizeof(unsigned int) - 1)
/* protect buffer should be 128x align */
#define M4U_PROTECT_BUF_OFFSET (128-1)
static bool m4u_struct_init(void)
{
	...
	for (i = 0; i < M4U_CLIENT_MODULE_NUM; i++)
		gModuleMaxMVASize[i] = (800 * 0x00100000);

#ifdef M4U_PAGETABLE_ENHANCEMENT
	/* ======= alloc pagetable======================= */
	tmp_page = m4u_alloc_pages();

	pPT_nonsec = (unsigned int *)page_address(tmp_page);
	pt_pa_nonsec = (unsigned int)page_to_phys(tmp_page);

	M4UMSG("alloc_pages() success! pagetable_va=0x%x, pagetable_pa=0x%x.\n",
	...
	pPT_sec = (unsigned int)page_address(tmp_page);
	pt_pa_sec = (unsigned int)page_to_phys(tmp_page);
...
/* ======= alloc pagetable done======================= */
	...
	pPT_nonsec =
	    dma_alloc_coherent(NULL, PT_TOTAL_ENTRY_NUM * sizeof(unsigned int),
			       &pt_pa_nonsec, GFP_KERNEL);
	if (!pPT_nonsec) {
	...
	if ((pt_pa_nonsec & M4U_PAGE_TABLE_ALIGN) != 0) {
		...
		M4UMSG
		    ("dma_alloc_coherent memory not align. PageTablePA=0x%x we will try again\n",
		     pt_pa_nonsec);
		dma_free_coherent(NULL, PT_TOTAL_ENTRY_NUM * sizeof(unsigned int),
				  pPT_nonsec, pt_pa_nonsec);
		tmp =
		    (unsigned int)dma_alloc_coherent(NULL,
						     PT_TOTAL_ENTRY_NUM *
						     sizeof(unsigned int) +
						     M4U_PAGE_TABLE_ALIGN, &pt_pa_nonsec,
						     GFP_KERNEL);
		if (!tmp) {
			M4UMSG("dma_alloc_coherent error!  dma memory not available.\n");
			return false;
	...
	pPT_sec =
	    dma_alloc_coherent(NULL, PT_TOTAL_ENTRY_NUM * sizeof(unsigned int), &pt_pa_sec,
			       GFP_KERNEL);
	if (!pPT_sec) {
	...
	if ((pt_pa_sec & M4U_PAGE_TABLE_ALIGN) != 0) {
		...
		M4UMSG
		    ("dma_alloc_coherent memory not align. PageTablePA=0x%x we will try again\n",
		     pt_pa_sec);
		dma_free_coherent(NULL, PT_TOTAL_ENTRY_NUM * sizeof(unsigned int), pPT_sec,
				  pt_pa_sec);
		tmp =
		    (unsigned int)dma_alloc_coherent(NULL,
						     PT_TOTAL_ENTRY_NUM *
						     sizeof(unsigned int) +
						     M4U_PAGE_TABLE_ALIGN, &pt_pa_sec, GFP_KERNEL);
		if (!tmp) {
			M4UMSG("dma_alloc_coherent error!  dma memory not available.\n");
			return false;
		}
	...
	pProtectVA =
		kmalloc(TF_PROTECT_BUFFER_SIZE * TOTAL_M4U_NUM +
				    M4U_PROTECT_BUF_OFFSET, GFP_KERNEL | __GFP_ZERO);
	if (NULL == pProtectVA) {
	...
	pProtectVA =
	    (unsigned int *)(((unsigned int)pProtectVA + M4U_PROTECT_BUF_OFFSET) &
			     (~M4U_PROTECT_BUF_OFFSET));
...
/* gM4U_align_page_va = (unsigned int)page_address(tmp_page); */
	...
	pM4URegBackUp = kmalloc(BACKUP_REG_SIZE, GFP_KERNEL | __GFP_ZERO);
	...
	m4u_sec_init(pt_pa_nonsec, gM4U_L2_enable);
#endif
...
/* =============================== */

	{
		...
		for (i = 0; i < SMI_LARB_NR; i++) {
			...
			larb_base = get_larb_base_addr(i);

			The larb assign doesn't exist, skip this larb
			if (larb_base == SMI_ERROR_ADDR)
				continue;

			larb_clock_on(i);
			set SMI_SHARE_EN to 0
			M4U_WriteReg32(larb_base, SMI_SHARE_EN, 0x0);
...
/* *(pReg++) = COM_ReadReg32(REG_SMI_BUS_SEL); */
	...
	if (pt_pa_nonsec != *pM4URegBackUp) {
		M4UERR
		    ("PT_BASE in memory is error after backup! expect PTPA=0x%x, backupReg=0x%x\n",
...
static char *m4u_get_port_name(M4U_PORT_ID_ENUM portID)
{
	...
	case VDEC_PP_EXT:
		return "VDEC_PP_EXT";
	case VDEC_PPWRAP_EXT:
		return "VDEC_PPWRAP_EXT";
	case VDEC_AVC_MV_EXT:
		return "VDEC_AVC_MV_EXT";
	case VDEC_PRED_RD_EXT:
		return "VDEC_PRED_RD_EXT";
	case VDEC_PRED_WR_EXT:
		return "VDEC_PRED_WR_EXT";
	case VDEC_VLD_EXT:
		return "VDEC_VLD_EXT";
	case VDEC_VLD2_EXT:
		return "VDEC_VLD2_EXT";
	case VDEC_TILE_EXT:
		return "VDEC_TILE_EXT";


	case VDEC_IMG_RESZ_EXT:
		return "VDEC_IMG_RESZ_EXT";
	case VENC_RCPU:
		return "VENC_RCPU";
	case VENC_REC_FRM:
		return "VENC_REC_FRM";
	...
	case VENC_LT_RCPU:
		return "VENC_LT_RCPU";
	case VENC_LT_REC_FRM:
		return "VENC_LT_REC_FRM";
	case VENC_LT_BSDMA:
		return "VENC_LT_BSDMA";
	case JPGDEC_BSDMA:
		return "JPGDEC_BSDMA";
	case VENC_SV_COMV:
		return "VENC_SV_COMV";
	...
	case JPGENC_BSDMA:
		return "JPGENC_BSDMA";
	case VENC_CUR_LUMA:
		return "VENC_CUR_LUMA";
	case VENC_CUR_CHROMA:
		return "VENC_CUR_CHROMA";
	case VENC_REF_LUMA:
		return "VENC_REF_LUMA";
	case VENC_REF_CHROMA:
		return "VENC_REF_CHROMA";
	case IMG_RESZ:
		return "IMG_RESZ";

	case VENC_LT_SV_COMV:
		return "VENC_LT_SV_COMV";
	case VENC_LT_RD_COMV:
		return "VENC_LT_RD_COMV";
	case VENC_LT_CUR_LUMA:
		return "VENC_LT_CUR_LUMA";
	case VENC_LT_CUR_CHROMA:
		return "VENC_LT_CUR_CHROMA";
	case VENC_LT_REF_LUMA:
		return "VENC_LT_REF_LUMA";
	case VENC_LT_REF_CHROMA:
		return "VENC_LT_REF_CHROMA";
...
static void m4u_memory_usage(bool bPrintAll)
{
	...
	for (i = 0; i < M4U_CLIENT_MODULE_NUM; i++) {
		M4UMSG
		    ("id=%-2d, name=%-10s, max=%-5dKB, current=%-5dKB, locked_page=%-3d\n",
		     i, m4u_get_module_name(i), pmodule_max_size[i] / 1024,
		     pmodule_current_size[i] / 1024, pmodule_locked_pages[i]);
	}
}
...
#endif
			regval =
			    m4uHw_get_field_by_mask(0, REG_SMI_SECUR_CON_OF_PORT(i),
						    F_SMI_SECUR_CON_VIRTUAL(i));
#ifndef CONFIG_MTK_FPGA
			/* disable_mux(MT_MUX_MM, "m4u");*/
#endif
			if (regval)
				M4UMSG(KERN_INFO "%s(%d), ", m4u_get_port_name(i), i);
		}
		M4UMSG(KERN_INFO "\n");
	}
#endif
...
static int m4u_dump_seq_range_info(void)
{
	...
	for (i = 0; i < TOTAL_RANGE_NUM; i++) {
		if (1 == pRangeDes[i].Enabled) {
			M4UMSG
			    ("pRangeDes[%d]: Enabled=%d, module=%s, MVAStart=0x%x, MVAEnd=0x%x, entrycount=%d\n",
			     i, pRangeDes[i].Enabled,
			     m4u_get_module_name(pRangeDes[i].eModuleID),
...
int m4u_do_dump_info(int m4u_index)
{
	...
	for (i = 0; i < TOTAL_WRAP_NUM; i++) {
		if (1 == pWrapDes[i].Enabled) {
			M4UMSG
			    ("pWrapDes[%d]: Enabled=%d, module=%s, MVAStart=0x%x, MVAEnd=0x%x\n",
			     i, pWrapDes[i].Enabled,
			     m4u_get_port_name(pWrapDes[i].eModuleID), pWrapDes[i].MVAStart,
			     pWrapDes[i].MVAEnd);
		}
	}
			  ...
			  unsigned int *map_va, unsigned int *map_size)
{
	...
	for (map_page_num = 0; map_page_num < page_num; map_page_num++) {
		...
		if (sec)
			...
			pa = *(unsigned int *)mva_pteAddr_nonsec(mva +
								 map_page_num * M4U_PAGE_SIZE);
	...
	case 8:		/* get profile report */
		{
			...
			/* for(i=0;i<TOTAL_M4U_NUM;i++) */
			/* { */
			/* start to count performance for next 1 second */
			/* m4u_monitor_start(m4u_get_port_by_index(i)); */
			/* } */
		}
		break;
	...
	case 9:		/* stop profile and get report */
		{
			...
			/* for(i=0;i<TOTAL_M4U_NUM;i++) */
			/* { */
			/* m4u_monitor_stop(m4u_get_port_by_index(i));
			 * print performance in last 1 second */
			/* } */
		}
		break;
...
static void m4u_print_mva_list(struct file *filep, const char *pMsg)
{
	...
	list_for_each(pListHead, &(pNode->mvaList)) {
		...
		M4UMSG("module=%s, va=0x%x, size=0x%x, mva=0x%x, flags=%d\n",
		       m4u_get_module_name(pList->eModuleId), pList->bufAddr, pList->size,
		       pList->mvaStart, pList->flags);
	}
	mutex_unlock(&(pNode->dataMutex));
...
#if  __M4U_MAP_MVA_TO_KERNEL_FOR_DEBUG__
	pList->mapped_kernel_va_for_debug = 0;
#endif
	return pList;
}
						...
						struct file *a_pstFile)
{
	...
	list_for_each(pListHead, &(pNode->mvaList)) {
		...
		if (pList->mvaStart == p_m4u_module->MVAStart) {
			...
			ret = NULL;
			M4UMSG
			    ("error: input argument isn't valid, can't find the node at garbage list\n");
		}
	}
