...
/* (TOTAL_MVA_RANGE/DEFAULT_PAGE_SIZE)  total page table entries */
#define PT_TOTAL_ENTRY_NUM    (1*1024*1024)
...
struct m4u_device {
	...
	void __iomem *m4u_base[TOTAL_M4U_NUM];
	void __iomem *L2_base;
	void __iomem *smi_common_ao;
...
/* SMI related */
#ifdef SMI_LARB_NR	/* it is 6 in smi */
#undef SMI_LARB_NR
#endif
...
typedef struct _M4U_MOUDLE {
	/* module ID used inside M4U driver, defined in M4U_PORT_MODULE_ID_ENUM */
	/* MVA alloc / dealloc */
	M4U_MODULE_ID_ENUM eModuleID;
...
#define MTK_M4U_T_ALLOC_MVA_SEC       _IOWR(MTK_M4U_MAGICNO, 26, int)
#define MTK_M4U_T_DEALLOC_MVA_SEC     _IOWR(MTK_M4U_MAGICNO, 27, int)
...
int __m4u_dealloc_mva(M4U_MODULE_ID_ENUM eModuleID,
		      ...
		      const unsigned int MVA, struct sg_table *sg_table);
...
int m4u_do_dma_cache_maint(M4U_MODULE_ID_ENUM eModuleID, const void *start, size_t size,
			   int direction);
int m4u_do_mva_map_kernel(unsigned int mva, unsigned int size, int sec, unsigned int *map_va,
...
extern int m4u_get_user_pages(int eModuleID, struct task_struct *tsk, struct mm_struct *mm,
			      unsigned long start, int nr_pages, int write, int force,
			      struct page **pages, struct vm_area_struct **vmas);
extern void smp_inner_dcache_flush_all(void);
extern int m4u_reclaim_mva_callback_ovl(int moduleID, unsigned int va, unsigned int size,
					unsigned int mva);
