...
#ifdef MTK_M4U_DBG
#define M4UDBG(string, args...)	pr_dbg("M4U_K"string, ##args)
#else
...
#define M4UMSG(string, args...)	\
	pr_info("M4U_K [pid=%d]"string, current->tgid, ##args)
#define M4UWARN(string, args...)	\
	pr_warn("M4U_K [pid=%d]"string, current->tgid, ##args)
#else
#define M4UMSG(string, args...)
#define M4UWARN(string, args...)
#endif

/*aee_kernel_exception(THIS_MODULE, "M4U [M4U_K] error:"string, ##args);*/

#define M4UERR(string, args...) \
	pr_err("M4U_K [pid=%d]error_assert_fail: "	\
	string, current->tgid, ##args)
...
void init_mlock_cnt(void)
{
	...
	pMlock_cnt = vmalloc(mlock_cnt_size);
	if (!pMlock_cnt) {
			 ...
			 struct page **pages, struct vm_area_struct **vmas)
{
	...
	if (!!pages != !!(gup_flags & FOLL_GET)) {
		...
		M4UMSG(" pages=0x%x, gup_flags & FOLL_GET=0x%x\n", (unsigned int)pages, gup_flags & FOLL_GET);
	}
	...
	do {
		...
		do {
			...
			MMProfileLogEx(M4U_MMP_Events[PROFILE_FOLLOW_PAGE], MMProfileFlagStart,
				       eModuleID, start & (~0xFFF));
			page = follow_page(vma, start, foll_flags);
			MMProfileLogEx(M4U_MMP_Events[PROFILE_FOLLOW_PAGE], MMProfileFlagEnd,
				       eModuleID, 0x1000);
			while (!page) {
				int ret;
				...
				M4UDBG("Trying to allocate for %dth page(vaddr: 0x%08x)\n", i,
				       start);
				MMProfileLogEx(M4U_MMP_Events[PROFILE_FORCE_PAGING],
					       MMProfileFlagStart, eModuleID, start & (~0xFFF));
				ret =
				    handle_mm_fault(mm, vma, start,
						    (foll_flags & FOLL_WRITE) ? FAULT_FLAG_WRITE : 0);
				MMProfileLogEx(M4U_MMP_Events[PROFILE_FORCE_PAGING],
					       MMProfileFlagEnd, eModuleID, 0x1000);
				if (ret & VM_FAULT_ERROR) {
					if (ret & VM_FAULT_OOM) {
						M4UMSG("handle_mm_fault() error: no memory,");
						M4UMSG(" addr:0x%08lx (%d pages are allocated), module=%d\n",
							start, i, eModuleID);
						/* m4u_dump_maps(start); */
						return i ? i : -ENOMEM;
					}
				...
				MMProfileLogEx(M4U_MMP_Events[PROFILE_FOLLOW_PAGE],
					       MMProfileFlagStart, eModuleID, start & (~0xFFF));
				page = follow_page(vma, start, foll_flags);
				MMProfileLogEx(M4U_MMP_Events[PROFILE_FOLLOW_PAGE],
					       MMProfileFlagEnd, eModuleID, 0x1000);
			...
			if (pages) {
				pages[i] = page;
				MMProfileLogEx(M4U_MMP_Events[PROFILE_MLOCK], MMProfileFlagStart,
					       eModuleID, start & (~0xFFF));
				...
				if (PageMlocked(page) == 0) {
					...
					dump_page(page, "Can't mlock page");
				} else {
					...
					if (pfn < mlock_cnt_size) {
						...
						M4UERR
						    ("mlock_cnt_size is too small: pfn=%d, size=%d\n",
						     pfn, mlock_cnt_size);
					}
				...
				MMProfileLogEx(M4U_MMP_Events[PROFILE_MLOCK], MMProfileFlagEnd,
					       eModuleID, 0x1000);
