...
static void set_adaptive_cpu_power_limit(unsigned int limit)
{
	...
	if (prv_adp_cpu_pwr_lim != adaptive_cpu_power_limit) {
		pr_debug("set_adaptive_cpu_power_limit %d, T=%d,%d,%d,%d,%d,%d,%d,%d\n",
...
static void set_static_cpu_power_limit(unsigned int limit)
{
	...
	if (prv_stc_cpu_pwr_lim != static_cpu_power_limit) {
		pr_debug("set_static_cpu_power_limit %d, T=%d,%d,%d,%d,%d,%d,%d,%d\n",
			 (final_limit != 0x7FFFFFFF) ? final_limit : 0, GPU_TS1_T, CA7_TS2_T,
			 CA15_TS2_T, GPU_TS2_T, CORE_TS2_T, CA7_TS3_T, CA15_TS4_T, GPU_TSABB_T);
...
static void set_adaptive_gpu_power_limit(unsigned int limit)
{
	...
	pr_debug("set_adaptive_gpu_power_limit %d\n",
...
static void set_static_gpu_power_limit(unsigned int limit)
{
	...
	final_limit = MIN(adaptive_gpu_power_limit, static_gpu_power_limit);
	pr_debug("set_static_gpu_power_limit %d\n", (final_limit != 0x7FFFFFFF) ? final_limit : 0);
...
static void dump_spm_reg(void)
{
	...
	pr_debug("SPM_SLEEP_ISR_RAW_STA       =0x%08x\n", spm_read(SPM_SLEEP_ISR_RAW_STA));
	pr_debug("SPM_PCM_REG13_DATA          =0x%08x\n", spm_read(SPM_PCM_REG13_DATA));
	pr_debug("SPM_SLEEP_WAKEUP_EVENT_MASK =0x%08x\n", spm_read(SPM_SLEEP_WAKEUP_EVENT_MASK));
	pr_debug("SPM_POWERON_CONFIG_SET      =0x%08x\n", spm_read(SPM_POWERON_CONFIG_SET));
	pr_debug("SPM_POWER_ON_VAL1		     =0x%08x\n", spm_read(SPM_POWER_ON_VAL1));
	pr_debug("SPM_PCM_IM_LEN			 =0x%08x\n", spm_read(SPM_PCM_IM_LEN));
	pr_debug("SPM_PCM_PWR_IO_EN		     =0x%08x\n", spm_read(SPM_PCM_PWR_IO_EN));

	pr_debug("SPM_PCM_CON0			     =0x%08x\n", spm_read(SPM_PCM_CON0));
	pr_debug("SPM_PCM_CON1			     =0x%08x\n", spm_read(SPM_PCM_CON1));
	pr_debug("SPM_PCM_IM_PTR			 =0x%08x\n", spm_read(SPM_PCM_IM_PTR));
	pr_debug("SPM_PCM_REG1_DATA			 =0x%08x\n", spm_read(SPM_PCM_REG1_DATA));
	pr_debug("SPM_PCM_REG2_DATA			 =0x%08x\n", spm_read(SPM_PCM_REG2_DATA));
	pr_debug("SPM_PCM_REG3_DATA			 =0x%08x\n", spm_read(SPM_PCM_REG3_DATA));
	pr_debug("SPM_PCM_REG7_DATA			 =0x%08x\n", spm_read(SPM_PCM_REG7_DATA));
	pr_debug("SPM_PCM_REG9_DATA			 =0x%08x\n", spm_read(SPM_PCM_REG9_DATA));
	pr_debug("SPM_PCM_REG12_DATA			 =0x%08x\n", spm_read(SPM_PCM_REG12_DATA));
	pr_debug("SPM_PCM_REG14_DATA			 =0x%08x\n", spm_read(SPM_PCM_REG14_DATA));
	pr_debug("SPM_PCM_REG15_DATA			 =0x%08x\n", spm_read(SPM_PCM_REG15_DATA));
	pr_debug("SPM_PCM_FSM_STA			 =0x%08x\n", spm_read(SPM_PCM_FSM_STA));
...
static void thermal_interrupt_handler(int bank)
{
	...
	pr_debug("thermal_interrupt_handler,bank=0x%08x,ret=0x%08x\n", bank, ret);
	/* pr_info("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n"); */
	...
	if (ret & THERMAL_MON_CINTSTS0)
		pr_debug("thermal_isr: thermal sensor point 0 - cold interrupt trigger\n");

	if (ret & THERMAL_MON_HINTSTS0)
		pr_debug("<<<thermal_isr>>>: thermal sensor point 0 - hot interrupt trigger\n");


	if (ret & THERMAL_MON_HINTSTS1)
		pr_debug("<<<thermal_isr>>>: thermal sensor point 1 - hot interrupt trigger\n");


	if (ret & THERMAL_MON_HINTSTS2)
		pr_debug("<<<thermal_isr>>>: thermal sensor point 2 - hot interrupt trigger\n");



	if (ret & THERMAL_tri_SPM_State0)
		pr_debug("thermal_isr: Thermal state0 to trigger SPM state0\n");
	if (ret & THERMAL_tri_SPM_State1) {
		...
		pr_debug("THERMAL_tri_SPM_State1, T=%d,%d,%d,%d,%d,%d,%d,%d\n",
	...
	if (ret & THERMAL_tri_SPM_State2)
		pr_debug("thermal_isr: Thermal state2 to trigger SPM state2\n");
...
static irqreturn_t thermal_all_bank_interrupt_handler(int irq, void *dev_id)
{
	...
	pr_debug("thermal_all_bank_interrupt_handler\n");
...
{
	pr_debug("thermal_config_Bank0_TS:\n");
...
static void thermal_config_Bank1_TS(void)
{
	...
	pr_debug("thermal_config_Bank1_TS\n");
...
static void thermal_config_Bank2_TS(void)
{
	...
	pr_debug("thermal_config_Bank2_TS\n");
...
{
	pr_debug("thermal_config_Bank3_TS\n");
...
{
	pr_debug("thermal_config_TS_in_banks bank_num=%d\n", bank_num);
...
static void set_tc_trigger_hw_protect(int temperature, int temperature2)
{
	...
	pr_debug("set_tc_trigger_hw_protect t1=%d t2=%d\n", temperature, temperature2);
	...
	/* pr_debug("set_tc_trigger_hw_protect 1 TEMPMONINT:temp=0x%x\n",temp); */
	...
	/* pr_debug("set_tc_trigger_hw_protect 2 TEMPMONINT:temp=0x%x\n",temp); */
...
static void thermal_cal_prepare(void)
{
	...
	pr_debug("[calibration] temp0=0x%x, temp1=0x%x\n", temp0, temp1);
	/* mtkpr_debug("thermal_cal_prepare\n"); */
...
static void thermal_cal_prepare_2(U32 ret)
{
	...
	pr_debug("[calibration] g_ge         = 0x%x\n", g_ge);
	pr_debug("[calibration] g_gain       = 0x%x\n", g_gain);

	pr_debug("[calibration] g_x_roomt1   = 0x%x\n", g_x_roomt[0]);
	pr_debug("[calibration] g_x_roomt2   = 0x%x\n", g_x_roomt[1]);
	pr_debug("[calibration] g_x_roomt3   = 0x%x\n", g_x_roomt[2]);
	pr_debug("[calibration] g_x_roomt4   = 0x%x\n", g_x_roomt[3]);
	pr_debug("[calibration] g_x_roomtabb = 0x%x\n", g_x_roomt[4]);
...
static S32 temperature_to_raw_room(U32 ret)
{
	...
	pr_debug("temperature_to_raw_room\n");
	...
	pr_debug("[Temperature_to_raw_roomt] temperature=%d, raw[%d]=%d", ret, index,
		 format_4[index]);
...
int get_immediate_temp2_wrap(void)
{
	...
	pr_debug("get_immediate_temp2_wrap curr_temp=%d\n", curr_temp);
...
int get_immediate_ts1_wrap(void)
{
	...
	pr_debug("get_immediate_ts1_wrap curr_temp=%d\n", curr_temp);
...
#endif
	pr_debug("get_immediate_ts2_wrap curr_temp=%d\n", curr_temp);
...
#endif
	pr_debug("get_immediate_ts3_wrap curr_temp=%d\n", curr_temp);
...
#endif
	pr_debug("get_immediate_ts4_wrap curr_temp=%d\n", curr_temp);
...
static int tscpu_get_temp(struct thermal_zone_device *thermal, unsigned long *t)
{
	...
	/* pr_debug("=================================================\n"); */
	pr_debug("bank0_T =%d,bank1_T =%d,bank2_T =%d,bank3_T =%d\n", bank0_T, bank1_T,
		 bank2_T, bank3_T);
	/* pr_debug("=================================================\n"); */
	pr_debug("tscpu_get_temp MAX CPU T=%d\n", curr_temp);
	...
	pr_debug("tscpu_get_temp CPU T1=%d\n", curr_temp);
...
int tscpu_get_bL_temp(thermal_TS_name ts_name)
{
	...
	pr_debug("tscpu_get_bL_temp,bank=%d\n", ts_name);
...
{
	/* pr_debug(" tscpu_get_Tj_temp,g_max_temp=%d\n",g_max_temp); */
	return g_max_temp;
}
...
//	pr_debug(" tscpu_get_cpu_temp CPU T1=%d\n",curr_temp);

    if((curr_temp > (trip_temp[0] - 15000)) || (curr_temp<-30000))
		pr_debug("tscpu_get_cpu_temp T=%d\n", curr_temp);
...
static int sysrst_cpu_set_cur_state(struct thermal_cooling_device *cdev, unsigned long state)
{
	...
	if (cl_dev_sysrst_state == 1) {
		mtkts_dump_cali_info();
		pr_debug("sysrst_cpu_set_cur_state = 1\n");
		pr_debug("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
		pr_debug("*****************************************\n");
		pr_debug("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
...
static void tscpu_set_GPIO_toggle_for_monitor(void)
{
	...
	pr_debug("tscpu_set_GPIO_toggle_for_monitor,g_GPIO_out_enable=%d\n", g_GPIO_out_enable);

	if (g_GPIO_out_enable == 1) {
		if (g_max_temp > g_trigger_temp) {

			pr_debug("g_max_temp %d > g_trigger_temp %d\n", g_max_temp, g_trigger_temp);
				    ...
				    loff_t *data)
{
	...
	if (sscanf(desc, "%s %d %s %d ", TEMP, &valTEMP, ENABLE, &valENABLE) == 4) {
		...
		if (!strcmp(TEMP, "TEMP")) {
			...
			pr_debug("g_trigger_temp=%d\n", valTEMP);
		} else {
			pr_debug("tscpu_write_GPIO_out TEMP bad argument\n");
			return -EINVAL;
		}

		if (!strcmp(ENABLE, "ENABLE")) {
			g_GPIO_out_enable = valENABLE;
			pr_debug("g_GPIO_out_enable=%d,g_GPIO_already_set=%d\n", valENABLE,
				 g_GPIO_already_set);
		} else {
			pr_debug("tscpu_write_GPIO_out ENABLE bad argument\n");
			return -EINVAL;
		}
				    ...
				    loff_t *data)
{
	...
	if (2 <= sscanf(desc, "%d %d", &trip, &factor)) {
		pr_debug("tscpu_write_fastpoll input %d %d\n", trip, factor);

		if ((trip >= 0) && (factor > 0)) {
			fast_polling_trip_temp = trip;
			fast_polling_factor = factor;

			pr_debug("tscpu_write_fastpoll applied %d %d\n", fast_polling_trip_temp,
				 fast_polling_factor);
		} else {
			pr_debug("tscpu_write_fastpoll out of range\n");
		}
...
/*
	curr_temp = get_immediate_temp();
		...
		if(i==0)
		{
			pr_debug("tscpu_write setting error");
		}
		else if(i==num_trip)
...
static void thermal_pause_all_periodoc_temp_sensing(void)
{
	...
	pr_debug("thermal_pause_all_periodoc_temp_sensing\n");
...
static void thermal_release_all_periodoc_temp_sensing(void)
{
	...
	/* pr_debug("thermal_release_all_periodoc_temp_sensing\n"); */
...
static void thermal_disable_all_periodoc_temp_sensing(void)
{
	...
	/* pr_debug("thermal_disable_all_periodoc_temp_sensing\n"); */
	...
	for (i = 0; i < ROME_BANK_NUM; i++) {
		...
		mtktscpu_switch_bank(i);
		/* pr_debug("thermal_disable_all_periodoc_temp_sensing:Bank_%d\n",i); */
...
static int ktp_thread(void *arg)
{
	...
	for (;;) {
		...
		pr_debug("ktp_thread awake,tc_mid_trip=%d\n", tc_mid_trip);
		...
		if ((temp_tc_mid_trip > -275000) && (max_temp >= (temp_tc_mid_trip - 5000))) {
			...
			pr_debug("ktp_thread unlimit cpu=%d\n", final_limit);
			...
			pr_debug("ktp_thread unlimit gpu=%d\n", final_limit);
			mt_gpufreq_thermal_protect((final_limit != 0x7FFFFFFF) ? final_limit : 0);
			...
			pr_debug("ktp_thread else temp=%d, trip=%d, ltd=%d\n", max_temp,
...
static void thermal_initial_all_bank(void)
{
	...
	pr_debug("thermal_initial_all_bank,ROME_BANK_NUM=%d\n", ROME_BANK_NUM);
...
#endif
	pr_debug("tscpu_config_all_tc_hw_protect,temperature=%d,temperature2=%d,\n",
		 temperature, temperature2);
...
static void tscpu_fast_initial_sw_workaround(void)
{
	int i = 0;
	unsigned long flags;
	/* pr_debug("tscpu_fast_initial_sw_workaround\n"); */
...
int tscpu_get_cpu_temp_met(MTK_THERMAL_SENSOR_CPU_ID_MET id)
{
	...
	if (id < 0 || id >= MTK_THERMAL_SENSOR_CPU_COUNT)
		...
		ret = (adaptive_cpu_power_limit != 0x7FFFFFFF) ? adaptive_cpu_power_limit : 0;
	} else if (ATM_GPU_LIMIT == id) {
		ret = (adaptive_gpu_power_limit != 0x7FFFFFFF) ? adaptive_gpu_power_limit : 0;
	} else {
...
static enum hrtimer_restart tscpu_update_tempinfo(struct hrtimer *timer)
{
	...
	unsigned long flags;
#endif
	/* pr_debug("tscpu_update_tempinfo\n"); */
...
static void tscpu_update_temperature_timer_init(void)
{
	...
	pr_debug("tscpu_update_temperature_timer_init\n");
...
static int thermal_fast_init(void)
{
	...
	/* high temperature */
	pr_debug("===========================================\n");
	pr_debug("thermal_interrupt_trigger_test: high temp=0x14\n");
	pr_debug("===========================================\n");
...
/* DRV_WriteReg32(TEMPSPARE2, temp);        // set sensor voltage and sensor valid */
	...
	if ((temp & 0x0FFF) != 0xD78) {	/* first filter valid should be equal to 0x14 */
		pr_debug("thermal_interrupt_trigger_test: fail, read TEMPMSR0 = 0x%x\n",
			 (temp & 0x0FFF));
	} else
		pr_debug("thermal_interrupt_trigger_test: pass, read TEMPMSR0 = 0x%x\n",
			 (temp & 0x0FFF));
