...
struct pinctrl_state *subflashlight_en_high = NULL;
struct pinctrl_state *subflashlight_en_low = NULL;
...
int flashlight_gpio_init(struct platform_device *pdev)
{
  ...
  if (IS_ERR(flashctrl)) {
    ...
    pr_err(KERN_ERR"--------Cannot find flash pinctrl -----------\n");
  ...
  subflashlight_en_high = pinctrl_lookup_state(flashctrl, "subflashlight_high");
  if (IS_ERR(subflashlight_en_high)) {
    ret = PTR_ERR(subflashlight_en_high);
    pr_err("%s : init err, subflashlight_en_high\n", __func__);
  }

  subflashlight_en_low = pinctrl_lookup_state(flashctrl, "subflashlight_low");
  if (IS_ERR(subflashlight_en_low)) {
    ret = PTR_ERR(subflashlight_en_low);
    pr_err("%s : init err, subflashlight_en_low\n", __func__);
  }

  PK_DBG("%s line %d \n",__func__,__LINE__);
  return ret;
}
unsigned char subflash_gpio_high(void)
{
    PK_DBG("%s enter\n", __func__);
    pinctrl_select_state(flashctrl, subflashlight_en_high);
    return 0;
}

unsigned char subflash_gpio_low(void)
{
    PK_DBG("%s enter\n", __func__);
    pinctrl_select_state(flashctrl, subflashlight_en_low);
    return 0;
}
