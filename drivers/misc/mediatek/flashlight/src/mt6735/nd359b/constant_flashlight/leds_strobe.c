...
struct pinctrl_state *sub_mode_gpio_h = NULL;
struct pinctrl_state *sub_mode_gpio_l = NULL;
...
int flashlight_gpio_init(struct platform_device *pdev)
{
  ...
  sub_mode_gpio_h= pinctrl_lookup_state(flashctrl, "sub_state_mode_output1");
  if (IS_ERR(sub_mode_gpio_h)) {
    ret = PTR_ERR(sub_mode_gpio_h);
    pr_err("%s : pinctrl err, sub_mode_gpio_h\n", __func__);
  }
  sub_mode_gpio_l = pinctrl_lookup_state(flashctrl, "sub_state_mode_output0");
  if (IS_ERR(sub_mode_gpio_l)) {
    ret = PTR_ERR(sub_mode_gpio_l);
    pr_err("%s : pinctrl err, sub_mode_gpio_l\n", __func__);
  }
