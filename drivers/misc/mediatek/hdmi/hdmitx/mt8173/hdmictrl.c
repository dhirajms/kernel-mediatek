...
void vConfigHdmiSYS(unsigned char bResIndex)
{
	...
	if (hdmi_dpi_output == 1) {
		...
		/* vWriteHdmiSYSMsk(MMSYS_CG_CON1, HDMI_HDCP_CLK_EN|
		HDMI_HDCP24_CLK_EN, HDMI_HDCP24_CLK|HDMI_HDCP_CLK); */
...
void vBlackHDMI2Only(void)
{
	...
	vWriteHdmiGRLMsk(HDCP_TOP_CTRL, OTP14VOVR_EN, OTP14VOVR_EN);
}

void MuteHDMI2Audio(void)
{
	vWriteHdmiGRLMsk(HDCP_TOP_CTRL, OTPAMUTEOVR_SET, OTPAMUTEOVR_SET);
	vWriteHdmiGRLMsk(HDCP_TOP_CTRL, OTP14AOVR_EN, OTP14AOVR_EN);
...
void vUnBlackHDMI2Only(void)
{
	...
	vWriteHdmiGRLMsk(HDCP_TOP_CTRL, OTP14VOVR_DIS, OTP14VOVR_EN);
}
...
void UnMuteHDMI2Audio(void)
{
	...
	vWriteHdmiGRLMsk(HDCP_TOP_CTRL, OTP14AOVR_DIS, OTP14AOVR_EN);
}
