...
static DEFINE_SEMAPHORE(hdmi_edid_mutex);
...
int hdmi_internal_power_on(void)
{
	...
	hdmi_is_boot_time = 0;
	if (down_interruptible(&hdmi_edid_mutex)) {
		pr_err("[hdmi]can't get semaphore in %s()\n", __func__);
		return 0;
	}
	up(&hdmi_edid_mutex);
...
void hdmi_internal_power_off(void)
{
	...
	if (hdmi_cec_on == 0) {
		clk_disable(hdmi_ref_clock[INFRA_SYS_CEC]);
		clk_unprepare(hdmi_ref_clock[INFRA_SYS_CEC]);
	}
	print_clock_reg();
...
static irqreturn_t hdmi_irq_handler(int irq, void *dev_id)
{
	...
	if ((port_hpd_value != port_hpd_value_bak) && (port_hpd_value != 0x2)) {
