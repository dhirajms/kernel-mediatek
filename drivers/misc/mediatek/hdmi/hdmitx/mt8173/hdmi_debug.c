...
static inline int hdmidrv_dump_regs(enum HDMI_REG_ENUM Module, char *pbuf)
{
	__u32 i;
	char *buf;
	int temp_len = 0;
...
static unsigned int icomparestr(const char *szText, const struct TEXT3ENUM_T *prText2Enum)
{
...
void mt_hdmi_show_info(char *pbuf)
{
	...
	case hdmir_cmd:
		ret = sscanf(buf + strlen(prTextEnum->szText), "%x", &reg_offset);
		if (ret != -1) {
