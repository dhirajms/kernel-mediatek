...
s32 _pwrap_reset_spislv(void)
{
	...
	WRAP_WR32(PMIC_WRAP_MAN_CMD, (OP_WR << 14) | ((OP_PMIC_SEL << 13) | (OP_CSL << 8)));
	WRAP_WR32(PMIC_WRAP_MAN_CMD, (OP_WR << 14) | ((OP_PMIC_SEL << 13) | (OP_OUTS << 8)));	/* to reset counter */
	WRAP_WR32(PMIC_WRAP_MAN_CMD, (OP_WR << 14) | ((OP_PMIC_SEL << 13) | (OP_CSH << 8)));
	WRAP_WR32(PMIC_WRAP_MAN_CMD, (OP_WR << 14) | ((OP_PMIC_SEL << 13) | (OP_OUTS << 8)));
	WRAP_WR32(PMIC_WRAP_MAN_CMD, (OP_WR << 14) | ((OP_PMIC_SEL << 13) | (OP_OUTS << 8)));
	WRAP_WR32(PMIC_WRAP_MAN_CMD, (OP_WR << 14) | ((OP_PMIC_SEL << 13) | (OP_OUTS << 8)));
	WRAP_WR32(PMIC_WRAP_MAN_CMD, (OP_WR << 14) | ((OP_PMIC_SEL << 13) | (OP_OUTS << 8)));
...
static s32 _pwrap_init_reg_clock(u32 regck_sel)
{
...
#elif defined SLV_6323
	if (regck_sel == 1)
		return 0x10;	/* not support in 6323!! */
	wdata = 0x3;
#endif
	...
	/* Set Dummy cycle for both 6397(assume 12MHz)/6323 (assume 12MHz) */
#ifdef SLV_6397
	...
	if (regck_sel == 1) {	/* 6MHz in 6323  => no support ; // 12MHz in 6397 */
#ifdef SLV_6397
		WRAP_WR32(PMIC_WRAP_CSHEXT_WRITE, 0x0);	/* wait data written into register => 3T_PMIC */
...
#endif
	} else if (regck_sel == 2) {	/* 12MHz in 6323; 24MHz in 6397 */
#ifdef SLV_6397
...
s32 pwrap_init(void)
{
	...
	PWRAPLOG("the reset register0x%p = %x\n", INFRA_GLOBALCON_RST0, regValue);
	...
	/* Set SPI_CK_freq = 26MHz for both 6320/6323/6397 */
	...
	/* Enable 6320 option */
#ifdef SLV_6323
	WRAP_WR32(PMIC_WRAP_OP_TYPE, OP_TYPE_CK);
	WRAP_WR32(PMIC_WRAP_MSB_FIRST, MSB);
#endif
	...
	/* 6397; 0:safe mode, 1:12MHz, 2:24MHz */
	/* 6323; 0:safe mode, 1:NA, 2:12MHz */
	...
	WRAP_WR32(PMIC_WRAP_INT_EN, 0x7ffffffb);	/* except for [31] debug_int */
...
static irqreturn_t mt_pmic_wrap_irq(int irqno, void *dev_id)
{
	...
	WRAP_WR32(PMIC_WRAP_HARB_HPRIO, 1 << 2);
...
static int pwrap_of_iomap(void)
{
	 ...
	 * INFRACFG_AO, TOPCKGEN
	 */

	struct device_node *infracfg_ao_node;
	struct device_node *topckgen_node;

	infracfg_ao_node = of_find_compatible_node(NULL, NULL, "mediatek,mt2701-infracfg");
	if (!infracfg_ao_node) {
	...
	topckgen_node = of_find_compatible_node(NULL, NULL, "mediatek,mt2701-topckgen");
	if (!topckgen_node) {
	...
	pr_debug("INFRACFG_AO reg: 0x%p\n", infracfg_ao_base);
	pr_debug("TOPCKGEN reg: 0x%p\n", topckgen_base);
