...
#define PK_XLOG_INFO(fmt, arg...)	pr_err(PFX "[%s]" fmt, __func__, ##arg)
...
BOOL CAMERA_Regulator_PowerOnOFF(struct regulator *pwrreg, BOOL IsOn, int vol)
{
	...
	pr_debug("[%s] IsOn:%d , vol:%d\n", __func__, IsOn, vol);

	if (!pwrreg) {
		pr_err("[%s] camera power regulator is null\n", __func__);
		return FALSE;
	}
	...
	if (IsOn) {		/* Power ON */
		...
		if (ret != 0) {
			pr_err("[%s] camera power regulator fails to enabled, ret = 0x%x\n",
			       __func__, ret);
			return FALSE;
		}
		...
		if (voltage_count <= 0) {
			pr_err("[%s] camera power regulator fails to count, voltage_count = %d\n",
		...
		if (high_bound_voltage <= 0) {
			pr_err("[%s] camera power regulator fails to list, high_bound_voltage = %d\n",
				__func__, high_bound_voltage);
			return FALSE;
		}
		...
		if (ret != 0) {
			pr_err("[%s] camera power regulator fails to set vol = %d , ret = 0x%x\n",
			       __func__, vol, ret);
			return FALSE;
		}
	...
	} else {		/* Power OFF */
		{
			...
			if (low_bound_voltage <= 0) {
				pr_err("[%s] camera power regulator fails to list, low_bound_voltage = %d\n",
					__func__, low_bound_voltage);
				return FALSE;
			}
			ret = regulator_set_voltage(pwrreg, low_bound_voltage, high_bound_voltage);
			if (ret != 0) {
				pr_err("[%s] camera power regulator fails to set high_bound_voltage = %d , low_bound_voltage = %d\n",
...
BOOL CAMERA_Regulator_poweron(int PinIdx, int PwrType, int Voltage)
{
	...
	pr_debug("[%s] PinIndex=%d , PowerType = %d , Voltage = %d\n", __func__, PinIdx, PwrType,
		 Voltage);
