...
static imgsensor_info_struct imgsensor_info = {
    ...
    .pre = {
        ...
        .framelength = 623,            //record different mode's framelength
...
#define RG_TYPICAL   				0x0090	//modified when write WB !!!
#define BG_TYPICAL				0x0095	//modified when write WB!!!
...
static int16_t para_medium[108] = {
	128,	-6, 	29, 	28, 	-10,	128,
	12, 	-10,	19, 	-2, 	8,	3,
	41, 	15, 	0,	5,	5,	26,
	-7, 	0,	1,	-3, 	2,	-11,
	...
	LOG_INF("GC8024_OTP flag1 = 0x%x\n",flag1);


	for(index=0;index<2;index++)
	{
		switch((flag1>>(6 - 2 * index))&0x03)
	{
	case 0x00:
...
/*LSC*/
	if (((flag1&0x0c) == 0x04)||((flag2&0xc0) == 0x40))
	{
		kal_uint16 integral[108];
		...
		for (i = 1; i < 108; ++i)
		{
			integral[i] = integral[i-1] + LH_wei[i];
		}
		
	/*read 82B*/	
		if ((flag1&0x0c) == 0x04)
		{
			LOG_INF("GC8024_OTP_Shading Group1 is Valid !!\n");		
			addr = 0x05;
		}
		if ((flag2&0xc0) == 0x40)
		{
			LOG_INF("GC8024_OTP_Shading Group2 is Valid !!\n");		
			addr = 0x57;
		}		
		...
		{		
			...
			OtpLsc[cnt] = gc8024_read_otp(addr%0x80);
	...
	/*82B to 108B = R'''/G'''/B'''*/
		...
		if (com_test_result[0] & (1 << (LH_wei[0]-1)))
		{
			com_test_result[0] |= ~((1 << (LH_wei[0]-1))-1);
		}
		else
		{
			com_test_result[0] &= (1 << (LH_wei[0]-1))-1;
		...
		for (i = 1; i < 108; ++i)
		{
			...
			if (com_test_result[i] & (1 << (LH_wei[i]-1)))
			{
				com_test_result[i] |= ~((1 << (LH_wei[i]-1))-1);
			}
			else
			{
				com_test_result[i] &= (1 << (LH_wei[i]-1))-1;
	...
	/*R''/B''/G''=R'''/B'''/G'''+para_medium*/
		...
		for (i = 0; i < 108; ++i)
		{
			com_test_step2[i] = com_test_result[i] + para_medium[i];/*chart 1.7 tavis*/			
		}
	...
	else if((flag1&0x0c) == 0x08 || (flag2&0xc0) == 0x80)
//wb
	{
		if ((flag1&0x0c) == 0x08)
		{
			LOG_INF("GC8024_OTP_Shading Group1 is Invalid !!\n");
			gc8024_otp_info.lsc_flag = 0x02;			
		}
		if ((flag2&0xc0) == 0x80)
		{
			LOG_INF("GC8024_OTP_Shading Group2 is Invalid !!\n");
			gc8024_otp_info.lsc_flag = 0x02;			
		}	
	}
	else
	{
		LOG_INF("GC8024_OTP_Shading is Empty !!\n");
...
//info
	for(index=0;index<2;index++)
	{
		...
		case 0x01:
			...
			info_start_add = 0x75 + index * 5;
			gc8024_otp_info.module_info1 = gc8024_read_otp(info_start_add);
			gc8024_otp_info.module_info2 = gc8024_read_otp(info_start_add + 1); 
			gc8024_otp_info.module_info3 = gc8024_read_otp(info_start_add + 2);
			gc8024_otp_info.module_info4 = gc8024_read_otp(info_start_add + 3);
			gc8024_otp_info.module_info5 = gc8024_read_otp(info_start_add + 4);		
			break;
		case 0x02:
		case 0x03:	
	...
	LOG_INF("GC8024_OTP_INFO module_info1=0x%x\n",gc8024_otp_info.module_info1);
	LOG_INF("GC8024_OTP_INFO module_info2=0x%x\n",gc8024_otp_info.module_info2);
	LOG_INF("GC8024_OTP_INFO module_info3=0x%x\n",gc8024_otp_info.module_info3);
	LOG_INF("GC8024_OTP_INFO module_info4=0x%x\n",gc8024_otp_info.module_info4);
	LOG_INF("GC8024_OTP_INFO module_info5=0x%x\n",gc8024_otp_info.module_info5);		
}
...
static void gc8024_gcore_update_awb(void)
{
	...
	if(0x01==(gc8024_otp_info.wb_flag&0x01))
	{
...
static void capture_setting(kal_uint16 currefps)
{
	...
	/*window*/
	write_cmos_sensor(0x90,0x01);
	write_cmos_sensor(0x92,CapStartY);
	write_cmos_sensor(0x94,CapStartX);
