...
#endif
#if defined(S5K3M2TRULY_MIPI_RAW)
      {SENSOR_DRVNAME_S5K3M2TRULY_MIPI_RAW,
        {
          {SensorMCLK, Vol_High, 0},
          {DOVDD, Vol_1800, 0},
          {AVDD, Vol_2800, 0},
...
#endif
#if defined(S5K3L8ST_MIPI_RAW)
  {SENSOR_DRVNAME_S5K3L8ST_MIPI_RAW,
    {
      {SensorMCLK, Vol_High, 0},
      {AVDD, Vol_2800, 0},
      {DVDD, Vol_1200, 0},
...
#endif
#if defined(S5K3L8LC_MIPI_RAW)
  {SENSOR_DRVNAME_S5K3L8LC_MIPI_RAW,
    {
      {SensorMCLK, Vol_High, 0},
      {AVDD, Vol_2800, 0},
      {DVDD, Vol_1200, 0},
...
#endif
#if defined(S5K4H8ST_MIPI_RAW)
      {SENSOR_DRVNAME_S5K4H8ST_MIPI_RAW,
      {
       {SensorMCLK, Vol_High, 0},
       {DOVDD, Vol_1800, 0},
       {AVDD, Vol_2800, 0},
...
#endif
#if defined(IMX219CMK_MIPI_RAW)
	  {SENSOR_DRVNAME_IMX219CMK_MIPI_RAW,
	  {
	   {SensorMCLK, Vol_High, 0},
	   {PDN, Vol_Low, 5},
	   {RST, Vol_Low, 5},
...
#endif
#if defined(OV8858TRULY_MIPI_RAW)
  {SENSOR_DRVNAME_OV8858TRULY_MIPI_RAW,
    {
      {SensorMCLK, Vol_High, 0},
      {PDN, Vol_Low, 0},
      {RST, Vol_Low, 0},
...
#endif
#if defined(GC5034_MIPI_RAW)
	  {SENSOR_DRVNAME_GC5034_MIPI_RAW,
	  {
	   {SensorMCLK, Vol_High, 10},
	   {PDN, Vol_Low, 5},
	   {RST, Vol_Low, 5},
...
#endif
#if defined(GC5025SUB_MIPI_RAW)
	  {SENSOR_DRVNAME_GC5025SUB_MIPI_RAW,
	  {
	   {SensorMCLK, Vol_High, 10},
	   {PDN, Vol_Low, 5},
	   {RST, Vol_Low, 5},
...
#endif
#if defined(OV9760SUB_MIPI_RAW)
  {SENSOR_DRVNAME_OV9760SUB_MIPI_RAW,
    {
      {SensorMCLK,Vol_High, 0},
      {PDN,	Vol_Low, 0},
      {RST,	Vol_Low, 0},
...
int mtkcam_gpio_set(int PinIdx, int PwrType, int Val)
{
	...
	case RST:
		if (PinIdx == 0) {
			if (Val == 0 && !IS_ERR(cam0_rst_l))
				...
				PK_ERR("%s : pinctrl err, PinIdx %d, Val %d, MAIN RST\n", __func__,PinIdx ,Val);
		} else if (PinIdx == 1) {
			if (Val == 0 && !IS_ERR(cam1_rst_l))
				...
				PK_ERR("%s : pinctrl err, PinIdx %d, Val %d, SUB RST\n", __func__,PinIdx ,Val);
		} else {
				PK_ERR("%s : unsupportable PinIdx %d, Val %d RST\n", __func__,PinIdx ,Val);
		}
		break;
	case PDN:
		if (PinIdx == 0) {
			if (Val == 0 && !IS_ERR(cam0_pnd_l))
				...
				PK_ERR("%s : pinctrl err, PinIdx %d, Val %d, MAIN PDN\n", __func__,PinIdx ,Val);
		} else if (PinIdx == 1) {
			if (Val == 0 && !IS_ERR(cam1_pnd_l))
				...
				PK_ERR("%s : pinctrl err, PinIdx %d, Val %d, SUB PDN\n", __func__,PinIdx ,Val);
		} else {
				PK_ERR("%s : unsupportable PinIdx %d, Val %d, PDN\n", __func__,PinIdx ,Val);
		}
		break;
	case AVDD:
	case MAIN2_AVDD:
	...
	case AFVDD:
		if (Val == 0 && !IS_ERR(cam_af_pdn_l))
			...
			PK_ERR("%s : pinctrl err, PinIdx %d, Val %d, AFVDD\n", __func__,PinIdx ,Val);
        break;
	...
	if (pwInfo.PowerType == AVDD) {
		if (pinSetIdx == 2) {
			if (PowerCustList.PowerCustInfo[CUST_MAIN2_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				if (TRUE != _hwPowerDown(pwInfo.PowerType)) {
					PK_ERR("[CAMERA SENSOR] Fail to disble main2 analog power\n");
					return FALSE;
				}
			} else {
			...
			if (PowerCustList.PowerCustInfo[CUST_SUB_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				if (TRUE != _hwPowerDown(pwInfo.PowerType)) {
					PK_ERR("[CAMERA SENSOR] Fail to disable sub analog power\n");
					return FALSE;
				}
			...
			if (PowerCustList.PowerCustInfo[CUST_MAIN2_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				...
				if (PowerCustList.PowerCustInfo[CUST_SUB_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
					if (TRUE != _hwPowerDown(SUB_DVDD)) {
						PK_ERR("[CAMERA SENSOR] Fail to disbble sub digital power\n");
						return FALSE;
					}
				} else {
		...
		if (PowerCustList.PowerCustInfo[CUST_DOVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
			...
			if (mtkcam_gpio_set(pinSetIdx, DOVDD, 1-PowerCustList.PowerCustInfo[CUST_DOVDD].Voltage)) {
				PK_ERR("[CAMERA CUST_DODD] set gpio failed!!\n");/* 1-voltage for reverse*/
			}
		}
	} else if (pwInfo.PowerType == AFVDD) {
		if (PowerCustList.PowerCustInfo[CUST_AFVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
            ...
            if (mtkcam_gpio_set(pinSetIdx, AFVDD, Vol_Low)) {
                PK_ERR("[CAMERA CUST_AF AFVDD] set gpio failed!!\n");
            }
		} else {
		       ...
		       char *mode_name)
{
	...
	if (On) {
	    ...
	    /* power OFF */
		PK_INFO("PowerOFF: SensorName=%s pinSetIdx=%d, sensorIdx:%d\n",currSensorName, pinSetIdx, SensorIdx);
