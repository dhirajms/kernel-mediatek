...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC2385_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
#define ANALOG_GAIN_8 756  // 11.81x
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x09,0x00); //row start[10:8]
	write_cmos_sensor(0x0a,0x00); //row start[7:0]
	write_cmos_sensor(0x0b,0x00); //col start[10:8]
	write_cmos_sensor(0x0c,0x04); //04  col start[7:0]
	write_cmos_sensor(0x17,MIRROR);//d7
	write_cmos_sensor(0x18,0x02);
	write_cmos_sensor(0x19,0x17);
	//write_cmos_sensor(0x1a,0x18);
	write_cmos_sensor(0x1c,0x10);
	write_cmos_sensor(0x20,0x71);//add spring 20170213
	write_cmos_sensor(0x22,0xa1);//add spring 20170213
	write_cmos_sensor(0x2f,0x14);
	write_cmos_sensor(0x3f,0x40);
	write_cmos_sensor(0xcd,0x94); 
	write_cmos_sensor(0xce,0x45);//43 
	write_cmos_sensor(0xd1,0x0c); 
	write_cmos_sensor(0xe3,0x1b);
	write_cmos_sensor(0xe4,0xf8);
	write_cmos_sensor(0xd7,0x9b);//txlow=-1.1V
	write_cmos_sensor(0xd8,0x99);//9b rsgh=2.88V vref=3.08
	write_cmos_sensor(0xda,0x3b);//rsth=vref
	write_cmos_sensor(0xd9,0xb5);//d5 txh=3.2V rsgl=0.6V
	write_cmos_sensor(0xdb,0x75);//Vpix=2.8V
	write_cmos_sensor(0x29,0x20);//EQP
	write_cmos_sensor(0x21,0x38);//EQC
	write_cmos_sensor(0xb0,0x46);//44   global gain
	...
	write_cmos_sensor(0x94,STARTX); //08->00->06 spring 20170213
	//write_cmos_sensor(0x95,0x04); //height 1200
	//write_cmos_sensor(0x96,0xb0);
	//write_cmos_sensor(0x97,0x06); //width 1600
	//write_cmos_sensor(0x98,0x40);
	
	/*mipi set*/
	write_cmos_sensor(0xfe,0x00);
	write_cmos_sensor(0xed,0x00);//[7]lane_ena [6]NA [5]ULP_mode [4]MIPI_ena[3]NA [2]RAW8 [1]line_sync_mode
	write_cmos_sensor(0xfe,0x03);
	write_cmos_sensor(0x01,0x03); //03? //DPHY_analog_mode1 [1]phy_lane0_en [0]phy_clk_en
	write_cmos_sensor(0x02,0x82);
	write_cmos_sensor(0x03,0xe0); //60
	write_cmos_sensor(0x04,0x04);//fifo_prog_full_level
	...
	write_cmos_sensor(0x15,0x00);//02->00 spring 20170213
	write_cmos_sensor(0x1b,0x10);//fifo2_prog_full_level
	write_cmos_sensor(0x1c,0x10);//fifo2_push_prog_full_level
