...
#define MTKCAM_USING_CCF      /* it's defined in Makefile.custom */
...
#include <sync_write.h>	/* For mt65xx_reg_sync_writel(). */
...
struct clk *g_ispclk_larb2_smi;
struct clk *g_ispclk_cam_smi;
struct clk *g_ispclk_cam_cam;
struct clk *g_ispclk_sen_tg;
struct clk *g_ispclk_sen_cam;
struct clk *g_ispclk_cam_sv;
struct clk *g_ispclk_fd;
...
do { \
	...
	Out = _PASS1;\
	LOG_WRN("[DMA_TRANS] Warning: invalid dma(%d)\n", dma);\
	} \
} while (0)
...
*
********************************************************************************/
static inline MUINT32 ISP_GetIRQState(eISPIrq eIrq, MUINT32 type, MUINT32 userNumber,
					MUINT32 stus)
{
	MUINT32 ret;
	unsigned long flags;
...
MBOOL ISP_chkModuleSetting(void)
{
    ...
    if (cam_tg1_vf_con & 0x01)
    {
        ...
        grab_width = ((cam_tg1_sen_grab_pxl >>16) & 0x7fff) - (cam_tg1_sen_grab_pxl & 0x7fff);
        grab_height = ((cam_tg1_sen_grab_lin >>16) & 0x1fff) - (cam_tg1_sen_grab_lin & 0x1fff);
        ...
        {
            //Error
            LOG_INF("HwRWCtrl:: Flicker Error: The window size must be multiples of //2. horizontally and vertically!!");
            LOG_INF("HwRWCtrl:: Flicker Error: CAM_FLK_SIZE.FLK_SIZE_X(%d) and CAM_FLK_SIZE.FLK_SIZE_Y(%d) value can't be 0!!", FLK_SIZE_X, FLK_SIZE_Y);
        }
        if (MTRUE == bmx_enable)
        {
        ...
        {
            //Error
            LOG_INF("HwRWCtrl:: Flicker Error: bmx_enable(%d), sgg_sel(%d), grab_width(%d), grab_height(%d), bmx_width(%d), bmx_height(%d)!!", bmx_enable, sgg_sel, grab_width, grab_height, bmx_width, bmx_height);
            LOG_INF("HwRWCtrl:: Flicker Error: (CAM_FLK_NUM.FLK_NUM_X.value(%d) * CAM_FLK_SIZE.FLK_SIZE_X.value(%d)) + CAM_FLK_OFST.FLK_OFST_X.value(%d) <= sggmux_h_size.value(%d)!!",
            FLK_NUM_X, FLK_SIZE_X, FLK_OFST_X, sggmux_h_size);
            LOG_INF("HwRWCtrl:: Flicker Error: (CAM_FLK_NUM.FLK_NUM_Y.value(%d) * CAM_FLK_SIZE.FLK_SIZE_Y.value(%d)) + CAM_FLK_OFST.FLK_OFST_Y.value(%d) <= sggmux_v_size.value(%d)!!",
            FLK_NUM_Y, FLK_SIZE_Y, FLK_OFST_Y, sggmux_v_size);
        }
        //
        ...
        {
            //Error
            LOG_INF("HwRWCtrl:: Flicker Error: flko_xs(%d) must be equal ((CAM_FLK_NUM.FLK_NUM_X.value(%d) * CAM_FLK_NUM.FLK_NUM_Y.value(%d) * CAM_FLK_SIZE.FLK_SIZE_Y.value(%d) * 2) - 1)!!",
        ...
        if ( (AF_WIN_WD < 8) || (AF_FLO_WD_1 < 8) ||  (AF_FLO_WD_2 < 8)  ||  (AF_FLO_WD_3 < 8)
              || (AF_WIN_HT < 4) || (AF_FLO_HT_1 < 4) ||  (AF_FLO_HT_2 < 4)  ||  (AF_FLO_HT_3 < 4) )
        {
            LOG_INF("HwRWCtrl:: AF Error: The min horizontal window size is 8. (AF_WIN_WD(%d), AF_FLO_WD_1(%d), AF_FLO_WD_2(%d), AF_FLO_WD_3(%d))!!",
                AF_WIN_WD, AF_FLO_WD_1, AF_FLO_WD_2, AF_FLO_WD_3);
            LOG_INF("HwRWCtrl:: AF Error: The min vertical window size is 4. (AF_WIN_HT(%d), AF_FLO_HT_1(%d), AF_FLO_HT_2(%d), AF_FLO_HT_3(%d))!!",
        ...
        {
            LOG_INF("HwRWCtrl:: AF Error: The horizontal window start point and size must be multiples of 2 !!");
            LOG_INF("HwRWCtrl:: AF Error: AF_WINX_0(%d)/AF_WINX_1(%d)/AF_WINX_2(%d)/AF_WINX_3(%d)/AF_WINX_4(%d)/AF_WINX_5(%d)!!",
        ...
        if ( ((AF_FLO_WINX_1%2)!=0) || ((AF_FLO_WINX_2%2)!=0) || ((AF_FLO_WINX_3%2)!=0)
            || ((AF_FLO_WINY_1%2)!=0) || ((AF_FLO_WINY_2%2)!=0) || ((AF_FLO_WINY_3%2)!=0)
            || ((AF_FLO_WD_1%2)!=0) || ((AF_FLO_WD_2%2)!=0) || ((AF_FLO_WD_3%2)!=0)
            || ((AF_FLO_HT_1%2)!=0) || ((AF_FLO_HT_2%2)!=0) || ((AF_FLO_HT_3%2)!=0) )
        {
            LOG_INF("HwRWCtrl:: AF Error: Horizontal start position of floating window must be multiples of 2!!");
            LOG_INF("HwRWCtrl:: AF Error: Horizontal size of floating window must be multiples of 2!!");
            LOG_INF("HwRWCtrl:: AF Error: AF_FLO_WINX_1(%d)/AF_FLO_WINX_2(%d)/AF_FLO_WINX_3(%d)!!",
                AF_FLO_WINX_1, AF_FLO_WINX_2, AF_FLO_WINX_3);
            LOG_INF("HwRWCtrl:: AF Error: AF_FLO_WD_1(%d)/AF_FLO_WD_2(%d)/AF_FLO_WD_3(%d)!!",
                AF_FLO_WD_1, AF_FLO_WD_2, AF_FLO_WD_3);
            LOG_INF("HwRWCtrl:: AF Error: AF_FLO_HT_1(%d)/AF_FLO_HT_2(%d)/AF_FLO_HT_3(%d)!!",
        ...
        {
            //Error
            LOG_INF("HwRWCtrl:: AF Error: bmx_enable(%d), sgg_sel(%d), bmx_width(%d), grab_width(%d)!!", bmx_enable, sgg_sel, bmx_width, grab_width);
            LOG_INF("HwRWCtrl:: AF Error: AF_IMAGE_WD(%d) must be the same as input frame width(%d)!!", AF_IMAGE_WD, AF_InputWidth);
        }
        ...
        if ( (AF_WINX_0 < 16) || (AF_LeftOverWidth < 16) )
        {
            ...
            {
                LOG_INF("HwRWCtrl:: AF Error: minimum horizontal size should be larger than 24*(1<<AF_DECI_1) for non-zero 24-tap FIR statistics result!!");
                LOG_INF("HwRWCtrl:: AF Error: AF_WINX_0(%d), AF_DECI(%d), AF_LeftOverWidth(%d), AF_IMAGE_WD(%d), AF_WIN_WD(%d)!!",AF_WINX_0, AF_DECI, AF_LeftOverWidth, AF_IMAGE_WD, AF_WIN_WD);
            }
        }
        ...
        {
            //Error
            LOG_INF("Error HwRWCtrl:: bmx_enable(%d), bmx_width(%d), bmx_height(%d), grab_width(%d), grab_height(%d)!!", bmx_enable, bmx_width, bmx_height, grab_width, grab_height);
            LOG_INF("Error HwRWCtrl:: input frame width(%d) >= AWB_W_HNUM(%d) * AWB_W_HPIT(%d) + AWB_W_HORG(%d) !!", AAO_InWidth, AWB_W_HNUM, AWB_W_HPIT, AWB_W_HORG);
        }
        if (AAO_InHeight < (AWB_W_VNUM * AWB_W_VPIT + AWB_W_VORG))
        {
            //Error
            LOG_INF("Error HwRWCtrl:: bmx_enable(%d), bmx_width(%d), bmx_height(%d), grab_width(%d), grab_height(%d)!!", bmx_enable, bmx_width, bmx_height, grab_width, grab_height);
            LOG_INF("Error HwRWCtrl:: input frame height(%d) >= AWB_W_VNUM(%d) * AWB_W_VPIT(%d) + AWB_W_VORG(%d) !!", AAO_InHeight, AWB_W_VNUM, AWB_W_VPIT, AWB_W_VORG);
        }
        ...
        {
            //Error
            LOG_INF("EIS Error, 8. EIS_MB_INTERVAL.EIS_WIN_HSIZE >= (EIS_PREP_ME_CTRL1.EIS_NUM_HRP+1)*16+2!!, EIS_WIN_HSIZE:%d, EIS_NUM_HRP:%d", EIS_WIN_HSIZE, EIS_NUM_HRP);
            ...
            (((EIS_RP_VOFST + ((EIS_WIN_VSIZE-1)* (EIS_NUM_VWIN-1)) + (EIS_NUM_VRP<<4))*EIS_OP_VERT) >= EIS_IMG_HEIGHT) )
        {
            ...
            LOG_INF("EIS Error, 10. (EIS_MB_OFFSET.EIS_RP_HOFST(%d) + ((EIS_MB_INTERVAL.EIS_WIN_HSIZE(%d)-1)*(EIS_PREP_ME_CTRL1.EIS_NUM_HWIN(%d)-1))+EIS_PREP_ME_CTRL1.EIS_NUM_HRP(%d)*16)*EIS_PREP_ME_CTRL1.EIS_OP_HORI(%d) < EIS_IMAGE_CTRL.WIDTH(%d)!!",
                EIS_RP_HOFST, EIS_WIN_HSIZE, EIS_NUM_HWIN, EIS_NUM_HRP, EIS_OP_HORI, EIS_IMG_WIDTH);
            LOG_INF("EIS Error, 10. (EIS_MB_OFFSET.EIS_RP_VOFST(%d) + ((EIS_MB_INTERVAL.EIS_WIN_VSIZE(%d)-1)*(EIS_PREP_ME_CTRL1.EIS_NUM_VWIN(%d)-1))+EIS_PREP_ME_CTRL1.EIS_NUM_VRP(%d)*16)*EIS_PREP_ME_CTRL1.EIS_OP_VERT(%d) < EIS_IMG_HEIGHT.WIDTH(%d)!!",
        ...
        switch (eis_sel)
        {
            case 0:
                ...
                {
                    if ( (grab_width != EIS_IMG_WIDTH) || (grab_height != EIS_IMG_HEIGHT) )
                    {
                        //Error
                        bError = MTRUE;
                    }                                       
            ...
            case 1:
                ...
                {
                    //bmx_width
                    if ( (bmx_width/2 != EIS_IMG_WIDTH ) || (bmx_height != EIS_IMG_HEIGHT) )
                    {
                        //Error
                        bError = MTRUE;
                    }
            ...
            case 2:
                if (MTRUE == bmx_enable)
                {
                    if ( (EIS_IMG_WIDTH != rrz_out_width/2) || (EIS_IMG_HEIGHT != rrz_out_height) )
                    {
                        //Error
                        bError = MTRUE;
                    }
                ...
                {
                    if ( (EIS_IMG_WIDTH != rrz_out_width) || (EIS_IMG_HEIGHT != rrz_out_height) )
                    {
                        //Error
                        bError = MTRUE;
                    }
        ...
        if (MTRUE == bError)
        {
            LOG_INF("EIS Error, 4. EIS_IMAGE_CTRL.WIDTH = EIS input image width but if (two_pix mode) EIS_IMAGE_CTRL.WIDTH = input image width/2!!\n");
            LOG_INF("EIS Error, 5. EIS_IMAGE_CTRL.HEIGHT != EIS input image height!!\n");
            LOG_INF("eis_sel:%d, scenario:%d, sgg_sel:%d, twin_mode:%d",eis_sel, scenario, sgg_sel, bmx_enable);
            LOG_INF("EIS_IMG_WIDTH:%d, EIS_IMG_HEIGHT:%d, rrz_out_width:%d, rrz_out_height:%d",EIS_IMG_WIDTH, EIS_IMG_HEIGHT, rrz_out_width, rrz_out_height);
    ...
    if (cam_tg2_vf_con & 0x01)
    {
        ...
        if ( (AF_WIN_WD < 8) || (AF_FLO_WD_1 < 8) ||  (AF_FLO_WD_2 < 8)  ||  (AF_FLO_WD_3 < 8)
              || (AF_WIN_HT < 4) || (AF_FLO_HT_1 < 4) ||  (AF_FLO_HT_2 < 4)  ||  (AF_FLO_HT_3 < 4) )
        {
            LOG_INF("HwRWCtrl:: AF_D Error: The min horizontal window size is 8. (AF_WIN_WD(%d), AF_FLO_WD_1(%d), AF_FLO_WD_2(%d), AF_FLO_WD_3(%d))!!",
                AF_WIN_WD, AF_FLO_WD_1, AF_FLO_WD_2, AF_FLO_WD_3);
            LOG_INF("HwRWCtrl:: AF_D Error: The min vertical window size is 4. (AF_WIN_HT(%d), AF_FLO_HT_1(%d), AF_FLO_HT_2(%d), AF_FLO_HT_3(%d))!!",
                AF_WIN_HT, AF_FLO_HT_1, AF_FLO_HT_2, AF_FLO_HT_3);
        }
        //3. The horizontal window start point and size must be multiples of 2 (AF_WINX_x/AF_FLO_WINX_x/AF_WIN_WD/AF_FLO_WD_x)
        if ( ((AF_WINX_0%2)!=0) || ((AF_WINX_1%2)!=0) || ((AF_WINX_2%2)!=0)
            ...
            || ((AF_WINY_3%2)!=0) || ((AF_WINY_4%2)!=0) || ((AF_WINY_5%2)!=0) )
        {
            LOG_INF("HwRWCtrl:: AF_D Error: The horizontal window start point and size must be multiples of 2 !!");
            LOG_INF("HwRWCtrl:: AF_D Error: AF_WINX_0(%d)/AF_WINX_1(%d)/AF_WINX_2(%d)/AF_WINX_3(%d)/AF_WINX_4(%d)/AF_WINX_5(%d)!!",
                AF_WINX_0, AF_WINX_1, AF_WINX_2, AF_WINX_3, AF_WINX_4, AF_WINX_5);
        ...
        if ( ((AF_FLO_WINX_1%2)!=0) || ((AF_FLO_WINX_2%2)!=0) || ((AF_FLO_WINX_3%2)!=0)
            ...
            || ((AF_FLO_HT_1%2)!=0) || ((AF_FLO_HT_2%2)!=0) || ((AF_FLO_HT_3%2)!=0) )
        {
            LOG_INF("HwRWCtrl:: AF_D Error: Horizontal start position of floating window must be multiples of 2!!");
            LOG_INF("HwRWCtrl:: AF_D Error: Horizontal size of floating window must be multiples of 2!!");
            LOG_INF("HwRWCtrl:: AF_D Error: AF_FLO_WINX_1(%d)/AF_FLO_WINX_2(%d)/AF_FLO_WINX_3(%d)!!",
                AF_FLO_WINX_1, AF_FLO_WINX_2, AF_FLO_WINX_3);
            LOG_INF("HwRWCtrl:: AF_D Error: AF_FLO_WD_1(%d)/AF_FLO_WD_2(%d)/AF_FLO_WD_3(%d)!!",
                AF_FLO_WD_1, AF_FLO_WD_2, AF_FLO_WD_3);
            LOG_INF("HwRWCtrl:: AF_D Error: AF_FLO_HT_1(%d)/AF_FLO_HT_2(%d)/AF_FLO_HT_3(%d)!!",
                AF_FLO_HT_1, AF_FLO_HT_2, AF_FLO_HT_3);
        }
        //5. Horizontal size of each normal window is limited to 510*2 (AF_WIN_WD)
        //6. Vertical size of each normal window is limited to 511*2 (AF_WIN_HT)
        ...
        {
            //Error
            LOG_INF("HwRWCtrl:: AF_D Error: bmx_enable(%d), sgg_sel(%d), bmx_width(%d), grab_width(%d)!!", bmx_enable, sgg_sel, bmx_width, grab_width);
            LOG_INF("HwRWCtrl:: AF_D Error: AF_IMAGE_WD(%d) must be the same as input frame width(%d)!!", AF_IMAGE_WD, AF_InputWidth);
        }
        //16. "CAM_AF_WINX_x.AF_WINX_x + CAM_AF_SIZE.AF_WIN_WD <= input frame width" for a valid window
        //17. "CAM_AF_WINY_x.AF_WINY_x + CAM_AF_SIZE.AF_WIN_HT <= input frame height" for a valid window
        ...
        if ( (AF_WINX_0 < 16) || (AF_LeftOverWidth < 16) )
        {
            if ( AF_WIN_WD < (24*(1<<AF_DECI)) )
            {
                LOG_INF("HwRWCtrl:: AF_D Error: minimum horizontal size should be larger than 24*(1<<AF_DECI_1) for non-zero 24-tap FIR statistics result!!");
                LOG_INF("HwRWCtrl:: AF_D Error: AF_WINX_0(%d), AF_DECI(%d), AF_LeftOverWidth(%d), AF_IMAGE_WD(%d), AF_WIN_WD(%d)!!",AF_WINX_0, AF_DECI, AF_LeftOverWidth, AF_IMAGE_WD, AF_WIN_WD);
            }
        }
        ...
        {
            //Error
            LOG_INF("Error HwRWCtrl:: AWB_D bmx_enable(%d), bmx_width(%d), bmx_height(%d), grab_width(%d), grab_height(%d)!!", bmx_enable, bmx_width, bmx_height, grab_width, grab_height);
            LOG_INF("Error HwRWCtrl:: AWB_D input frame width(%d) >= AWB_W_HNUM(%d) * AWB_W_HPIT(%d) + AWB_W_HORG(%d) !!", AAO_InWidth, AWB_W_HNUM, AWB_W_HPIT, AWB_W_HORG);
        }
        if (AAO_InHeight < (AWB_W_VNUM * AWB_W_VPIT + AWB_W_VORG))
        {
            //Error
            LOG_INF("Error HwRWCtrl:: AWB_D bmx_enable(%d), bmx_width(%d), bmx_height(%d), grab_width(%d), grab_height(%d)!!", bmx_enable, bmx_width, bmx_height, grab_width, grab_height);
            LOG_INF("Error HwRWCtrl:: AWB_D input frame height(%d) >= AWB_W_VNUM(%d) * AWB_W_VPIT(%d) + AWB_W_VORG(%d) !!", AAO_InHeight, AWB_W_VNUM, AWB_W_VPIT, AWB_W_VORG);
        }
...
static inline void Get_ccf_clock(struct platform_device *pDev)
{
	...
	g_ispclk_cam_cam = devm_clk_get(&pDev->dev, "IMG_CAM_CAM");
	BUG_ON(IS_ERR(g_ispclk_cam_cam));
	g_ispclk_sen_tg = devm_clk_get(&pDev->dev, "IMG_SEN_TG");
	BUG_ON(IS_ERR(g_ispclk_sen_tg));
	g_ispclk_sen_cam = devm_clk_get(&pDev->dev, "IMG_SEN_CAM");
	BUG_ON(IS_ERR(g_ispclk_sen_cam));
	g_ispclk_cam_sv = devm_clk_get(&pDev->dev, "IMG_CAM_SV");
	BUG_ON(IS_ERR(g_ispclk_cam_sv));
	g_ispclk_fd = devm_clk_get(&pDev->dev, "IMG_FD");
	BUG_ON(IS_ERR(g_ispclk_fd));
	g_ispclk_smi_common = devm_clk_get(&pDev->dev, "MM_SMI_COMMON");
...
/*	BUG_ON(IS_ERR(g_pmdev_disp)); pm_runtime_get_sync(g_pmdev_disp); */
	...
	BUG_ON(IS_ERR(g_ispclk_cam_smi));
	clk_prepare(g_ispclk_cam_smi);
	BUG_ON(IS_ERR(g_ispclk_cam_cam));
	clk_prepare(g_ispclk_cam_cam);
	BUG_ON(IS_ERR(g_ispclk_sen_tg));
	clk_prepare(g_ispclk_sen_tg);
	BUG_ON(IS_ERR(g_ispclk_sen_cam));
...
static inline void Enable_ccf_clock(void)
{
	...
	BUG_ON(IS_ERR(g_ispclk_cam_smi));
	clk_enable(g_ispclk_cam_smi);
	BUG_ON(IS_ERR(g_ispclk_cam_cam));
	clk_enable(g_ispclk_cam_cam);
	BUG_ON(IS_ERR(g_ispclk_sen_tg));
	clk_enable(g_ispclk_sen_tg);
	BUG_ON(IS_ERR(g_ispclk_sen_cam));
...
{
	BUG_ON(IS_ERR(g_ispclk_larb2_smi));
	clk_disable(g_ispclk_larb2_smi);
	BUG_ON(IS_ERR(g_ispclk_cam_smi));
	clk_disable(g_ispclk_cam_smi);
	BUG_ON(IS_ERR(g_ispclk_cam_cam));
	clk_disable(g_ispclk_cam_cam);
	BUG_ON(IS_ERR(g_ispclk_sen_tg));
	clk_disable(g_ispclk_sen_tg);
	BUG_ON(IS_ERR(g_ispclk_sen_cam));
...
{
	BUG_ON(IS_ERR(g_ispclk_larb2_smi));
	clk_unprepare(g_ispclk_larb2_smi);
	BUG_ON(IS_ERR(g_ispclk_cam_smi));
	clk_unprepare(g_ispclk_cam_smi);
	BUG_ON(IS_ERR(g_ispclk_cam_cam));
	clk_unprepare(g_ispclk_cam_cam);
	BUG_ON(IS_ERR(g_ispclk_sen_tg));
	clk_unprepare(g_ispclk_sen_tg);
	BUG_ON(IS_ERR(g_ispclk_sen_cam));
...
static volatile int sof_pass1done[2] = { 0, 0 };	/* record lost p1_done or not, 1 for lost p1_done. 0 for normal , 2 for last working buffer. */
static volatile MUINT32 lost_pass1_done_cnt =0;
static volatile MUINT32 lost_pass1_d_done_cnt =0;
...
static void ISP_FBC_DUMP(MUINT32 dma_id, MUINT32 VF_1, MUINT32 VF_2, MUINT32 VF_3, MUINT32 VF_4)
{
	...
	if (VF_1) {
		...
		{
			str[0] = '\0';
			LOG_INF("current fillled buffer(buf cnt:0x%x):\n",
...
static MINT32 ISP_RTBC_DEQUE(MINT32 dma, ISP_DEQUE_BUF_INFO_STRUCT * pdeque_buf)
{
	...
	if (0 == pdeque_buf->count) {
		...
		for (i = 0; i <= pstRTBuf->ring_buf[rt_dma].total_count - 1; i++) {
			LOG_DBG("[rtbc][DEQUE]Buf List:%d/%d/0x%x/%lld/0x%x/%d/  ",
				i,
	...
	if (pdeque_buf->count) {
		...
		for (i = 0; i < pdeque_buf->count; i++) {
			...
			if (IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
				LOG_DBG
				    ("[rtbc][DEQUE]:index(%d)/PA(0x%x)/memID(%d)/size(0x%x)/VA(%lld)",
...
static long ISP_Buf_CTRL_FUNC(unsigned long Param)
{
	...
	if (copy_from_user(&rt_buf_ctrl, (void __user *)Param, sizeof(ISP_BUFFER_CTRL_STRUCT)) == 0) {
		...
		case ISP_RT_BUF_CTRL_EXCHANGE_ENQUE: /* add for build pass */
			/*  */
					...
					if (_bFlag == MTRUE) {
						...
						if (0 != rt_buf_ctrl.ex_data_ptr) {
							...
							if (copy_from_user(&deque_buf.data[0], (void __user *)rt_buf_ctrl.ex_data_ptr, sizeof(ISP_RT_BUF_INFO_STRUCT)) == 0) {
									...
									/*  */
									{
										...
										if (pstRTBuf->ring_buf[rt_dma].empty_count < pstRTBuf->ring_buf[rt_dma].total_count)
											...
											LOG_ERR("[rtbc]dma(%d),PA(0x%x),over enque_1",
											     rt_dma, pstRTBuf->ring_buf[rt_dma].data[i].base_pAddr);
											return
											    -EFAULT;
										}
								...
								spin_unlock_irqrestore(&(IspInfo.SpinLockIrq[irqT_Lock]), flags);
								LOG_ERR
								    ("[rtbc][ENQUE_ext]:copy_from_user fail, dst_buf(%p), user_buf(%p)",
								     (void *)&deque_buf.data[0],
									...
									if (1) {
										if (rt_buf_info.bufIdx != pstRTBuf->ring_buf[rt_dma].data[i].bufIdx) {
											LOG_ERR("[rtbc][replace2]error: BufIdx MisMatch. 0x%x/0x%x",
											     rt_buf_info.bufIdx, pstRTBuf->ring_buf[rt_dma].data[i].bufIdx);
										}
									}
#else
									pstRTBuf->ring_buf[rt_dma].empty_count++;
							...
							if (i == ISP_RT_BUF_SIZE) {
								...
								for (x = 0; x < ISP_RT_BUF_SIZE; x++) {
									LOG_DBG
									    ("[rtbc]dma(%d),idx(%d) PA(0x%x) VA(%lld)",
									     rt_dma, x,
									     pstRTBuf->ring_buf[rt_dma].data[x].base_pAddr,
		...
		case ISP_RT_BUF_CTRL_DEQUE:
			...
			if ((reg_val & 0x01) || (reg_val2 & 0x01) || (camsv_reg_cal[0] & 0x01) || (camsv_reg_cal[1] & 0x01)) {
				if (MTRUE == _bFlag) {
					...
					if (IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
						LOG_DBG
						    ("[rtbc][DEQUE](%d):d(%d)/id(0x%x)/bs(0x%x)/va(%lld)/pa(0x%x)/t(%d.%d)/img(%d,%d,%d,%d,%d,%d,%d,%d,%d)/m(0x%x)/fc(%d)/rrz(%d,%d,%d,%d,%d,%d),dmao(%d,%d,%d,%d),lm#(0x%x)",
		...
		case ISP_RT_BUF_CTRL_CLEAR:
			...
			case _imgo_:
			case _rrzo_:
				memset((void *)&IspInfo.IrqInfo.LastestSigTime_usec[ISP_IRQ_TYPE_INT_P1_ST], 0, sizeof(MUINT32)*32);
				memset((void *)&IspInfo.IrqInfo.LastestSigTime_sec[ISP_IRQ_TYPE_INT_P1_ST], 0, sizeof(MUINT32)*32);
        memset((void *)&IspInfo.IrqInfo.Eismeta[ISP_IRQ_TYPE_INT_P1_ST],0,sizeof(MUINT32)*EISMETA_RINGSIZE);
        gEismetaRIdx = 0;
        gEismetaWIdx = 0;
        gEismetaInSOF = 0;
      ...
      case _imgo_d_:
      case _rrzo_d_:
        memset((void *)&IspInfo.IrqInfo.LastestSigTime_usec[ISP_IRQ_TYPE_INT_P1_ST_D],0,sizeof(MUINT32)*32);
        memset((void *)&IspInfo.IrqInfo.LastestSigTime_sec[ISP_IRQ_TYPE_INT_P1_ST_D], 0, sizeof(MUINT32)*32);
			  memset((void *)&IspInfo.IrqInfo.Eismeta[ISP_IRQ_TYPE_INT_P1_ST_D],0,sizeof(MUINT32)*EISMETA_RINGSIZE);
        gEismetaRIdx_D = 0;
        gEismetaWIdx_D = 0;
        gEismetaInSOF_D = 0;
		...
		case ISP_RT_BUF_CTRL_DMA_EN:
			{
				...
				if (copy_from_user(array, (void __user *)rt_buf_ctrl.pExtend, sizeof(MUINT8) * _rt_dma_max_) == 0) {
					MUINT32 z;
					bRawEn = MFALSE;
					bRawDEn = MFALSE;
					for (z = 0; z < _rt_dma_max_; z++) {
...
static MINT32 ISP_SOF_Buf_Get(eISPIrq irqT, CQ_RTBC_FBC * pFbc, volatile MUINT32 *pCurr_pa, unsigned long long sec, unsigned long usec, MBOOL bDrop)
{
#if defined(_rtbc_use_cq0c_)
						...
						for (m = 0; m < ISP_RT_BUF_SIZE;) {
							LOG_ERR
							    ("dma_%d,cur_%d,bFilled_%d != EMPTY(%d %d %d %d)\n",
							     ...
							     pstRTBuf->ring_buf[i_dma].data[m].bFilled,
							     pstRTBuf->ring_buf[i_dma].data[m + 1].bFilled,
							     pstRTBuf->ring_buf[i_dma].data[m + 2].bFilled,
							     pstRTBuf->ring_buf[i_dma].data[m + 3].bFilled);
							m = m + 4;
						}
					}
					/* start + 1 */
	...
	case ISP_ED_BUFQUE_CTRL_WAIT_DEQUE:	/* a dequeue thread is waiting to do dequeue */
		...
		if (idx == -1) {
			...
			restTime = wait_event_interruptible_timeout(
						*((wait_queue_head_t *)&WaitQueueHead_EDBuf_WaitDeque),
      ...
      if (2 == lost_pass1_done_cnt)
      {
          ...
          for(k=0 ; k < pstRTBuf->ring_buf[rt_dma].total_count ; k++){
               if (ISP_RTBC_BUF_FILLED == pstRTBuf->ring_buf[rt_dma].data[k].bFilled)
               {
                   ...
                   {
                      IspInfo.IrqInfo.Status[j][ISP_IRQ_TYPE_INT_P1_ST] |= ISP_IRQ_P1_STATUS_PASS1_DON_ST;
                   }
                   lost_pass1_done_cnt = 0;
                   IRQ_LOG_KEEPER(_IRQ,m_CurrentPPB,_LOG_INF,"raw buf rdy but lost pass1 done, wakeup by sof!!");
                   break;
               }
          }
      }
      ...
      if (2 == lost_pass1_d_done_cnt)
      {
          ...
          for(k=0 ; k < pstRTBuf->ring_buf[rt_dma].total_count ; k++){
              if (ISP_RTBC_BUF_FILLED == pstRTBuf->ring_buf[rt_dma].data[k].bFilled)
              {
                  for(j=0;j<ISP_IRQ_USER_MAX;j++)
                  {
                      IspInfo.IrqInfo.Status[j][ISP_IRQ_TYPE_INT_P1_ST_D] |= ISP_IRQ_P1_STATUS_D_PASS1_DON_ST;
                  }
                  lost_pass1_d_done_cnt = 0;
                  IRQ_LOG_KEEPER(_IRQ,m_CurrentPPB,_LOG_INF,"raw-D buf rdy but lost pass1 done, wakeup by sof!!");
                  break;
              }
          }
      }
...
static MINT32 ISP_open(struct inode *pInode, struct file *pFile)
{
	...
	/*  */
#ifdef MTKCAM_USING_CCF
	/* Prepare clk MUST NOT be in the spinlock section, because it may sleep */
...
#ifdef MTKCAM_USING_CCF
	/* UnPrepare clk MUST NOT be in the spinlock section, because it may sleep */
	if (G_u4EnableClockCount == 0)
		Unprepare_ccf_clock();
#endif
...
static MINT32 mmap_kmem(struct file *filp, struct vm_area_struct *vma)
{
	...
	LOG_DBG("Vma->vm_pgoff(0x%lx),Vma->vm_start(0x%lx),Vma->vm_end(0x%lx),length(%ld)",
		vma->vm_pgoff, vma->vm_start, vma->vm_end, length);
	if (length > ISP_RTBUF_REG_RANGE) {
		LOG_ERR("mmap range error! : length(%ld),ISP_RTBUF_REG_RANGE(0x%x)!", length, ISP_RTBUF_REG_RANGE);
		return -EAGAIN;
	}
...
static MINT32 ISP_mmap(struct file *pFile, struct vm_area_struct *pVma)
{
	...
	if (pVma->vm_pgoff == RT_BUF_TBL_NPAGES) {
		...
		LOG_DBG("pVma->vm_pgoff(0x%lx),phy(0x%lx),pVmapVma->vm_start(0x%lx),pVma->vm_end(0x%lx),length(%ld)",
		...
		case IMGSYS_BASE_ADDR:	/* imgsys */
			if (length > ISP_REG_RANGE) {
				LOG_ERR("mmap range error : length(%ld),ISP_REG_RANGE(0x%x)!", length, ISP_REG_RANGE);
				return -EAGAIN;
			}
			break;
		case SENINF_BASE_ADDR:
			if (length > SENINF_REG_RANGE) {
				LOG_ERR("mmap range error : length(%ld),SENINF_REG_RANGE(0x%x)!", length, SENINF_REG_RANGE);
				return -EAGAIN;
			}
			break;
		case PLL_BASE_ADDR:
			if (length > PLL_RANGE) {
				LOG_ERR("mmap range error : length(%ld),PLL_RANGE(0x%x)!", length, PLL_RANGE);
				return -EAGAIN;
			}
			break;
		case MIPIRX_CONFIG_ADDR:
			if (length > MIPIRX_CONFIG_RANGE) {
				LOG_ERR("mmap range error : length(%ld),MIPIRX_CONFIG_RANGE(0x%x)!", length, MIPIRX_CONFIG_RANGE);
				return -EAGAIN;
			}
			break;
		case MIPIRX_ANALOG_ADDR:
			if (length > MIPIRX_ANALOG_RANGE) {
				LOG_ERR("mmap range error : length(%ld),MIPIRX_ANALOG_RANGE(0x%x)!", length, MIPIRX_ANALOG_RANGE);
				return -EAGAIN;
			}
			break;
		case GPIO_BASE_ADDR:
			if (length > GPIO_RANGE) {
				LOG_ERR("mmap range error : length(%ld),GPIO_RANGE(0x%x)!", length,	GPIO_RANGE);
				return -EAGAIN;
			}
			break;
		default:
...
{
	MINT32 Ret = 0;
    /* struct resource *pRes = NULL; */ /* Move below */
	MINT32 i = 0;
	MUINT8 n;
	int new_count;
#ifdef CONFIG_OF
...
/* Houston Add MTCMOS status checking ----- */
	...
	for (i = 0; i < ISP_CAM_IRQ_IDX_NUM; i++) {
		...
		if (ISP_CAM0_IRQ_IDX == i) {
			Ret = request_irq(cam_isp_dev->irq[i], (irq_handler_t) ISP_Irq_CAM, IRQF_TRIGGER_NONE, "ISP", NULL);	/* IRQF_TRIGGER_NONE dose not take effect here, real trigger mode set in dts file */
		} else if (ISP_CAMSV0_IRQ_IDX == i) {
			Ret = request_irq(cam_isp_dev->irq[i], (irq_handler_t) ISP_Irq_CAMSV, IRQF_TRIGGER_NONE, "ISP", NULL);	/* IRQF_TRIGGER_NONE dose not take effect here, real trigger mode set in dts file */
		} else if (ISP_CAMSV1_IRQ_IDX == i) {
			Ret = request_irq(cam_isp_dev->irq[i], (irq_handler_t) ISP_Irq_CAMSV2, IRQF_TRIGGER_NONE, "ISP", NULL);	/* IRQF_TRIGGER_NONE dose not take effect here, real trigger mode set in dts file */
		}
...
{
    /* Config m4u port, due to the m4u can't backup the labr0/labr1/labr2 table When spin_lock since two */
...
/*    port.ePortID = M4U_PORT_MDP_WROT;*/ /* M4U port Renaming to M4U_PORT_MDP_WROT0 in 8173 */
    ...
    /*port.ePortID = M4U_PORT_IMGO_S;*/	/* M4U port Renaming to M4U_PORT_IMGO_D in 8173 */
    port.ePortID = M4U_PORT_IMGO_D;
    m4u_config_port(&port);
...
*
********************************************************************************/
static ssize_t ISP_DumpRegToProc(struct file *file, char __user *pPage, size_t Count,
				 loff_t *ppos)
{
	char *p = pPage;
	off_t off = *ppos;
	...
	/*  */
	LOG_DBG("- E. pPage: %p. off: %d. Count: %d.", pPage, (unsigned int)off, (int)Count);
	/*  */
...
*
********************************************************************************/
static ssize_t ISP_RegDebug(struct file *pFile, const char __user *pBuffer, size_t Count,
				loff_t *ppos)
{
	char RegBuf[64];
	...
	LOG_DBG("- E. pFile: %p. pBuffer: %p. Count: %d.", (void *)pFile,
		(void *)pBuffer, (int)Count);
	/*	*/
	...
	if (sscanf(RegBuf, "%x %x", &Addr, &Data) == 2) {
		...
		LOG_ERR("Write => Addr: 0x%08lX, Write Data: 0x%08X. Read Data: 0x%08X.",
			ISP_ADDR_CAMINF + Addr, Data, ISP_RD32(ISP_ADDR_CAMINF + Addr));
	} else if (sscanf(RegBuf, "%x", &Addr) == 1) {
		LOG_ERR("Read => Addr: 0x%08lX, Read Data: 0x%08X.", ISP_ADDR_CAMINF + Addr,
			ISP_RD32(ISP_ADDR_CAMINF + Addr));
	}
	/*	*/
...
static ssize_t CAMIO_DumpRegToProc(struct file *file, char __user *pPage, size_t Count,
				   loff_t *ppos)
{
	char *p = pPage;
	off_t off = *ppos;
	...
	MINT32 ret = 0;
	/*  */
	LOG_DBG("- E. pPage: %p. off: %d. Count: %d.", (void *)pPage, (int)off, (int)Count);
	/*  */
	p += sprintf(p, "reg_0x%08lX = 0x%X\n", ISP_ADDR_CAMINF + proc_regOfst,
		     ISP_RD32(ISP_ADDR_CAMINF + proc_regOfst));
				  ...
				  loff_t *ppos)
{
	...
	if (sscanf(RegBuf, "%x %x", &Addr, &Data) == 2) {
		proc_regOfst = Addr;
		ISP_WR32(compat_ptr(ISP_GPIO_ADDR + Addr), Data);
		LOG_ERR("Write => Addr: 0x%08lX, Write Data: 0x%08X. Read Data: 0x%08X.",
			ISP_GPIO_ADDR + Addr, Data, ISP_RD32(ISP_GPIO_ADDR + Addr));
	} else if (sscanf(RegBuf, "%x", &Addr) == 1) {
		proc_regOfst = Addr;
		LOG_ERR("Read => Addr: 0x%08lX, Read Data: 0x%08X.", ISP_GPIO_ADDR + Addr,
...
/* FIX-ME: linux-3.10 procfs API changed */
	...
	/* 8173 M4U port naming is different from K2, but same port number */
	m4u_register_fault_callback(M4U_PORT_IMGO_D, ISP_M4U_TranslationFault_callback, NULL);
...
{
#if 0 /* Houston remvoe below to avoid build error: addressToDump[pos] , array subscript is above array bounds [-Werror=array-bounds] */
