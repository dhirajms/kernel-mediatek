...
{
	LOG_MSG("AllocatedTbl(0x%08X),EnableClk(%d)", (unsigned int)Sysram.AllocatedTbl, Sysram.EnableClk);
...
static int SYSRAM_mmap(struct file *pFile, struct vm_area_struct *pVma)
{
	...
	LOG_WRN
	    ("pVma->vm_pgoff(0x%lx),phy(0x%lx),pVmapVma->vm_start(0x%lx),pVma->vm_end(0x%lx),length(%ld)",
	...
	if ((length > ISP_VALID_REG_RANGE) || (pfn < IMGSYS_BASE_ADDR)
		...
		LOG_ERR("mmap range error : vm_start(0x%lx),vm_end(0x%lx),length(%ld),pfn(0x%x)!",
...
/* ------------------------------------------------------------------------------ */
static ssize_t SYSRAM_DumpLayoutToProc(struct file *file, char __user *pPage, size_t Count,
				       loff_t *ppos)
{
	off_t Off = *ppos;
	char *p = pPage;
...
/* ------------------------------------------------------------------------------ */
static ssize_t SYSRAM_ReadFlag(struct file *file, char __user *pPage, size_t Count, loff_t *ppos)
{
	char *p = pPage;
	MUINT32 len = 0;
	off_t Off = *ppos;
...
/* ------------------------------------------------------------------------------ */
static ssize_t SYSRAM_WriteFlag(struct file *pFile, const char __user *pBuffer, size_t Count,
				loff_t *ppos)
{
	char acBuf[32];
