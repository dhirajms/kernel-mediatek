...
#if 0    /* remove for regulator framework to control voltage */
	if (mt_gpufreq_ptpod_disable == true) {
		if (enable == 0) {
			pr_debug("mt_gpufreq_ptpod_disable == true\n");
...
void mt_gpufreq_disable_by_ptpod(void)
{
	...
	if (mt_gpufreq_ready == false) {
		pr_debug("mt_gpufreq_disable_by_ptpod: GPU DVFS not ready!\n");
		return;
	}
...
{
	if (mt_gpufreq_ready == false) {
		pr_debug("mt_gpufreq_get_frequency_by_level: GPU DVFS not ready!\n");
		return -ENOSYS;
	}

	if (num < mt_gpufreqs_num) {
		pr_debug("mt_gpufreq_get_frequency_by_level:num = %d, frequency= %d\n",
			num, mt_gpufreqs[num].gpufreq_khz);

		return mt_gpufreqs[num].gpufreq_khz;
	}

	pr_debug("mt_gpufreq_get_frequency_by_level:num = %d, NOT found! return 0!\n", num);
	return 0;
}
...
**********************************/
#ifdef CONFIG_HAS_EARLYSUSPEND
static void mt_gpufreq_early_suspend(struct early_suspend *h)
{
...
********************************/
static void mt_gpufreq_late_resume(struct early_suspend *h)
{
