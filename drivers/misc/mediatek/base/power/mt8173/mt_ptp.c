...
#define ptp_notice(fmt, args...)	pr_notice("[PTP] " fmt, ##args)
...
#ifdef CONFIG_PTP_VCORE_AO
			.det_id = PTP_DET_VCORE_AO,
#else
...
#ifdef CONFIG_PTP_VCORE_AO
static struct ptp_det_ops vcore_ao_det_ops = {
	.get_volt = get_volt_vcore,
	/* FIXME: set_volt API */
	.switch_bank = switch_to_vcore_ao,
...
#ifdef CONFIG_PTP_VCORE_AO
	[PTP_DET_VCORE_AO] = {
		.name = __stringify(PTP_DET_VCORE_AO),
		.ops = &vcore_ao_det_ops,
		.ctrl_id = PTP_CTRL_VCORE,
		.features = FEA_INIT01 | FEA_INIT02,
		.VBOOT = MV_TO_VAL(1050),	/* 1.05v: 0x38 */
		},
#endif
	[PTP_DET_VCORE_PDN] = {
...
void disble_vcore_tuning_machenism(void)
{
	ptp_detectors[ptp_ctrls[PTP_CTRL_VCORE].det_id].features &= (~FEA_MON);
	ptp_detectors[ptp_ctrls[PTP_CTRL_VCORE].det_id].ops->set_volt = NULL;
}
...
static int set_volt_vcore(struct ptp_det *det)
{
	...
	if (set_volt_vcore_times == 1) {
		...
		/* Don't tune down Vcore voltage after running the first time in monitor mode */
		disble_vcore_tuning_machenism();
		det->ops->switch_bank(det);
		/* disable PTP VCORE */
		ptp_write(PTP_PTPEN, 0x0);
...
static void switch_to_vcore_ao(struct ptp_det *det)
{
	...
	/* 0:0 = VCORE_PTPODSEL */
	ptp_write_field(PERI_VCORE_PTPOD_CON0, 0:0, SEL_VCORE_AO);
	/* 3:0 = APBSEL */
	ptp_write_field(PTP_PTPCORESEL, 3:0, det->ctrl_id);
...
#ifdef CONFIG_PTP_VCORE_AO
	/* 0:0 = VCORE_PTPODSEL */
	ptp_write_field(PERI_VCORE_PTPOD_CON0, 0:0, SEL_VCORE_PDN);
#endif
	/* 3:0 = APBSEL*/
	ptp_write_field(PTP_PTPCORESEL, 3:0, det->ctrl_id);
	ptp_ctrls[PTP_CTRL_VCORE].det_id = det_to_id(det);
...
static enum hrtimer_restart ptp_log_timer_func(struct hrtimer *timer)
{
	...
	for_each_det(det) {
		ptp_notice(
		"PTP_LOG: PTPOD[%s] (%d) - (%d, %d, %d, %d, %d, %d, %d, %d)-",
...
static void ptp_init_det(struct ptp_det *det, struct ptp_devinfo *devinfo)
{
	...
	case PTP_DET_GPU:
		...
		break;
#ifdef CONFIG_PTP_VCORE_AO
	case PTP_DET_VCORE_AO:
		det->MDES = devinfo->SOC_AO_MDES;
		det->BDES = devinfo->SOC_AO_BDES;
		det->DCMDET = devinfo->SOC_AO_DCMDET;
		det->DCBDET = devinfo->SOC_AO_DCBDET;
...
static inline void handle_init_err_isr(struct ptp_det *det)
{
	...
	ptp_isr_info("================== PTP init err: ===================\n");
	ptp_isr_info("PTPEN = 0x%08X, PTPINTSTS = 0x%08X\n",
				ptp_read(PTP_PTPEN), ptp_read(PTP_PTPINTSTS));
	ptp_isr_info("PTP_SMSTATE0 = 0x%08X\n", ptp_read(PTP_SMSTATE0));
	ptp_isr_info("PTP_SMSTATE1 = 0x%08X\n", ptp_read(PTP_SMSTATE1));
	ptp_isr_info("================== PTP init err: ===================\n");
...
static inline void ptp_isr_handler(struct ptp_det *det)
{
	...
	if (ptp_log_en) {
		...
		ptp_isr_info("[PTPOD]: Source :\n");
		ptp_isr_info("[PTPOD]: PTP_DESCHAR = 0x%x\n", ptp_read(PTP_DESCHAR));
		ptp_isr_info("[PTPOD]: PTP_DCCONFIG = 0x%x\n", ptp_read(PTP_DCCONFIG));
		ptp_isr_info("[PTPOD]: PTP_DETCHAR = 0x%x\n", ptp_read(PTP_DETCHAR));
		ptp_isr_info("[PTPOD]: PTP_AGECONFIG = 0x%x\n",
						ptp_read(PTP_AGECONFIG));
		ptp_isr_info("[PTPOD]: PTP_AGECHAR = 0x%x\n", ptp_read(PTP_AGECHAR));
		ptp_isr_info("[PTPOD]: PTP_TEMPCHAR = 0x%x\n", ptp_read(PTP_TEMPCHAR));
		ptp_isr_info("[PTPOD]: PTP_RUNCONFIG = 0x%x\n",
						ptp_read(PTP_RUNCONFIG));
		ptp_isr_info("[PTPOD]: PTP_FREQPCT30 = 0x%x\n",
						ptp_read(PTP_FREQPCT30));
		ptp_isr_info("[PTPOD]: PTP_FREQPCT74 = 0x%x\n",
						ptp_read(PTP_FREQPCT74));
		ptp_isr_info("[PTPOD]: PTP_LIMITVALS = 0x%x\n",
						ptp_read(PTP_LIMITVALS));
		ptp_isr_info("[PTPOD]: PTP_VBOOT = 0x%x\n", ptp_read(PTP_VBOOT));
		ptp_isr_info("[PTPOD]: PTP_DETWINDOW = 0x%x\n",
						ptp_read(PTP_DETWINDOW));
		ptp_isr_info("[PTPOD]: PTP_PTPCONFIG = 0x%x\n",
						ptp_read(PTP_PTPCONFIG));
		ptp_isr_info("[PTPOD]: PTP_TSCALCS = 0x%x\n", ptp_read(PTP_TSCALCS));

		ptp_isr_info("[PTPOD]: Output check :\n");
		ptp_isr_info("[PTPOD]: PTP_PTPINTSTS = 0x%x\n",
						ptp_read(PTP_PTPINTSTS));
		ptp_isr_info("[PTPOD]: PTP_DCVALUES = 0x%x\n", ptp_read(PTP_DCVALUES));
		ptp_isr_info("[PTPOD]: PTP_AGECOUNT = 0x%x\n", ptp_read(PTP_AGECOUNT));
		ptp_isr_info("[PTPOD]: PTP_INIT2VALS = 0x%x\n",
						ptp_read(PTP_INIT2VALS));
		ptp_isr_info("[PTPOD]: PTP_VDESIGN30 = 0x%x\n",
						ptp_read(PTP_VDESIGN30));
		ptp_isr_info("[PTPOD]: PTP_VDESIGN74 = 0x%x\n",
						ptp_read(PTP_VDESIGN74));
		ptp_isr_info("[PTPOD]: PTP_TEMP = 0x%x\n", ptp_read(PTP_TEMP));
		ptp_isr_info("[PTPOD]: PTP_DVT30 = 0x%x\n", ptp_read(PTP_DVT30));
		ptp_isr_info("[PTPOD]: PTP_DVT74 = 0x%x\n", ptp_read(PTP_DVT74));
		ptp_isr_info("[PTPOD]: PTP_VOP30 = 0x%x\n", ptp_read(PTP_VOP30));
		ptp_isr_info("[PTPOD]: PTP_VOP74 = 0x%x\n", ptp_read(PTP_VOP74));
	}
...
static irqreturn_t ptp_isr(int irq, void *dev_id)
{
	...
	if (!(BIT(PTP_CTRL_VCORE) & ptp_read(PTP_PTPODINTST))) {
		/* 0:0 = VCORE_PTPODSEL */
		switch (ptp_read_field(PERI_VCORE_PTPOD_CON0, 0:0)) {
		case SEL_VCORE_AO:
...
void ptp_init01(void)
{
	...
	for_each_det_ctrl(det, ctrl) {
		...
		if (atomic_read(&ctrl->in_init)) {
			...
			ptp_error("@%s():%d, wait_for_completion(%s) in\n",
						__func__, __LINE__, det->name);
			wait_for_completion(&ctrl->init_done);
			ptp_error("@%s():%d, wait_for_completion(%s) out\n",
						__func__, __LINE__, det->name);
		}
	}
...
static int ptp_probe(struct platform_device *pdev)
{
	...
	/* If chip ic is E2, we don't need to tune Vcore voltage down. */
	/* if (mt_get_chip_sw_ver() == CHIP_SW_VER_02) {
		ptp_error("This AP is E2 IC so don't tune vcore voltage down.\n");
		disble_vcore_tuning_machenism();
	} */
	...
	/* disable DVFS and set vproc = 1.00v when ptpod_id == 1 */
	mt_cpufreq_disable_by_ptpod(MT_CPU_DVFS_LITTLE);
	mt_cpufreq_disable_by_ptpod(MT_CPU_DVFS_BIG);
#ifdef VCORE_1P0V
	ret = regulator_count_voltages(reg_vcore);
	ret = regulator_list_voltage(reg_vcore, ret - 1);
	...
	ptp_init01();
	ptp_error("PTP-OD module initiailization done\n");
...
struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
	...
	case 1:
		ptp_log_en = 0;
		ptp_notice("ptp log enabled, ptp_log_en = 0.\n");
		hrtimer_start(&ptp_log_timer, ns_to_ktime(LOG_INTERVAL),
						HRTIMER_MODE_REL);
		break;

	case 2:
		ptp_notice("ptp log enabled, ptp_log_en = 1.\n");
		hrtimer_start(&ptp_log_timer, ns_to_ktime(LOG_INTERVAL),
						HRTIMER_MODE_REL);
		break;
...
static int __init ptp_init(void)
{
	...
	ptp_error("ptpod_irq_number = 0x%x\n", ptpod_irq_number);
	ptp_error("[PTP] ptpod_base=0x%16p\n", (void *)ptpod_base);
